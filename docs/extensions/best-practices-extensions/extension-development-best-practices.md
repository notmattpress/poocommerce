...
# PooCommerce extension development best practices
...
1. **Check if PooCommerce is active**. Most PooCommerce plugins do not need to run unless PooCommerce is already active. [Learn how to check if PooCommerce is active](/docs/extensions/core-concepts/check-if-woo-is-active).
...
23. **Never use internal PooCommerce code in your extensions**. Classes in the `Automattic\PooCommerce\Internal` namespace and code marked with `@internal` annotations are for PooCommerce core use only. **Backwards compatibility is not guaranteed** for internal code: it may change, be renamed, or be removed in any future release. Using internal code in your extension may cause it to break when merchants update PooCommerce. See the [Internal namespace documentation](https://github.com/poocommerce/poocommerce/blob/trunk/plugins/poocommerce/src/Internal/README.md) for details.
