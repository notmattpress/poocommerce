...
- `$context` *(array)* – The existing context data array. This may already contain data from PooCommerce core or other extensions.
- `$email` *(\WC_Email)* – The PooCommerce email object being processed. You can use this to access the email ID, recipient, and the object associated with the email (such as an order or customer).
