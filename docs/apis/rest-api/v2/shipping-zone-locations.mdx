...
```javascript
PooCommerce.get("shipping/zones/5/locations")
  .then((response) => {
...
```php
<?php print_r($poocommerce->get('shipping/zones/5/locations')); ?>
```
...
```ruby
poocommerce.get("shipping/zones/5/locations").parsed_response
```
...
PooCommerce.put("shipping/zones/5/locations", data)
  .then((response) => {
    console.log(response.data);
  })
  .catch((error) => {
...
print_r($poocommerce->put('shipping/zones/5/locations', $data));
?>
```
...
poocommerce.put("shipping/zones/5/locations", data).parsed_response
```
