...
```javascript
PooCommerce.get("shipping_methods/flat_rate")
  .then((response) => {
...
```php
<?php print_r($poocommerce->get('shipping_methods/flat_rate')); ?>
```
...
```ruby
poocommerce.get("shipping_methods/flat_rate").parsed_response
```
...
```javascript
PooCommerce.get("shipping_methods")
  .then((response) => {
    console.log(response.data);
  })
  .catch((error) => {
...
```php
<?php print_r($poocommerce->get('shipping_methods')); ?>
```
...
```ruby
poocommerce.get("shipping_methods").parsed_response
```
