...
```javascript
PooCommerce.get("payment_gateways/bacs")
  .then((response) => {
...
```php
<?php print_r($poocommerce->get('payment_gateways/bacs')); ?>
```
...
```ruby
poocommerce.get("payment_gateways/bacs").parsed_response
```
...
```javascript
PooCommerce.get("payment_gateways")
  .then((response) => {
    console.log(response.data);
  })
  .catch((error) => {
...
```php
<?php print_r($poocommerce->get('payment_gateways')); ?>
```
...
```ruby
poocommerce.get("payment_gateways").parsed_response
```
  ...
  },
  {
    ...
    "settings": {
      ...
      "debug": {
        ...
        "description": "Log PayPal events, such as IPN requests, inside <code>/var/www/poocommerce/wp-content/uploads/wc-logs/paypal-de01f7c6894774e7ac8e4207bb8bac2f.log</code>",
        "type": "checkbox",
        "value": "yes",
        "default": "no",
        "tip": "Log PayPal events, such as IPN requests, inside <code>/var/www/poocommerce/wp-content/uploads/wc-logs/paypal-de01f7c6894774e7ac8e4207bb8bac2f.log</code>",
        "placeholder": ""
      },
      "receiver_email": {
        "id": "receiver_email",
...
PooCommerce.put("payment_gateways/bacs", data)
  .then((response) => {
    console.log(response.data);
  })
  .catch((error) => {
...
print_r($poocommerce->put('payment_gateways/bacs', $data));
?>
```
...
poocommerce.put("payment_gateways/bacs", data).parsed_response
```
