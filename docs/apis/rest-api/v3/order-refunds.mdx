...
| `compute_totals`   | boolean   | When true, the server computes per-line refund amounts from quantities and validates the request against the order's refund history. Available since PooCommerce 11.1. See [Server-computed refunds](#server-computed-refunds-compute_totals). Default is `false`. `WRITE-ONLY` |
...
PooCommerce.post( 'orders/723/refunds', data )
	.then( ( response ) => {
...
print_r($poocommerce->post('orders/723/refunds', $data));
?>
```
...
poocommerce.post("orders/723/refunds", data).parsed_response
```
...
Available since PooCommerce 11.1.
...
When `amount` is supplied together with `compute_totals`, it must be at least the computed line items total and no more than the order's remaining refundable amount. Requests that exceed what is still refundable are rejected with HTTP 400 or 422 and error codes such as `poocommerce_rest_invalid_refund_amount`, `poocommerce_rest_refund_exceeds_remaining`, `poocommerce_rest_quantity_exceeds_refundable`, and `poocommerce_rest_duplicate_line_item`.

::::warning Older stores silently ignore compute_totals

Stores running PooCommerce below 11.1 drop the unknown `compute_totals` parameter and process the request with the classic behavior. A quantity-only request then creates a refund of `0.00` instead of the intended amount. Before sending computed-form requests, verify that the store supports the flag:
...
Invalid requests return the same errors as creation with `compute_totals`, for example `poocommerce_rest_invalid_refund_amount` for a non-positive total and `poocommerce_rest_preview_exceeds_max_refundable` with HTTP 422 when the preview exceeds the remaining refundable amount.
...
```javascript
PooCommerce.get( 'orders/723/refunds/726' )
	.then( ( response ) => {
		console.log( response.data );
	} )
	.catch( ( error ) => {
...
```php
<?php print_r($poocommerce->get('orders/723/refunds/726')); ?>
```
...
```ruby
poocommerce.get("orders/723/refunds/726").parsed_response
```
...
```javascript
PooCommerce.get( 'orders/723/refunds' )
	.then( ( response ) => {
		console.log( response.data );
	} )
	.catch( ( error ) => {
...
```php
<?php print_r($poocommerce->get('orders/723/refunds')); ?>
```
...
```ruby
poocommerce.get("orders/723/refunds").parsed_response
```
...
```javascript
PooCommerce.delete( 'orders/723/refunds/726', {
	force: true,
} )
	.then( ( response ) => {
		console.log( response.data );
...
```php
<?php print_r($poocommerce->delete('orders/723/refunds/726', ['force' => true])); ?>
```
...
```ruby
poocommerce.delete("orders/723/refunds/726", force: true).parsed_response
```
