...
PooCommerce.post( 'orders/723/actions/send_order_details', data )
	.then( ( response ) => {
...
print_r($poocommerce->post('orders/723/actions/send_order_details', $data));
?>
```
...
poocommerce.post("orders/723/actions/send_order_details", data).parsed_response
```
...
```json
{
	"code": "poocommerce_rest_missing_email",
...
PooCommerce.post( 'orders/723/actions/send_email', data )
	.then( ( response ) => {
		console.log( response.data );
	} )
	.catch( ( error ) => {
...
print_r($poocommerce->post('orders/723/actions/send_email', $data));
?>
```
...
poocommerce.post("orders/723/actions/send_email", data).parsed_response
```
...
```json
{
	"code": "poocommerce_rest_invalid_email_template",
...
```javascript
PooCommerce.get( 'orders/723/actions/email_templates' )
	.then( ( response ) => {
		console.log( response.data );
	} )
	.catch( ( error ) => {
...
```php
<?php
print_r($poocommerce->get('orders/723/actions/email_templates'));
?>
```
...
```ruby
poocommerce.post("orders/723/actions/email_templates").parsed_response
```
