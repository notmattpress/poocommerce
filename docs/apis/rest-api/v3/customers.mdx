...
```php
<?php print_r($poocommerce->get('customers/25')); ?>
```
...
```ruby
poocommerce.get("customers/25").parsed_response
```
...
PooCommerce.put( 'customers/25', data )
	.then( ( response ) => {
		console.log( response.data );
	} )
	.catch( ( error ) => {
...
print_r($poocommerce->put('customers/25', $data));
?>
```
...
poocommerce.put("customers/25", data).parsed_response
```
...
```javascript
PooCommerce.delete( 'customers/25', {
	force: true,
} )
	.then( ( response ) => {
		console.log( response.data );
...
```php
<?php print_r($poocommerce->delete('customers/25', ['force' => true])); ?>
```
...
```ruby
poocommerce.delete("customers/25", force: true).parsed_response
```
...
```javascript
PooCommerce.get( 'customers/26/downloads' )
	.then( ( response ) => {
		console.log( response.data );
	} )
	.catch( ( error ) => {
...
```php
<?php print_r($poocommerce->get('customers/26/downloads')); ?>
```
...
```ruby
poocommerce.get("customers/26/downloads").parsed_response
```
...
[
	{
		...
		"file": {
			"name": "Song 2",
			"file": "http://example.com/wp-content/uploads/poocommerce_uploads/2013/06/Song.mp3"
		},
		"_links": {
			"collection": [
				{
