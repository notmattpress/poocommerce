...
```javascript
PooCommerce.get( 'products/custom-fields/names' )
	.then( ( response ) => {
...
```php
<?php print_r($poocommerce->get('products/custom-fields/names')); ?>
```
...
```ruby
poocommerce.get("products/custom-fields/names").parsed_response
```
