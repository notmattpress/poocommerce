...
PooCommerce.post( 'taxes/classes', data )
	.then( ( response ) => {
...
print_r($poocommerce->post('taxes/classes', $data));
?>
```
...
poocommerce.post("taxes/classes", data).parsed_response
```
...
```php
<?php print_r($poocommerce->get('taxes/classes')); ?>
```
...
```ruby
poocommerce.get("taxes/classes").parsed_response
```
...
```javascript
PooCommerce.delete( 'taxes/classes/zero-rate', {
	force: true,
} )
	.then( ( response ) => {
		console.log( response.data );
...
```php
<?php print_r($poocommerce->delete('taxes/classes/zero-rate', ['force' => true])); ?>
```
...
```ruby
poocommerce.delete("taxes/classes/zero-rate", force: true).parsed_response
```
