...
| `version`              | string    | Version of PooCommerce when the order was made. `READ-ONLY`                                                                            |
...
```php
<?php print_r($poocommerce->get('orders/154')); ?>
```
...
```ruby
poocommerce.get("orders/154").parsed_response
```
...
PooCommerce.put("orders/154", data)
  .then((response) => {
    console.log(response.data);
  })
  .catch((error) => {
...
print_r($poocommerce->put('orders/154', $data));
?>
```
...
poocommerce.put("orders/154", data).parsed_response
```
...
```php
<?php print_r($poocommerce->delete('orders/154', ['force' => true])); ?>
```
...
```ruby
poocommerce.delete("orders/154", force: true).parsed_response
```
