...
Webhooks can be managed via the PooCommerce settings screen or by using the REST API endpoints. The `WC_Webhook` class manages all data storage and retrieval of the webhook custom post type, as well as enqueuing webhook actions and processing/delivering/logging webhooks. On `poocommerce_init`, active webhooks are loaded.
...
### Topics
The topic is a combination resource (e.g. order) and event (e.g. created) and maps to one or more hook names (e.g. `poocommerce_checkout_order_processed`). Webhooks can be created using the topic name and the appropriate hooks are automatically added.
...
Custom topics can also be used which map to a single hook name, for example you could add a webhook with topic `action.poocommerce_add_to_cart` that is triggered on that event. Custom topics pass the first hook argument to the payload, so in this example the `cart_item_key` would be included in the payload.
...
### Visual interface
You can find the Webhooks interface going to "PooCommerce" > "Settings" > "API" > "Webhooks", see our [Visual Webhooks docs](https://docs.poocommerce.com/document/webhooks/) for more details.
...
| `hooks`         | array     | PooCommerce action names associated with the webhook. `READ-ONLY`                                                                                      |
...
|         Attribute          |   Type  |                                                              Description                                                              |
|----------------------------|---------|---------------------------------------------------------------------------------------------------------------------------------------|
| `User-Agent`               | string  | The request user agent, default is "PooCommerce/\{version\} Hookshot (WordPress/\{version\})". `READ-ONLY`  |
...
PooCommerce.post("webhooks", data)
  .then((response) => {
...
print_r($poocommerce->post('webhooks', $data));
?>
```
...
poocommerce.post("webhooks", data).parsed_response
```
...
```json
{
  ...
  "event": "updated",
  "hooks": [
    "poocommerce_process_shop_order_meta",
    "poocommerce_api_edit_order",
    "poocommerce_order_edit_status",
    "poocommerce_order_status_changed"
  ],
  "delivery_url": "http://requestb.in/1g0sxmo1",
...
```php
<?php print_r($poocommerce->get('webhooks/142')); ?>
```
...
```ruby
poocommerce.get("webhooks/142").parsed_response
```
...
```php
<?php print_r($poocommerce->get('webhooks')); ?>
```
...
```ruby
poocommerce.get("webhooks").parsed_response
```
...
[
  {
    ...
    "hooks": [
      ...
      "poocommerce_api_create_customer"
    ],
    "delivery_url": "http://requestb.in/1g0sxmo1",
    "date_created": "2016-05-15T20:17:52",
    "date_modified": "2016-05-15T20:17:52",
...
PooCommerce.put("webhooks/142", data)
  .then((response) => {
    console.log(response.data);
  })
  .catch((error) => {
...
print_r($poocommerce->put('webhooks/142', $data));
?>
```
...
poocommerce.put("webhooks/142", data).parsed_response
```
...
```javascript
PooCommerce.delete("webhooks/142")
  .then((response) => {
    console.log(response.data);
  })
  .catch((error) => {
...
```php
<?php print_r($poocommerce->delete('webhooks/142')); ?>
```
...
```ruby
poocommerce.delete("webhooks/142").parsed_response
```
...
PooCommerce.post("webhooks/batch", data)
  .then((response) => {
    console.log(response.data);
  })
  .catch((error) => {
...
print_r($poocommerce->post('webhooks/batch', $data));
?>
```
...
poocommerce.post("webhooks/batch", data).parsed_response
```
...
```javascript
PooCommerce.get("webhooks/142/deliveries/54")
  .then((response) => {
    console.log(response.data);
  })
  .catch((error) => {
...
```php
<?php print_r($poocommerce->get('webhooks/142/deliveries/54')); ?>
```
...
```ruby
poocommerce.get("webhooks/142/deliveries/54").parsed_response
```
...
```json
{
  ...
  "request_headers": {
    "User-Agent": "PooCommerce/2.6.0 Hookshot (WordPress/4.5.2)",
...
```javascript
PooCommerce.get("webhooks/142/deliveries")
  .then((response) => {
    console.log(response.data);
  })
  .catch((error) => {
...
```php
<?php print_r($poocommerce->get('webhooks/142/deliveries')); ?>
```
...
```ruby
poocommerce.get("webhooks/142/deliveries").parsed_response
```
