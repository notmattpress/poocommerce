...
```php
<?php print_r($poocommerce->get('coupons/113')); ?>
```
...
```ruby
poocommerce.get("coupons/113").parsed_response
```
...
PooCommerce.put("coupons/113", data)
  .then((response) => {
    console.log(response.data);
  })
  .catch((error) => {
...
print_r($poocommerce->put('coupons/113', $data)); 
?>
```
...
poocommerce.put("coupons/113", data).parsed_response
```
...
```javascript
PooCommerce.delete("coupons/113", {
  force: true
})
  .then((response) => {
    console.log(response.data);
...
```php
<?php print_r($poocommerce->delete('coupons/113', ['force' => true])); ?>
```
...
```ruby
poocommerce.delete("coupons/113", force: true).parsed_response
```
