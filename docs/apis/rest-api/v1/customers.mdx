...
| `username`      | string    | Customer login name. Can be generated automatically from the customer's email address if the option `poocommerce_registration_generate_username` is equal to `yes` `CANNOT BE CHANGED` `MAYBE MANDATORY`                                                                                               |
...
PooCommerce.post("customers", data)
  .then((response) => {
...
print_r($poocommerce->post('customers', $data));
?>
```
...
poocommerce.post("customers", data).parsed_response
```
...
```php
<?php print_r($poocommerce->get('customers/2')); ?>
```
...
```ruby
poocommerce.get("customers/2").parsed_response
```
...
```php
<?php print_r($poocommerce->get('customers')); ?>
```
...
```ruby
poocommerce.get("customers").parsed_response
```
...
PooCommerce.put("customers/2", data)
  .then((response) => {
    console.log(response.data);
  })
  .catch((error) => {
...
print_r($poocommerce->put('customers/2', $data));
?>
```
...
poocommerce.put("customers/2", data).parsed_response
```
...
```javascript
PooCommerce.delete("customers/2", {
  force: true
})
  .then((response) => {
    console.log(response.data);
...
```php
<?php print_r($poocommerce->delete('customers/2', ['force' => true])); ?>
```
...
```ruby
poocommerce.delete("customers/2", force: true).parsed_response
```
...
PooCommerce.post("customers/batch", data)
  .then((response) => {
    console.log(response.data);
  })
  .catch((error) => {
...
print_r($poocommerce->post('customers/batch', $data));
?>
```
...
poocommerce.post("customers/batch", data).parsed_response
```
...
```javascript
PooCommerce.get("customers/2/downloads")
  .then((response) => {
    console.log(response.data);
  })
  .catch((error) => {
...
```php
<?php print_r($poocommerce->get('customers/2/downloads')); ?>
```
...
```ruby
poocommerce.get("customers/2/downloads").parsed_response
```
