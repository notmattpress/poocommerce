...
*Note: Keys/Apps was found at PooCommerce > Settings > API > Key/Apps prior to PooCommerce 3.4.*

![PooCommerce REST API keys settings](v3/images/poocommerce-api-keys-settings.png)
...
![Creating a new REST API key](v3/images/poocommerce-creating-api-keys.png)
...
![Generated REST API key](v3/images/poocommerce-api-key-generated.png)
...
This endpoint can be used by any APP to *allow users to generate API keys* for your APP. This makes integration with PooCommerce API easier because the user only needs to grant access to your APP via a URL. After being redirected back to your APP, the API keys will be sent back in a separate POST request.

The following image illustrates how this works:

![Authentication Endpoint flow](v3/images/poocommerce-auth-endpoint-flow.png)

:::warning
This endpoint works exclusively for users to generate API keys and facilitate integration between the PooCommerce REST API and an application. In no way is this endpoint intended to be used as login method for customers.
:::
...
![Authentication Endpoint example](v3/images/poocommerce-auth-endpoint-example.png)
...
```javascript
const PooCommerceRestApi = require("@poocommerce/poocommerce-rest-api").default;
// import PooCommerceRestApi from "@poocommerce/poocommerce-rest-api"; // Supports ESM

const PooCommerce = new PooCommerceRestApi({
...
use Automattic\PooCommerce\Client;

$poocommerce = new Client(
    'https://example.com',
    'consumer_key',
    'consumer_secret',
    [
...
```ruby
require "poocommerce_api"

poocommerce = PooCommerce::API.new(
  "https://example.com",
  "consumer_key",
  "consumer_secret",
  {
...
* Authorization header is supported starting PooCommerce 3.0.
