...
pnpm --filter=@poocommerce/plugin-poocommerce build:api

# Regenerate the test fixture's tree (the DummyApi used by the test suite)
pnpm --filter=@poocommerce/plugin-poocommerce build:api:test

# Check whether the committed output is stale (used by CI)
pnpm --filter=@poocommerce/plugin-poocommerce build:api:check
```
...
- `plugins/poocommerce/src/Api/**`
- `plugins/poocommerce/src/Internal/Api/Autogenerated/**`
- `plugins/poocommerce/bin/api-builder/**`
