...
This guide covers how to add to or change the code API (the PHP classes the GraphQL layer is generated from). It applies both to PooCommerce core's own code API and to the ones implemented by plugins (see [Creating a dual API in a plugin](./creating-a-dual-api-in-a-plugin.md) for the plugin-specific bootstrap).
...
2. Regenerate the GraphQL layer: `pnpm --filter=@poocommerce/plugin-poocommerce build:api`.
