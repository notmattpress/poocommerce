...
| --- | --- | --- |
| `id` | integer \| null | The order item metadata row ID. `null` when no displayed metadata row backs the entry, which is the case for anything an extension added through `poocommerce_order_item_get_formatted_meta_data`. |
