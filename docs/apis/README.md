...
The dual API is an experimental, code-first API architecture: you write plain PHP classes (the code API) and a build script generates a matching GraphQL endpoint from them. The engine is provided by the PooCommerce Dual API plugin, and any plugin can use it to build its own dual API.
