...
1. **Run the [“Release: Compile changelog” workflow](https://github.com/poocommerce/poocommerce/actions/workflows/release-compile-changelog.yml).**
...
2. **(Skip for `-rc.1`) Run the version-bump workflow.**
   - Update the PooCommerce version in relevant files on the release branch.
   - Review and merge the PR after CI passes.
3. **Build the release ZIP file.**
   - Run the [“Release: Build ZIP file” workflow](https://github.com/poocommerce/poocommerce/actions/workflows/release-build-zip-file.yml) from the release branch.
