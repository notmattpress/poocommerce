...
When a security fix warrants it (CVSS score >=9), PooCommerce will provide security patches for the **last 21 major versions**. If the current stable PooCommerce version is 11.0, that means version **9.0 and newer**. Otherwise, we publish fixes for the latest version.
