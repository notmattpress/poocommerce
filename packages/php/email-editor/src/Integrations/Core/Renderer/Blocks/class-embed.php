...
class Embed extends Abstract_Block_Renderer {
	...
	private function get_translated_provider_label( string $provider ): string {
		switch ( $provider ) {
			...
			case 'vimeo':
				return __( 'Watch on Vimeo', 'poocommerce' );
			case 'tiktok':
				return __( 'Watch on TikTok', 'poocommerce' );
			case 'dailymotion':
				return __( 'Watch on Dailymotion', 'poocommerce' );
			default:
	...
	private function render_link_embed_card( string $url, array $parsed_block, Rendering_Context $rendering_context ): string {
		...
		if ( ! empty( $excerpt ) ) {
			...
			$content_parts .= sprintf(
				...
				esc_url( $url ),
				esc_attr( $link_color ),
				esc_html__( 'Continue reading', 'poocommerce' )
			);
		}
