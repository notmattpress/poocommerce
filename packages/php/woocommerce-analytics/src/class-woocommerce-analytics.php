...
class Woocommerce_Analytics {
	 ...
	 * @var string
	 */
	const PROXY_SPEED_MODULE_AUTHORIZED_OPTION = 'poocommerce_analytics_proxy_speed_module_authorized';
	 ...
	 * @var string
	 */
	const PROXY_TRACKING_EVER_ENABLED_OPTION = 'poocommerce_analytics_proxy_tracking_ever_enabled';
	...
	public static function maybe_add_proxy_speed_module() {
		...
		if ( ! self::init_filesystem() ) {
			if ( function_exists( 'wc_get_logger' ) ) {
				wc_get_logger()->error( 'PooCommerce Analytics proxy speed module not installed: filesystem unavailable.', array( 'source' => 'poocommerce-analytics' ) );
			}
			return;
		}
		...
		if ( ! is_dir( WPMU_PLUGIN_DIR ) ) {
			if ( function_exists( 'wc_get_logger' ) ) {
				wc_get_logger()->error( 'PooCommerce Analytics proxy speed module not installed: mu-plugins directory could not be created.', array( 'source' => 'poocommerce-analytics' ) );
			}
			return;
		}
