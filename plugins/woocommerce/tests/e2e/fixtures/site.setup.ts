...
setup( 'setup site', async ( { baseURL, restApi } ) => {
	...
	await setup.step( 'dismiss email improvements prompts', async () => {
		await setOption(
			request,
			baseURL,
			'poocommerce_admin_dismissed_email_improvements_modal',
			'yes'
		);
		await setOption(
			request,
			baseURL,
			'poocommerce_admin_dismissed_try_email_improvements_modal',
			'yes'
		);
	} );
	...
	await setup.step( 'determine if multisite', async () => {
		...
		// remote hosts on arbitrary PooCommerce versions, where a filter on
		// `poocommerce_rest_prepare_system_status` could return a stringy value.
