...
class EmailLoggerTest extends WC_Unit_Test_Case {
	...
	public function tearDown(): void {
		...
		remove_all_filters( 'poocommerce_mail_callback' );
	...
	public function test_send_returns_false_and_warns_when_mail_callback_returns_non_scalar(): void {
		...
		$this->assertStringContainsString( 'poocommerce_mail_callback filter should return a boolean; object returned.', $notices[0]['message'] );
	...
	public function test_send_coerces_scalar_callback_return_and_warns( $callback_return, bool $expected, string $expected_type ): void {
		...
		$this->assertNotEmpty( $notices );
		$this->assertStringContainsString( "poocommerce_mail_callback filter should return a boolean; {$expected_type} returned.", $notices[0]['message'] );
	}
	...
	/**
	 * @testdox send() passes false to poocommerce_email_sent when the mail callback returns a non-bool.
	 */
	public function test_send_fires_email_sent_action_with_bool_when_callback_returns_non_bool(): void {
		...
		$this->assertIsBool( $received_return, 'poocommerce_email_sent should receive a bool even when the mail callback returns a non-bool' );
		$this->assertFalse( $received_return );

		remove_all_actions( 'poocommerce_email_sent' );
	}
}
