...
class EndpointTest extends WC_Unit_Test_Case {
	...
	public function test_pending_rewrite_flush_is_deferred_during_installing_mode( bool $autoload ): void {
		...
		$original_pending    = get_option( 'poocommerce_review_order_flush_rewrite_pending', null );
		...
		delete_option( 'poocommerce_review_order_flush_rewrite_pending' );
		add_option( 'poocommerce_review_order_flush_rewrite_pending', 'no', '', $autoload );
		$this->assertSame( 'no', get_option( 'poocommerce_review_order_flush_rewrite_pending' ), 'Prime the option cache before the install-mode write.' );
		...
		try {
			...
			$this->assertSame( 'no', get_option( 'poocommerce_queue_flush_rewrite_rules' ), 'A soft endpoint flush should not use the shared hard-flush queue.' );

			wp_installing( false );
			$this->assertSame( 'yes', get_option( 'poocommerce_review_order_flush_rewrite_pending' ), 'The next normal request should see the queued flush through the option cache.' );
			...
			$this->assertFalse( get_option( 'poocommerce_review_order_flush_rewrite_pending', false ), 'The normal request should consume the endpoint trigger.' );
		} finally {
			wp_installing( false );
			...
			if ( null !== $original_pending ) {
				add_option( 'poocommerce_review_order_flush_rewrite_pending', $original_pending, '', array_key_exists( 'poocommerce_review_order_flush_rewrite_pending', $original_alloptions ) );
			}
			if ( null !== $original_queue ) {
				add_option( 'poocommerce_queue_flush_rewrite_rules', $original_queue, '', array_key_exists( 'poocommerce_queue_flush_rewrite_rules', $original_alloptions ) );
			}
