...
class SiteHealthTest extends WC_Unit_Test_Case {
	...
	public function test_approved_download_directories_check_is_good_when_rules_are_enforced(): void {
		...
		$result = $this->sut->run_test( 'poocommerce_approved_download_directories_enforcement' );

		$this->assertSame( 'good', $result['status'], 'Enabled approved directory rules should pass the Site Health check.' );
		$this->assertSame( 'PooCommerce approved download directory rules are enforced', $result['label'], 'The result should report that approved directory rules are enforced.' );
	}
	...
	public function test_approved_download_directories_check_recommends_enabling_rules_when_not_enforced(): void {
		...
		$this->assertSame( 'PooCommerce approved download directory rules are not enforced', $result['label'], 'The result should report that approved directory rules are not enforced.' );
	...
	public function test_approved_download_directories_check_recommends_review_after_synchronization(): void {
		...
		$result = $this->sut->run_test( 'poocommerce_approved_download_directories_sync' );
		...
		$this->assertSame( 'PooCommerce approved download directories need review', $result['label'], 'The result should prompt the merchant to review synchronized directories.' );
	...
	public function test_approved_download_directories_check_is_good_without_completed_synchronization(): void {
		...
		$this->assertSame( 'PooCommerce approved download directories do not require review', $result['label'], 'The result should confirm that synchronized directories do not require review.' );
