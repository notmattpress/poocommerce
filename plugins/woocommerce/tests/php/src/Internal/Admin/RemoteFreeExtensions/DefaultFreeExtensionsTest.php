...
class DefaultFreeExtensionsTest extends WC_Unit_Test_Case {
	 ...
	 * @dataProvider core_profiler_poocommerce_payments_visibility_provider
	 *
	 * @param string $country        Store country and optional state.
	 * @param bool   $should_include Whether WooPayments should be recommended.
	 */
	public function test_core_profiler_poocommerce_payments_visibility_by_country( string $country, bool $should_include ): void {
		update_option( 'poocommerce_default_country', $country );
		update_option( 'poocommerce_store_address', '1 Test Street' );
		update_option( 'poocommerce_remote_variant_assignment', 60 );
		update_option( 'active_plugins', array() );
		update_option( 'poocommerce_onboarding_profile', array() );
		...
		if ( $should_include ) {
			$this->assertContains( 'poocommerce-payments', $plugin_slugs );
		} else {
			$this->assertNotContains( 'poocommerce-payments', $plugin_slugs );
		}
	}
	 ...
	 */
	public function core_profiler_poocommerce_payments_visibility_provider(): array {
		return array(
