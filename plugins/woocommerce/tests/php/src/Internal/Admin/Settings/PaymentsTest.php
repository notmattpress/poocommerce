...
class PaymentsTest extends WC_REST_Unit_Test_Case {
	...
	public function test_get_payment_providers_only_gateways_no_suggestions() {
		...
		$gateways = array(
			new FakePaymentGateway( 'paypal', array( 'plugin_slug' => 'poocommerce' ) ),
			// The offline PMs.
			new FakePaymentGateway( 'bacs', array( 'plugin_slug' => 'poocommerce' ) ),
			new FakePaymentGateway( 'cheque', array( 'plugin_slug' => 'poocommerce' ) ),
			new FakePaymentGateway( 'cod', array( 'plugin_slug' => 'poocommerce' ) ),
		);
		$this->mock_providers
			->expects( $this->atLeastOnce() )
			->method( 'get_payment_gateways' )
