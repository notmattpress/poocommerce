...
namespace Automattic\PooCommerce\Tests\Internal\Admin\Settings\PaymentProviders\WooPayments;
...
class WooPaymentsServiceTest extends WC_Unit_Test_Case {
	...
	public function test_onboarding_test_account_init_with_existing_test_account() {
		...
		$this->assertEquals(
			array(
				'message' => __( 'A test account is already set up.', 'poocommerce' ),
			),
			$result
		);
	...
	public function test_onboarding_test_account_init_with_existing_live_account() {
		...
		$this->assertEquals(
			array(
				'message' => __( 'An account is already set up. Reset the onboarding first.', 'poocommerce' ),
			),
			$result
		);
		$this->assertCount( 0, $requests_made );
