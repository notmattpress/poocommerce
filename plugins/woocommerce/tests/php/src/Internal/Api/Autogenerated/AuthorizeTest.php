...
class AuthorizeTest extends AutogeneratedTestCase {
	...
	public function test_ignored_authorize_method_is_skipped(): void {
		...
		$this->assertMatchesRegularExpression(
			'/new \\\\Automattic\\\\PooCommerce\\\\Api\\\\Attributes\\\\RequiredCapability\\(\s*\'manage_options\'\s*\\)/',
			$source
		);
	...
	public function test_composed_authorize_receives_preauthorized_flag(): void {
		...
		$this->assertMatchesRegularExpression(
			'/public static function compute_preauthorized\\([^)]*\\): bool\\s*\\{\\s*return \\(\\s*new \\\\Automattic\\\\PooCommerce\\\\Api\\\\Attributes\\\\RequiredCapability\\(\s*\'manage_options\'\s*\\)\s*\\)->authorize\\(\s*\\$principal\s*\\);/',
			$source
		);
