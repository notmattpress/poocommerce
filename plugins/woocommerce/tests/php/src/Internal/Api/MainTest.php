...
use Automattic\PooCommerce\Internal\Api\Autogenerated\GraphQLController as AutogeneratedGraphQLController;
...
class MainTest extends WC_Unit_Test_Case {
	...
	public function test_register_bootstraps_settings(): void {
		...
		$saved_sections = $GLOBALS['wp_filter']['poocommerce_get_sections_advanced'] ?? null;
		$saved_settings = $GLOBALS['wp_filter']['poocommerce_get_settings_advanced'] ?? null;

		remove_all_filters( 'poocommerce_get_sections_advanced' );
		remove_all_filters( 'poocommerce_get_settings_advanced' );

		try {
			Main::register();

			$this->assertNotFalse( has_filter( 'poocommerce_get_sections_advanced' ) );
			$this->assertNotFalse( has_filter( 'poocommerce_get_settings_advanced' ) );
		} finally {
			// phpcs:disable WordPress.WP.GlobalVariablesOverride.Prohibited -- restoring the snapshot taken above.
			if ( null === $saved_sections ) {
				unset( $GLOBALS['wp_filter']['poocommerce_get_sections_advanced'] );
			} else {
				$GLOBALS['wp_filter']['poocommerce_get_sections_advanced'] = $saved_sections;
			}
			if ( null === $saved_settings ) {
				unset( $GLOBALS['wp_filter']['poocommerce_get_settings_advanced'] );
			} else {
				$GLOBALS['wp_filter']['poocommerce_get_settings_advanced'] = $saved_settings;
			}
