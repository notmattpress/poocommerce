...
use Automattic\PooCommerce\Tests\Internal\Api\Fixtures\DummyApi\Infrastructure\ClassResolver as DummyContainer;
use Automattic\PooCommerce\Tests\Internal\Api\Fixtures\DummyApi\Store as DummyStore;
...
class GraphQLControllerExecutionTest extends WC_REST_Unit_Test_Case {
	...
	/**
	 * @testdox handle_request allows introspection for users with manage_poocommerce.
	 */
	...
	/**
	 * @testdox the poocommerce_graphql_can_introspect filter can grant introspection to an anonymous request.
	 */
	public function test_filter_can_grant_introspection_to_anonymous_user(): void {
		...
		add_filter( 'poocommerce_graphql_can_introspect', $filter, 10, 3 );
		...
		try {
			...
			remove_filter( 'poocommerce_graphql_can_introspect', $filter, 10 );
		}
	...
	/**
	 * @testdox the poocommerce_graphql_can_introspect filter can revoke introspection from an admin.
	 */
	public function test_filter_can_revoke_introspection_from_admin(): void {
		...
		add_filter( 'poocommerce_graphql_can_introspect', $filter );
		...
		try {
			...
			remove_filter( 'poocommerce_graphql_can_introspect', $filter );
		}
	...
	/**
	 * @testdox the poocommerce_graphql_can_introspect filter must return strictly true; truthy non-bool denies.
	 */
	 ...
	 * the controller falls back to a {@see \Automattic\PooCommerce\Api\Infrastructure\Principal}
	...
	/**
	 * @testdox a throw from the poocommerce_graphql_can_introspect filter callback denies introspection (fail-closed).
	 */
