...
class DefaultShippingPartnersTest extends WC_Unit_Test_Case {
	...
	public function test_wcshipping_is_present() {
		...
		$this->assertContains( 'poocommerce-shipping', $ids );
	}
	...
	public function test_no_extensions_are_recommended_if_poocommerce_shipping_is_active() {
		...
		$this->assertNotContains( 'poocommerce-shipping', $ids );
	...
	/**
	 * Asserts the PooCommerce Shipping spec includes a layout_row block with required fields.
	 *
	 * @return void
	 */
	...
	/**
	 * Asserts the PooCommerce Shipping spec declares both row and column layouts as available.
	 *
	 * @return void
	 */
	...
	/**
	 * Returns the PooCommerce Shipping raw spec array, failing the test if not found.
	 *
	 * @return array
	 */
	private function get_wcshipping_spec(): array {
		...
		foreach ( $specs as $spec ) {
			if ( 'poocommerce-shipping' === $spec['id'] ) {
				return $spec;
			}
		}

		$this->fail( 'PooCommerce Shipping spec not found.' );
	}
}
