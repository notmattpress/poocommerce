...
class Cart extends ControllerTestCase {
	...
	public function test_shipping_topology(
		...
		try {
			...
			update_option( 'poocommerce_default_customer_address', 'blank' === $address_key ? '' : 'base' );
			update_option( 'poocommerce_shipping_cost_requires_address', $requires_address ? 'yes' : 'no' );
	...
	private function configure_shipping_topology( FixtureData $fixtures, bool $has_default_rate, bool $has_gb_rate, bool $has_pickup, array &$topology ): void {
		...
		if ( $has_default_rate ) {
			...
			update_option(
				'poocommerce_flat_rate_' . $topology['default_instance_id'] . '_settings',
				array(
					'enabled'    => 'yes',
		...
		if ( $has_gb_rate ) {
			...
			update_option(
				'poocommerce_flat_rate_' . $topology['gb_instance_id'] . '_settings',
				array(
					'enabled'    => 'yes',
					'title'      => 'UK delivery',
					'tax_status' => 'none',
	...
	public function test_add_item_preserves_cart_item_data_identity(): void {
		...
		add_filter( 'poocommerce_store_api_add_to_cart_data', $add_cart_item_data, 10, 2 );
		...
		try {
			...
			remove_filter( 'poocommerce_store_api_add_to_cart_data', $add_cart_item_data, 10 );
		}
	}
	...
	public function test_rejected_add_item_leaves_managed_stock_cart_unchanged(): void {
		...
		try {
			...
			$this->assertSame( 'poocommerce_rest_product_partially_out_of_stock', $second_data['code'], 'The rejection should report the Store API partial-stock code.' );
			$this->assertSame( $before, $this->get_cart_line_identity_map(), 'The rejected re-add should leave every cart line unchanged.' );
		} finally {
