...
class AddToCartWithOptions extends \WP_UnitTestCase {
	...
	public function test_stepper_not_visible_for_sold_individually_products_and_manage_stock() {
		...
		$this->assertStringNotContainsString( 'data-block-name="poocommerce/add-to-cart-with-options-quantity-selector"', $markup, 'The Add to Cart with Options form does not contain a quantity selector block for sold individually products.' );
		...
		$simple_product->save();
		$markup = do_blocks( '<!-- wp:poocommerce/single-product {"productId":' . $simple_product_id . '} --><!-- wp:poocommerce/add-to-cart-with-options /--><!-- /wp:poocommerce/single-product -->' );
		$this->assertStringNotContainsString( 'data-block-name="poocommerce/add-to-cart-with-options-quantity-selector"', $markup, 'The Add to Cart with Options form does not contain a quantity selector block for products with manage stock set to true and stock quantity set to 1.' );

		$simple_product->set_stock_quantity( 10 );
		$simple_product->save();
		$markup = do_blocks( '<!-- wp:poocommerce/single-product {"productId":' . $simple_product_id . '} --><!-- wp:poocommerce/add-to-cart-with-options /--><!-- /wp:poocommerce/single-product -->' );
		$this->assertStringContainsString( 'data-block-name="poocommerce/add-to-cart-with-options-quantity-selector"', $markup, 'The Add to Cart with Options form contains a quantity selector block for products with manage stock set to true and stock quantity > 1.' );
	}
}
