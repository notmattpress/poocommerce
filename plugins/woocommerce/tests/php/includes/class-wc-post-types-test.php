...
class WC_Post_Types_Test extends WC_Unit_Test_Case {
	...
	public function test_trashing_the_shop_page_does_not_queue_a_rewrite_flush(): void {
		$previous_shop_page_id = get_option( 'poocommerce_shop_page_id' );
		$shop_page_id          = $this->prepare_shop_page();
		...
		try {
			...
			// post types for core's own suite, so a theme that supports PooCommerce would hand
			// that name to `has_archive` and leave it there for the rest of the process.
			update_option( 'poocommerce_shop_page_id', $previous_shop_page_id );
		}
	}
