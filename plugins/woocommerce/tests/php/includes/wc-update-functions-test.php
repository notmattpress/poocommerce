...
class WC_Update_Functions_Test extends \WC_Unit_Test_Case {
	...
	public function test_wc_update_450_sanitize_coupons_code_invalidates_the_lookup_cache(): void {
		...
		update_option( 'poocommerce_update_450_last_coupon_id', $coupon_id - 1 );
		...
		$this->assertSame( 'yes', get_option( 'poocommerce_update_450_codes_changed' ), 'The rewrite should be recorded for the last batch.' );
		$this->assertSame(
			$prefix_before,
		...
		$this->assertFalse( get_option( 'poocommerce_update_450_codes_changed' ), 'The migration should clean up the flag it persisted.' );
	}
	...
	public function test_wc_update_1130_accepts_a_cursor_saved_by_a_concurrent_run( int $ahead ): void {
		...
		$option       = 'poocommerce_update_1130_last_repaired_order_id';
		$orders_table = OrdersTableDataStore::get_orders_table_name();
