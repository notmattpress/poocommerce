...
class WC_CLI_Update_Command_Test extends WC_Unit_Test_Case {
	...
	public function test_db_version_is_updated_if_have_callbacks() {
		...
		$this->assertTrue(
			version_compare( get_option( 'poocommerce_db_version' ), WC()->version, '>=' ),
			'After applying updates via WP CLI, the `poocommerce_db_version` option should match the last db update version in code.'
		);
	}
