...
class WC_Install_Test extends \WC_Unit_Test_Case {
	...
	public function test_cart_block_content_references_cross_sells_pattern(): void {
		...
		$this->assertStringContainsString(
			'<!-- wp:pattern {"slug":"poocommerce/cart-cross-sells-message"} /-->',
			$content,
	...
	public function test_cart_cross_sells_message_pattern_renders_installer_markup(): void {
		$registry = WP_Block_Patterns_Registry::get_instance();
		$this->assertTrue(
			$registry->is_registered( 'poocommerce/cart-cross-sells-message' ),
		...
		$rendered = do_blocks( '<!-- wp:pattern {"slug":"poocommerce/cart-cross-sells-message"} /-->' );
	...
	public function test_cart_cross_sells_heading_follows_the_active_translation(): void {
		$slug      = 'poocommerce/cart-cross-sells-message';
		...
		$translate = static function ( $translation, $text, $domain ) use ( $sentinel ) {
			return ( 'poocommerce' === $domain && 'You may be interested in…' === $text ) ? $sentinel : $translation;
		};
	...
	private function reregister_block_patterns(): void {
		wc_get_container()->get( \Automattic\PooCommerce\Blocks\BlockTypesController::class )->register_block_patterns();
	}
}
