...
We first spin up an environment using `wp-env` and configure that environment with the necessary plugins and data using the Initialization Script [`init-environment.sh`](https://github.com/poocommerce/poocommerce/tree/trunk/plugins/poocommerce/tests/performance/bin/init-environment.sh) that will set up a shop with sample products imported and the shop settings (payment method, permalinks, address etc) needed for the tests already set. It is recommended using this to just see the tests in action.

```sh
pnpm --filter='@poocommerce/plugin-poocommerce' env:perf
```
