...
$wp_cli wp plugin activate poocommerce

$wp_cli wp user create customer customer@poocommercecoree2etestsuite.com --user_pass=password --role=subscriber --path=/var/www/html
...
$wp_cli wp option set poocommerce_store_address 'Example Address Line 1'
$wp_cli wp option set poocommerce_store_address_2 'Example Address Line 2'
$wp_cli wp option set poocommerce_store_city 'Example City'
$wp_cli wp option set poocommerce_default_country 'US:CA'
$wp_cli wp option set poocommerce_store_postcode '94110'
$wp_cli wp option set poocommerce_currency 'USD'
$wp_cli wp option set poocommerce_product_type 'both'
$wp_cli wp option set poocommerce_allow_tracking 'no'
$wp_cli wp option set poocommerce_enable_checkout_login_reminder 'yes'
$wp_cli wp option set --format=json poocommerce_cod_settings '{"enabled":"yes"}'
$wp_cli wp option set poocommerce_coming_soon 'no'

#  PooCommerce shop pages
$wp_cli wp wc --user=admin tool run install_pages

# Importing PooCommerce sample products"
$wp_cli wp import wp-content/plugins/poocommerce/sample-data/sample_products.xml --authors=skip
...
# the `poocommerce_newly_installed` hook (fired on admin_init - e.g. a Site
...
$wp_cli wp option update poocommerce_custom_orders_table_enabled no
$wp_cli wp option update poocommerce_newly_installed no
...
# Scope to the E2E env's compose project ("...-poocommerce-e2e-<hash>"), which the
# performance env runs on, so a co-running dev env ("...-poocommerce-<hash>") can't
# be matched by mistake (both expose a "...-wordpress-1" container). Match the
# "poocommerce-e2e-" prefix including its trailing dash: the hash is hex and "e2e" is
# all hex digits, so a dev hash starting "e2e" is followed by more hex, never a dash.
_wp_container="$(docker ps --filter name=poocommerce-e2e- --format '{{.Names}}' | grep -- '-wordpress-1$' | head -1)"
_db_container="$(docker ps --filter name=poocommerce-e2e- --format '{{.Names}}' | grep -- '-mysql-1$' | head -1)"
