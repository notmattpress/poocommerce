...
class WC_Tests_Core_Functions extends WC_Unit_Test_Case {
	...
	/**
	 * @testdox Should return the requested PooCommerce currency symbol.
	 *
	 * @since 2.2
	 */
	public function test_get_poocommerce_currency_symbol() {
		...
		$this->assertEquals( 'MOP&#36;', get_poocommerce_currency_symbol( 'MOP' ) );
		$this->assertSame( 'CHW', get_poocommerce_currency_symbol( 'CHW' ) );
		$this->assertSame( 'K', get_poocommerce_currency_symbol( 'ZMW' ), 'The Zambian kwacha should use its local display symbol.' );
