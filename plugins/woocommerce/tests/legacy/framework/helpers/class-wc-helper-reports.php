...
class WC_Helper_Reports {
	...
	public static function reset_stats_dbs() {
		...
		$wpdb->query( 'DELETE FROM ' . \Automattic\PooCommerce\Admin\API\Reports\Taxes\DataStore::get_db_table_name() ); // @codingStandardsIgnoreLine.
		$wpdb->query( "DELETE FROM {$wpdb->wc_category_lookup}" ); // @codingStandardsIgnoreLine.

		$category_lookup = \Automattic\PooCommerce\Internal\Admin\CategoryLookup::instance();
