...
namespace Automattic\PooCommerce\Api\Infrastructure\DesignTime;
 ...
 * sibling PooCommerce plugins that ship their own code-API classes. Each
 ...
 * constructor defaults reproduce PooCommerce core's own configuration, so
...
class ApiBuilder {
	...
	/**
	 * @param ?string $api_dir                 Absolute path to the directory containing the code-API classes to scan. Null = PooCommerce core's `src/Api`.
	 * @param ?string $autogenerated_dir       Absolute path to the directory where generated code will be written. The directory is wiped on each build. Null = PooCommerce core's `src/Internal/Api/Autogenerated`.
	 * @param ?string $api_namespace           PSR-4 namespace that maps to $api_dir. Null = `Automattic\PooCommerce\Api`.
	 * @param ?string $autogenerated_namespace PSR-4 namespace that maps to $autogenerated_dir. Null = `Automattic\PooCommerce\Internal\Api\Autogenerated`.
	 ...
	 * @param ?string $phpcbf_path             Absolute path to a phpcbf executable used to format generated files. Null = use PooCommerce core's vendored phpcbf. To skip linting entirely, pass $skip_linter to build().
	 */
	public function __construct(
		...
		// subset to default back to PooCommerce core's own values would turn
		...
		$this->api_namespace           = $api_namespace ?? 'Automattic\\PooCommerce\\Api';
		$this->autogenerated_namespace = $autogenerated_namespace ?? 'Automattic\\PooCommerce\\Internal\\Api\\Autogenerated';
		$this->phpcbf_path             = $phpcbf_path ?? realpath( __DIR__ . '/../..' ) . '/vendor/bin/phpcbf';

		// Default composer working dir to PooCommerce core's plugin dir when no
	...
	/**
	 * Configure an ApiBuilder by convention for a sibling PooCommerce plugin.
	 *
	 ...
	 * `$plugin_root`; phpcbf defaults to PooCommerce core's vendored copy
	 ...
	 * @param string $namespace_prefix Top-level PSR-4 namespace the plugin publishes under (e.g. 'Automattic\\PooCommerceSimpleMath').
	 */
	 ...
	 *
	 * Keeps the plugin-side script tiny: after locating PooCommerce and
	 * requiring PooCommerce's own autoloader, the plugin only needs to call
	...
	public function build( bool $skip_linter = false ): void {
		...
		if ( null !== $this->composer_working_dir ) {
			...
			// autoloader's merged-manifest step that PooCommerce relies on
			...
			if ( false === chdir( $this->composer_working_dir ) ) {
				...
				// `Automattic\PooCommerce\Tests\` → `tests/php/src/`) stay
	 ...
	 *
	 * PooCommerce core ships such a resolver at
	 * `Automattic\PooCommerce\Api\Infrastructure\ClassResolver` delegating to
	 ...
	 * {@see \Automattic\PooCommerce\Api\Infrastructure\Principal} wrapping
	 * `wp_get_current_user()`.
	 *
	 ...
	 * PooCommerce core does not ship a default class.
	 */
	 ...
	 *
	 *   1. PooCommerce core's well-known authorization attributes
	 ...
	 *
	 * Plain attributes without an `authorize()` method (e.g. {@see \Automattic\PooCommerce\Api\Attributes\Description})
	 ...
	 * {@see \Automattic\PooCommerce\Api\Utils\SchemaHandle::get_all_metadata()}
	 ...
	 * stock {@see \Automattic\PooCommerce\Api\Attributes\HiddenFromMetadataQuery}
	...
	private function generate_resolver( string $fqcn, \ReflectionClass $ref, string $kind ): void {
		...
		// {@see \Automattic\PooCommerce\Api\InputTypes\TracksProvidedFields}
	...
	private function generate_connection_code( string $namespace, string $node_type_class, string $node_type_namespace, string $node_type_alias, string $connection_class_name, string $edge_class_name ): string {
		...
		$code .= "use Automattic\\PooCommerce\\Api\\Infrastructure\\Schema\\ObjectType;\n";
		$code .= "use Automattic\\PooCommerce\\Api\\Infrastructure\\Schema\\Type;\n\n";
		...
		$code .= "\t\t\t\t\t'description' => __( 'A connection to a list of {$node_type_class} items.', 'poocommerce' ),\n";
	 ...
	 * `__( ..., 'poocommerce' )`, so any default description supplied by a
