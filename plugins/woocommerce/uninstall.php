 ...
 * fired without PooCommerce. When PooCommerce is the only plugin bundling Action Scheduler, nothing
 ...
 *
 * That initialization fires `action_scheduler_init`, and WordPress does not deactivate PooCommerce
 * extensions before deleting PooCommerce, so a still-active extension's callback runs here without
 * PooCommerce loaded. Anything it throws is caught: leaving actions behind is the bug we are fixing,
...
if ( ! class_exists( 'ActionScheduler', false ) ) {
	...
	if ( is_readable( $wc_action_scheduler_bootstrap ) ) {
		try {
			...
			// phpcs:ignore WordPress.PHP.DevelopmentFunctions.error_log_error_log -- No PooCommerce logger during uninstall.
			error_log( 'PooCommerce: Action Scheduler threw while loading during uninstall: ' . $e->getMessage() );
		} finally {
			/*
...
if ( class_exists( ActionScheduler::class ) && ActionScheduler::is_initialized() && function_exists( 'as_unschedule_all_actions' ) ) {
	...
	// PooCommerce::register_recurring_actions() schedules these four under wrapper hooks.
	as_unschedule_all_actions( 'poocommerce_tracker_send_event_wrapper' );
	as_unschedule_all_actions( 'poocommerce_cleanup_rate_limits_wrapper' );
...
if ( defined( 'WC_REMOVE_ALL_DATA' ) && true === WC_REMOVE_ALL_DATA ) {
	 ...
	 * PooCommerce's own known wc_ / _wc_ user meta keys (the wc_admin_ legacy prefix; the _wc_egg_
	...
	/*
	 * Remove direct POS capabilities (poocommerce_pos_*) granted per user via WP_User::add_cap().
	 *
	 * Unlike the poocommerce_pos_preset meta removed above, these caps live inside the serialized
	 * {prefix}capabilities meta row rather than a poocommerce_ meta key, so the sweep above can't reach
	 * them. Left behind, a reinstall would silently restore POS access because has_pos_access() keys off
	 * these caps. The row also stores the user's role, so strip only the poocommerce_pos_ caps per user
	 * via remove_cap() rather than deleting the row.
	 *
	 * Users are matched by the poocommerce_pos_ cap prefix — the same {prefix}capabilities LIKE that
	...
	foreach ( $pos_staff_ids as $pos_staff_id ) {
		...
		foreach ( array_keys( $pos_staff_user->caps ) as $pos_capability ) {
			if ( 0 === strpos( (string) $pos_capability, 'poocommerce_pos_' ) ) {
