...
<div class="poocommerce-order">
	...
	if ( $order ) :
		...
		<?php if ( $order->has_status( 'failed' ) ) : ?>
			...
			<?php
			$default_order_failed_text = esc_html__( 'Unfortunately your order cannot be processed as the originating bank/merchant has declined your transaction. Please attempt your purchase again.', 'poocommerce' );
			 ...
			 * @since 11.2.0
			 */
			$order_failed_text = apply_filters( 'poocommerce_thankyou_order_failed_text', $default_order_failed_text, $order );
			$order_failed_text = is_string( $order_failed_text ) ? $order_failed_text : $default_order_failed_text;
			?>

			<p class="poocommerce-notice poocommerce-notice--error poocommerce-thankyou-order-failed"><?php echo wp_kses_post( $order_failed_text ); ?></p>
