	...
	<div id="comments">
		<h2 class="poocommerce-Reviews-title">
			...
			if ( $count && wc_review_ratings_enabled() ) {
				...
				$reviews_title = sprintf( esc_html( _n( '%1$s review for %2$s', '%1$s reviews for %2$s', $count, 'poocommerce' ) ), esc_html( $count ), '<span>' . wp_kses_post( get_the_title() ) . '</span>' );
				/**
				 ...
				 */
				echo apply_filters( 'poocommerce_reviews_title', $reviews_title, $count, $product ); // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped -- The base heading is sanitised above and the filter returns extension-controlled markup by design.
			} else {
