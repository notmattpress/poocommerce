...
<?php endif; ?>
		...
		<form class="poocommerce-form poocommerce-form-login login" method="post" novalidate>
			...
			<p class="poocommerce-form-row poocommerce-form-row--wide form-row form-row-wide">
				...
				<input type="text" class="poocommerce-Input poocommerce-Input--text input-text" name="username" id="username" autocomplete="username" value="<?php echo ( ! empty( $_POST['username'] ) && is_string( $_POST['username'] ) ) ? esc_attr( wp_unslash( $_POST['username'] ) ) : ''; ?>" required aria-required="true" /><?php // @codingStandardsIgnoreLine ?>
			</p>
			<p class="poocommerce-form-row poocommerce-form-row--wide form-row form-row-wide">
