 ...
 */
if ( (bool) apply_filters( 'poocommerce_email_display_order_number', true, $order, $email ) ) {
	if ( $email_improvements_enabled ) {
	...
	foreach ( $item_totals as $total ) {
		if ( $email_improvements_enabled ) {
			if ( 'shipping' === ( $total['type'] ?? '' ) && isset( $total['meta'] ) && $total['value'] === $total['meta'] ) {
				 ...
				 */
				if ( (bool) apply_filters( 'poocommerce_email_order_shipping_show_free_label', $order->has_shipping_method( 'free_shipping' ), $order ) ) {
					$total['value'] = __( 'Free!', 'poocommerce' );
				}
			}
			$label = $total['label'];
			if ( isset( $total['meta'] ) ) {
