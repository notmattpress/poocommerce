...
foreach ( $items as $item_id => $item ) :
	...
	<tr class="<?php echo esc_attr( $order_item_class ); ?>">
			...
			<table class="order-item-data">
				<tr>
					...
					if ( $show_image ) {
						 ...
						 * @since 2.1.0
						 */
						echo '<td class="email-order-item-thumbnail" style="width: ' . esc_attr( $thumbnail_width ) . 'px;">' . wp_kses_post( apply_filters( 'poocommerce_order_item_thumbnail', $image, $item->item ) ) . '</td>';
					}
					?>
					<td>
						<?php
		...
		<td class="td font-family text-align-<?php echo esc_attr( $price_text_align ); ?>" style="vertical-align:middle;">
			 ...
			 * @since 2.4.0
			 */
			$quantity = apply_filters( 'poocommerce_email_order_item_quantity', $qty_display, $item->item );
			if ( '' !== $quantity ) {
