 ...
 *
 * This template can be overridden by copying it to yourtheme/poocommerce/emails/customer-verify-email.php.
 *
...
<p><?php printf( esc_html__( 'Hi %s,', 'poocommerce' ), esc_html( $user_display_name ) ); ?></p>
<?php /* translators: %s: the customer's email address. */ ?>
<p><?php printf( esc_html__( "Once you've confirmed that %s is your email address, we'll link any past orders to your account.", 'poocommerce' ), '<b>' . esc_html( $user_email ) . '</b>' ); ?></p>
<?php
wc_get_template(
...
?>
<p><?php esc_html_e( "If you didn't request this email, there's nothing to worry about, and you can safely ignore it.", 'poocommerce' ); ?></p>
