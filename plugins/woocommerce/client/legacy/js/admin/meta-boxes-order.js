...
jQuery( function ( $ ) {
	...
	var wc_meta_boxes_order_items = {
				...
				$.ajax( {
					...
					success: function( response ) {
						if ( response.success ) {
							...
							// Update notes.
							const notesEl = document.querySelector( '#poocommerce-order-notes ul.order_notes' );
		...
		validate_quantity_inputs: function() {
			var input = wc_meta_boxes_order_items.find_input_with_qty_below_min(
				document.querySelectorAll( '#poocommerce-order-items input.quantity' )
			);
		...
		backbone: {
			...
			search_tax_rates: function( modal, page, is_search ) {
				...
				$.ajax( {
					...
					type:     'GET',
					data:     {
						action:   'poocommerce_json_search_tax_rates',
				...
				return $( '<tr class="no-items"></tr>' ).append(
					$( '<td class="colspanchange wc-tax-rate-empty" colspan="5"></td>' ).append(
						...
						$( '<p class="wc-tax-rate-empty-action"></p>' ).append(
							$( '<a></a>', {
								class: 'poocommerce-BlankState-cta button',
	...
	var wc_meta_boxes_order_notes = {
		...
		update_note_type_ui: function() {
			...
			if ( label ) {
				$( '#poocommerce-order-notes button.add_note' ).text( label );
			}
			$( '#poocommerce-order-notes .add_note_email_settings' ).prop( 'hidden', ! isCustomer );
		},
			...
			$.post( poocommerce_admin_meta_boxes.ajax_url, data, function( response ) {
				...
				if ( window.wp && window.wp.a11y && 'function' === typeof window.wp.a11y.speak ) {
					var message = 'customer' === data.note_type
						? poocommerce_admin_meta_boxes.i18n_customer_order_note_added
						: poocommerce_admin_meta_boxes.i18n_order_note_added;
					if ( message ) {
						window.wp.a11y.speak( message, 'polite' );
					}
				}
		...
		delete_order_note: function() {
			...
			var message = note.hasClass( 'customer-note' )
				? poocommerce_admin_meta_boxes.i18n_delete_customer_note
				: poocommerce_admin_meta_boxes.i18n_delete_note;
			...
			if ( window.confirm( message ) ) {
				...
				$.post( poocommerce_admin_meta_boxes.ajax_url, data, function() {
					...
					if ( 0 === $list.find( 'li.note' ).length ) {
						$list.append( $( '<li class="no-items"></li>' ).text( poocommerce_admin_meta_boxes.i18n_no_notes_yet ) );
					}
				});
			}
