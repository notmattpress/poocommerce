...
jQuery( function ( $ ) {
	...
	$( 'select#product-type' )
		.on( 'change', function () {
			...
			const cogs_field_tip = $( '._cogs_value_field' ).find( '.poocommerce-help-tip' );
	...
	function add_placeholder_to_attribute_values_field( $attributeListItem ) {
		...
		if ( $used_for_variations_checkbox.length && $used_for_variations_checkbox.is( ':checked' ) ) {
			$attributeListItem.find( 'textarea' )
				.attr( 'placeholder', poocommerce_admin_meta_boxes.i18n_attributes_used_for_variations_placeholder );
		} else {
			$attributeListItem.find( 'textarea' )
				.attr( 'placeholder', poocommerce_admin_meta_boxes.i18n_attributes_default_placeholder );
		}
	}
	...
	const tooltipMarkup =
		`<span class="poocommerce-help-tip" tabindex="0" aria-label="${ poocommerce_admin_meta_boxes.i18n_product_image_tip }"></span>`;
	const tooltipData = {
		attribute: 'data-tip',
