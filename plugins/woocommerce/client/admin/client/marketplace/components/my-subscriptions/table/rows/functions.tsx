 ...
 */
import type { TableRow } from '@poocommerce/components/build-types/table/types';
...
function getStatusBadge(
	...
	if (
		...
		return {
			text: __( 'Auto-renew: off', 'poocommerce' ),
			level: StatusLevel.Warning,
			explanation: createInterpolateElement(
				__(
...
export function expiry( subscription: Subscription ): TableRow {
	...
	if ( subscription.is_agency || subscription.included_in_host_plan ) {
		...
		const text = isAgency
			? __( 'Managed by agency', 'poocommerce' )
			: __( 'Managed by host', 'poocommerce' );
		return {
			display: (
				<StatusPopover
					text={ text }
	...
	let expiryDateElement = __< string >( 'Never expires', 'poocommerce' );
...
export function autoUpdates( subscription: Subscription ): TableRow {
	return {
		display: (
			<div className="poocommerce-marketplace__my-subscriptions__auto-updates">
