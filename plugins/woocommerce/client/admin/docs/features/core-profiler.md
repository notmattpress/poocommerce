...
Some extensions offered on the Extensions page require a Jetpack (WordPress.com) connection to work. When a merchant selects one of these extensions, the Core Profiler routes them to a WordPress.com connection screen after installation, before returning them to the PooCommerce Admin homescreen.

### The `requires_jpc` field

Whether the connection step is shown is controlled by the `requires_jpc` field on each extension (JPC stands for "Jetpack Connection"). It was introduced in [PR #52887](https://github.com/poocommerce/poocommerce/pull/52887).

The field is defined on the `Extension` type in [`packages/js/data/src/onboarding/types.ts`](https://github.com/poocommerce/poocommerce/blob/trunk/packages/js/data/src/onboarding/types.ts):
...
For the extensions bundled with PooCommerce core, the field is set in `with_core_profiler_fields()` in [`src/Internal/Admin/RemoteFreeExtensions/DefaultFreeExtensions.php`](https://github.com/poocommerce/poocommerce/blob/trunk/plugins/poocommerce/src/Internal/Admin/RemoteFreeExtensions/DefaultFreeExtensions.php). As of writing, `jetpack` and `poocommerce-payments` (WooPayments) are flagged with `'requires_jpc' => true`.
...
-   `resolveSelect( onboardingStore ).getJetpackAuthUrl( { redirectUrl, from: 'poocommerce-core-profiler' } )`

The URL is built server-side in `JetpackConnection::get_authorization_url()` ([`src/Internal/Jetpack/JetpackConnection.php`](https://github.com/poocommerce/poocommerce/blob/trunk/plugins/poocommerce/src/Internal/Jetpack/JetpackConnection.php)), which registers the site with WordPress.com if needed and appends `from` and `calypso_env` query args.
...
The `from=poocommerce-core-profiler` parameter is what the WordPress.com side (Calypso, `client/jetpack-connect/authorize.js`) keys off to render the PooCommerce-branded connection screen instead of the default Jetpack-branded one. On approval, the merchant is redirected back to the PooCommerce Admin homescreen.
...
This notice is rendered by the `PluginsTermsOfService` component in [`client/core-profiler/pages/Plugins/components/plugin-terms-of-service/PluginsTermsOfService.tsx`](https://github.com/poocommerce/poocommerce/blob/trunk/plugins/poocommerce/client/admin/client/core-profiler/pages/Plugins/components/plugin-terms-of-service/PluginsTermsOfService.tsx).
