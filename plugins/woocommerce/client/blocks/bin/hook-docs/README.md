...
`pnpm --filter=@poocommerce/block-library build:docs` regenerates the hook documentation:
...
2. `wp-hooks-generator` scans the PHP sources in `plugins/poocommerce/src` and writes `data/actions.json` and `data/filters.json` — a gitignored intermediate that the next step consumes in the same run. The scan is limited to the PooCommerce Blocks PHP (`src/Blocks` and `src/StoreApi`) by the `extra.wp-hooks.ignore-files` list in `client/blocks/composer.json` — if a new top-level directory is added to `plugins/poocommerce/src` that should not appear in these docs, add it to that list.
