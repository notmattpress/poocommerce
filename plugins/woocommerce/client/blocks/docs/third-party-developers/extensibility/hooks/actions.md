...
- [poocommerce_after_add_to_cart_button](#poocommerce_after_add_to_cart_button)
- [poocommerce_after_add_to_cart_form](#poocommerce_after_add_to_cart_form)
- [poocommerce_after_add_to_cart_quantity](#poocommerce_after_add_to_cart_quantity)
- [poocommerce_after_main_content](#poocommerce_after_main_content)
- [poocommerce_after_shop_loop](#poocommerce_after_shop_loop)
- [poocommerce_after_single_variation](#poocommerce_after_single_variation)
- [poocommerce_after_variations_form](#poocommerce_after_variations_form)
- [poocommerce_after_variations_table](#poocommerce_after_variations_table)
- [poocommerce_applied_coupon](#poocommerce_applied_coupon)
- [poocommerce_archive_description](#poocommerce_archive_description)
- [poocommerce_before_add_to_cart_button](#poocommerce_before_add_to_cart_button)
- [poocommerce_before_add_to_cart_form](#poocommerce_before_add_to_cart_form)
- [poocommerce_before_add_to_cart_quantity](#poocommerce_before_add_to_cart_quantity)
- [poocommerce_before_main_content](#poocommerce_before_main_content)
- [poocommerce_before_shop_loop](#poocommerce_before_shop_loop)
- [poocommerce_before_single_variation](#poocommerce_before_single_variation)
- [poocommerce_before_variations_form](#poocommerce_before_variations_form)
...
- [`poocommerce_blocks_validate_location_{$location}_fields`](#poocommerce_blocks_validate_location_location_fields)
...
- [poocommerce_checkout_validate_order_before_payment](#poocommerce_checkout_validate_order_before_payment)
- [poocommerce_no_products_found](#poocommerce_no_products_found)
- [poocommerce_rest_checkout_process_payment_with_context](#poocommerce_rest_checkout_process_payment_with_context)
- [poocommerce_set_additional_field_value](#poocommerce_set_additional_field_value)
- [poocommerce_shop_loop](#poocommerce_shop_loop)
- [poocommerce_single_variation](#poocommerce_single_variation)
...
- [poocommerce_store_api_checkout_order_created](#poocommerce_store_api_checkout_order_created)
...
- [poocommerce_store_api_checkout_update_draft](#poocommerce_store_api_checkout_update_draft)
- [poocommerce_store_api_checkout_update_order_from_request](#poocommerce_store_api_checkout_update_order_from_request)
...
- [poocommerce_validate_additional_field](#poocommerce_validate_additional_field)
- [`poocommerce_{$product->get_type()}_add_to_cart`](#poocommerce_product-get_type_add_to_cart)
- [`poocommerce_{$product_type}_add_to_cart`](#poocommerce_product_type_add_to_cart)
...
```php
do_action( 'poocommerce_add_to_cart', string $cart_id, int $product_id, int $request_quantity, int $variation_id, array $variation, array $cart_item_data )
```
...
## poocommerce_after_single_variation
...
## poocommerce_after_variations_table
...
## poocommerce_before_single_variation
...
To ensure blocks are initialized, you must use the `poocommerce_blocks_loaded` hook instead of the `plugins_loaded` hook. This is because the functions hooked into plugins_loaded on the same priority load in an inconsistent and unpredictable manner.
...
## `poocommerce_blocks_validate_location_{$location}_fields`


Pass an error object to allow validation of an additional field.

```php
do_action( 'poocommerce_blocks_validate_location_{$location}_fields', \WP_Error $errors, mixed $fields, string $group )
```
...
```php
do_action( 'poocommerce_blocks_{$this->registry_identifier}_registration', \IntegrationRegistry $registry )
```
...
This action will be deprecated in the Store API because it encourages wc_add_notice: the API has to capture those notices and convert them to WP_Error objects. Prefer `poocommerce_store_api_cart_errors`, which passes a WP_Error to callbacks directly. Core keeps firing this action from the classic cart and checkout, so it is not deprecated there.
...
```php
do_action( 'poocommerce_checkout_validate_order_before_payment', \WC_Order $order, \WP_Error $validation_errors )
```
...
```php
do_action_ref_array( 'poocommerce_rest_checkout_process_payment_with_context', [ \PaymentContext $context, \PaymentResult $payment_result ] )
```
...
```php
do_action( 'poocommerce_set_additional_field_value', string $key, mixed $value, string $group, \WC_Customer|\WC_Order $wc_object )
```
...
```php
do_action( 'poocommerce_store_api_checkout_order_created', \WC_Order $order )
```

### Description

Use this hook for first-touch logic that should only run when the draft order is initially created (e.g. analytics, abandoned-cart trackers). As of PooCommerce 10.8.0 the Store API defers draft order creation to place-order time, so this action fires once at POST rather than on the first PATCH.
...
The action runs before payment is processed, so callbacks can still act on the order on its way to the gateway. Do not use this action for payment-completion logic or to call WC_Order::payment_complete(). Use poocommerce_payment_complete or poocommerce_order_status_completed instead.
...
add_action( 'poocommerce_store_api_checkout_order_processed', 'my_function_callback', 10 );
```


### See

- <https://github.com/poocommerce/poocommerce-gutenberg-products-block/pull/3238>
...
```php
do_action( 'poocommerce_store_api_checkout_update_draft', \WP_REST_Request $request )
```
...
No `WC_Order` exists at this point under deferred draft order creation. Read checkout state from `WC()->cart`, `WC()->customer`, and the supplied `$request`, and persist any extension-owned state to `WC()->session`. To apply that state to the real order at place-order time, hook `poocommerce_store_api_checkout_update_order_meta` or `poocommerce_store_api_checkout_update_order_from_request` — both fire against the real, persisted order at POST exactly as they always have.
...
```php
do_action( 'poocommerce_store_api_checkout_update_order_from_request', \WC_Order $order, \WP_REST_Request $request )
```
...
This is similar to existing core hook poocommerce_checkout_update_order_meta. We're using a new action:
...
- <https://github.com/poocommerce/poocommerce-gutenberg-products-block/pull/3686>
...
```php
do_action( 'poocommerce_validate_additional_field', \WP_Error $errors, string $field_key, mixed $field_value )
```
...
## `poocommerce_{$product_type}_add_to_cart`


Trigger the single product add to cart action that prints the markup.

```php
do_action( 'poocommerce_{$product_type}_add_to_cart' )
```
