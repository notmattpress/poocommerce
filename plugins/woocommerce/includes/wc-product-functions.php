 ...
 *
 * Args and usage: https://developer.poocommerce.com/docs/extensions/core-concepts/wc-get-products/
 *
 * @since  3.0.0
...
function wc_placeholder_img( $size = 'poocommerce_thumbnail', $attr = '' ) {
	...
	if ( $use_attachment_image && has_filter( 'poocommerce_placeholder_img_src' ) ) {
	 ...
	 */
	return apply_filters( 'poocommerce_placeholder_img', $image_html, $size, $dimensions, $attr );
}
...
function wc_schedule_product_sale_events( WC_Product $product ): void {
	...
	$schedule = function ( ?WC_DateTime $date, string $hook ) use ( $product_id ): void {
		...
		if ( empty( $identical_pending ) ) {
			as_schedule_single_action( $timestamp, $hook, $args, 'poocommerce-sales' );
		}
	};
...
function wc_maybe_schedule_product_sale_events( $product_id, $product = null ): void {
	...
	as_unschedule_all_actions( 'wc_product_start_scheduled_sale', array( 'product_id' => $product_id ), 'poocommerce-sales' );
	as_unschedule_all_actions( 'wc_product_end_scheduled_sale', array( 'product_id' => $product_id ), 'poocommerce-sales' );
 ...
 * of how the meta is written: PooCommerce CRUD, direct update_post_meta() calls from
 ...
 * `poocommerce_scheduled_sales` cron event or the before/after hooks fired within.
 *
...
function wc_get_product_visibility_options() {
	return apply_filters(
		...
		array(
			CatalogVisibility::VISIBLE => __( 'Shop and search results', 'poocommerce' ),
			CatalogVisibility::CATALOG => __( 'Shop only', 'poocommerce' ),
			CatalogVisibility::SEARCH  => __( 'Search results only', 'poocommerce' ),
			CatalogVisibility::HIDDEN  => __( 'Hidden', 'poocommerce' ),
		)
	);
}
...
function wc_get_product_stock_status_options() {
	return apply_filters(
		...
		array(
			ProductStockStatus::IN_STOCK     => __( 'In stock', 'poocommerce' ),
			ProductStockStatus::OUT_OF_STOCK => __( 'Out of stock', 'poocommerce' ),
			ProductStockStatus::ON_BACKORDER => __( 'On backorder', 'poocommerce' ),
		)
	);
}
...
function wc_get_product_category_list( $product_id, $sep = ', ', $before = '', $after = '', $orderby = '' ) {
		...
		if ( '' !== $orderby ) {
			wc_doing_it_wrong(
				...
				sprintf(
					...
					__( 'Unsupported $orderby value "%1$s". Use "name", "breadcrumb", or an empty string for WordPress term-list order.', 'poocommerce' ),
	...
	$term_links = apply_filters( 'term_links-product_cat', $links ); // phpcs:ignore WordPress.NamingConventions.ValidHookName.UseUnderscores, PooCommerce.Commenting.CommentHooks.MissingSinceComment
