...
class WC_Query {
	...
	public function pre_get_posts( $q ) {
		...
		if ( wc_current_theme_supports_poocommerce_or_fse() && $q->is_page() && 'page' === get_option( 'show_on_front' ) && absint( $q->get( 'page_id' ) ) === $shop_page_id ) {
			...
			if ( $q->is_search() ) {
				...
				// and the poocommerce_product_query_tax_query hook, which are scoped to product archives.
	...
	private function has_positive_search_terms(): bool {
		...
		$exclusion_prefix = (string) apply_filters( 'wp_query_search_exclusion_prefix', '-' ); // phpcs:ignore PooCommerce.Commenting.CommentHooks.MissingSinceComment
	...
	public function price_filter_post_clauses( $args, $wp_query ) {
		 ...
		 */
		if ( wc_tax_enabled() && TaxDisplayMode::INCLUSIVE === get_option( 'poocommerce_tax_display_shop' ) && ! wc_prices_include_tax() ) {
	...
	public static function get_main_meta_query() {
		// PHPStan infers $product_query as non-nullable from other call sites. See https://github.com/poocommerce/poocommerce/pull/64360#issuecomment-4360066970.
