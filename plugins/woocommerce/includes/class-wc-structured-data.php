		...
		if ( '' !== $product->get_price() ) {
			...
			if ( $product->is_type( ProductType::VARIABLE ) ) {
				...
				if ( $lowest === $highest ) {
					...
					if ( wc_tax_enabled() ) {
						$unit_price_spec['valueAddedTaxIncluded'] = 'incl' === get_option( 'poocommerce_tax_display_shop' );
					}
					$markup_offer = array(
						'@type'              => 'Offer',
					...
					if ( $product->is_on_sale() ) {
						...
						if ( wc_tax_enabled() ) {
							$sale_unit_price_spec['valueAddedTaxIncluded'] = 'incl' === get_option( 'poocommerce_tax_display_shop' );
						}
