...
abstract class WC_Product_Importer implements WC_Importer_Interface {
	...
	protected function match_product_id_by_global_unique_id( $global_unique_id, $type ) {
		...
		if ( WC_Product_Factory::get_product_type( $product_id ) !== $type ) {
			return new WP_Error(
				'poocommerce_product_importer_global_unique_id_type_mismatch',
				esc_html__( 'The Global Unique ID matches a product of a different type.', 'poocommerce' ),
				array(
					'id'               => $product_id,
			...
			foreach ( $data['raw_attributes'] as $position => $attribute ) {
				...
				if ( $attribute_id ) {
					...
					if ( ! empty( $options ) ) {
						 ...
						 */
						$attributes[] = apply_filters( 'poocommerce_product_importer_read_attribute', $attribute_object, $attribute, $product );
					}
					...
					// phpcs:ignore PooCommerce.Commenting.CommentHooks.MissingHookComment -- see same filter in the 'if' block.
					$attributes[] = apply_filters( 'poocommerce_product_importer_read_attribute', $attribute_object, $attribute, $product );
				}
			}
