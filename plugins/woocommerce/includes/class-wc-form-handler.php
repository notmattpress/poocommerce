...
class WC_Form_Handler {
	...
	public static function resend_set_password(): void {
		...
		if ( $last_sent_at > 0 && ( time() - $last_sent_at ) < self::SET_PASSWORD_RESEND_RATE_LIMIT_SECONDS ) {
			wc_add_notice( __( 'Please wait a moment before requesting another link to change your password.', 'poocommerce' ), 'notice' );
		...
		if ( is_wp_error( $key ) ) {
			wc_add_notice( __( 'Sorry, we were unable to resend the link. Please try again.', 'poocommerce' ), 'error' );
		} else {
			...
			WC()->mailer();
			// phpcs:ignore PooCommerce.Commenting.CommentHooks -- Re-fires poocommerce_reset_password_notification, documented in WC_Shortcode_My_Account::retrieve_password().
			do_action( 'poocommerce_reset_password_notification', $user->user_login, $key );
			wc_add_notice( __( 'We have emailed you a new link to change your password.', 'poocommerce' ) );
		}
	...
	public static function save_address() {
		 ...
		 */
		do_action( 'poocommerce_customer_save_address', $user_id, $address_type, $address, $customer );
	...
	public static function save_account_details() {
		...
		if ( is_email( $account_display_name ) && $account_display_name !== $current_display_name ) {
			wc_add_notice( __( 'Display name cannot be changed to email address due to privacy concern.', 'poocommerce' ), 'error', array( 'id' => 'account_display_name' ) );
		}
			...
			if ( ! WC()->cart->is_empty() && is_array( $cart_totals ) ) {
				foreach ( WC()->cart->get_cart() as $cart_item_key => $values ) {
					...
					if ( $passed_validation ) {
						 ...
						 * @since 10.6.0
						 */
						do_action( 'internal_poocommerce_cart_item_updated_from_user_request', $cart_item_key, $quantity, $old_quantity, WC()->cart );
					}
				}
			}
	...
	public static function add_to_cart_action( $url = false ) {
		...
		if ( $was_added_to_cart && 0 === wc_notice_count( 'error' ) ) {
			 ...
			 * @since 10.6.0
			 */
			do_action( 'internal_poocommerce_cart_item_added_from_user_request', $product_id, $quantity );
	...
	private static function add_to_cart_handler_variable( $product_id ) {
		...
		if ( empty( $variation_id ) && $product && $product->is_type( ProductType::VARIABLE ) ) {
			...
			if ( $is_in_product_page ) {
				...
				$error_message = sprintf( __( 'Please choose product options for %1$s.', 'poocommerce' ), esc_html( $product->get_name() ) );
			} else {
				/* translators: 1: product link, 2: product name */
				$error_message = sprintf( __( 'Please choose product options by visiting <a href="%1$s" title="%2$s">%2$s</a>.', 'poocommerce' ), esc_url( get_permalink( $product_id ) ), esc_html( $product->get_name() ) );
			}
