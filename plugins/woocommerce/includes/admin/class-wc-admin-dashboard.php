...
if ( ! class_exists( 'WC_Admin_Dashboard', false ) ) :
	...
	class WC_Admin_Dashboard {
		...
		public function init() {
			wp_add_dashboard_widget( 'poocommerce_dashboard_status', __( 'PooCommerce Status', 'poocommerce' ), array( $this, 'status_widget' ), null, null, 'normal', 'high' );

			// Reviews Widget.
			if ( 'yes' === get_option( 'poocommerce_enable_reviews', 'yes' ) && current_user_can( 'publish_shop_orders' ) && post_type_supports( 'product', 'comments' ) ) {
				wp_add_dashboard_widget( 'poocommerce_dashboard_recent_reviews', __( 'PooCommerce Recent Reviews', 'poocommerce' ), array( $this, 'recent_reviews' ), null, null, 'normal', 'high' );
			}
		...
		private function should_display_widget() {
			...
			return current_user_can( 'view_poocommerce_reports' ) || current_user_can( 'manage_poocommerce' ) || current_user_can( 'publish_shop_orders' );
		}
		...
		public function status_widget() {
			...
			echo '<p><span class="spinner is-active"></span><span class="wc-dashboard-widget-loading__text">' . esc_html__( 'Loading status data...', 'poocommerce' ) . '</span></p>';
			echo '</div>';
		...
		public function status_widget_content() {
			//phpcs:ignore
			$is_wc_admin_disabled = apply_filters( 'poocommerce_admin_disabled', false ) || ! FeaturesUtil::feature_is_enabled( 'analytics' );
				...
				<a href="<?php echo esc_url( admin_url( $net_sales_link ) ); ?>">
					...
					<?php
						printf(
							...
							esc_html__( 'Net sales this month %s', 'poocommerce' ),
				...
				<a href="<?php echo esc_url( admin_url( $top_seller_link . $top_seller->product_id ) ); ?>">
					...
					<?php
						printf(
							...
							esc_html( _n( 'Top seller this month %1$s (%2$d sale)', 'Top seller this month %1$s (%2$d sales)', $top_seller->qty, 'poocommerce' ) ),
				...
				<?php
					echo wp_kses_post(
						sprintf(
							...
							_n( 'Awaiting processing <strong>%s order</strong>', 'Awaiting processing <strong>%s orders</strong>', $processing_count, 'poocommerce' ),
							$processing_count
						)
					);
				?>
			...
			<li class="on-hold-orders">
				...
				<?php
					echo wp_kses_post(
						sprintf(
							/* translators: %s: order count */
							_n( 'On-hold <strong>%s order</strong>', 'On-hold <strong>%s orders</strong>', $on_hold_count, 'poocommerce' ),
							$on_hold_count
						)
					);
				?>
		...
		private function status_widget_stock_rows( $lowstock_link, $outofstock_link ) {
			...
			if ( false === $outofstock_count ) {
				...
				if ( is_null( $outofstock_count ) ) {
					...
					// poocommerce_notify_no_stock_amount option does not reclassify existing products'
			...
			<li class="low-in-stock">
				...
				<?php
					echo wp_kses_post(
						sprintf(
							/* translators: %s: order count */
							_n( 'Low in stock <strong>%s product</strong>', 'Low in stock <strong>%s products</strong>', $lowinstock_count, 'poocommerce' ),
							$lowinstock_count
						)
					);
				?>
			...
			<li class="out-of-stock">
				...
				<?php
					echo wp_kses_post(
						sprintf(
							/* translators: %s: order count */
							_n( 'Out of stock <strong>%s product</strong>', 'Out of stock <strong>%s products</strong>', $outofstock_count, 'poocommerce' ),
							$outofstock_count
						)
					);
				?>
		...
		public function recent_reviews() {
			...
			echo '<p><span class="spinner is-active"></span><span class="wc-dashboard-widget-loading__text">' . esc_html__( 'Loading reviews data...', 'poocommerce' ) . '</span></p>';
			echo '</div>';
