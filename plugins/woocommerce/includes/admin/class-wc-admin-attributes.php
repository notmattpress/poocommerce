...
class WC_Admin_Attributes {
	...
	private static function slug_field_description(): string {
		return sprintf(
			...
			__( 'Unique slug/reference for the attribute. Limited to %1$d bytes — roughly %2$d characters in your language; non-ASCII characters use 2–4 bytes each.', 'poocommerce' ),
	...
	private static function slug_byte_counter_script(): void {
		...
		$template = wp_json_encode( __( '%1$d / %2$d bytes', 'poocommerce' ), JSON_HEX_TAG | JSON_HEX_AMP );
		if ( false === $template ) {
		...
		$js_description = wp_json_encode(
			...
			sprintf( __( 'Unique slug/reference for the attribute. Non-ASCII characters use 2–4 bytes of the %d-byte limit.', 'poocommerce' ), $max_bytes ),
							...
							<tbody>
								...
								if ( $attribute_taxonomies ) {
									...
									foreach ( $attribute_taxonomies as $tax ) :
										...
										$actions  = array(
											'edit'   => '<a href="' . esc_url( add_query_arg( 'edit', $tax->attribute_id, 'edit.php?post_type=product&amp;page=product_attributes' ) ) . '">' . esc_html__( 'Edit', 'poocommerce' ) . '</a>',
											'delete' => '<a class="delete" href="' . esc_url( wp_nonce_url( add_query_arg( 'delete', $tax->attribute_id, 'edit.php?post_type=product&amp;page=product_attributes' ), 'poocommerce-delete-attribute_' . $tax->attribute_id ) ) . '">' . esc_html__( 'Delete', 'poocommerce' ) . '</a>',
										);
										 ...
										 * @since 11.1.0
										 */
										$actions     = (array) apply_filters( 'poocommerce_attribute_taxonomy_row_actions', $actions, $tax, $taxonomy );
										$row_actions = array();
										...
										<tr>
												...
												<td class="attribute-terms">
													...
													?>
													<br /><a href="edit-tags.php?taxonomy=<?php echo esc_attr( $taxonomy ); ?>&amp;post_type=product" class="configure-terms"><?php esc_html_e( 'Configure terms', 'poocommerce' ); ?></a>
												</td>
											</tr>
											<?php
										endforeach;
									...
									?>
										<tr>
											<td colspan="<?php echo esc_attr( $column_count ); ?>"><?php esc_html_e( 'No attributes currently exist.', 'poocommerce' ); ?></td>
										</tr>
										<?php
								}//end if
								?>
