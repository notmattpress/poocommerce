	...
	class WC_Admin_Settings {
				...
				add_action(
					...
					function () {
						if ( function_exists( 'get_current_screen' ) ) {
							...
							if ( 'poocommerce_page_wc-settings' === $screen->id ) {
		...
		public static function save() {
			...
			if ( ! current_user_can( 'manage_poocommerce' ) ) {
				wp_die( esc_html__( 'You do not have permission to save settings.', 'poocommerce' ), 403 );
			}
				...
				switch ( $value['type'] ) {
					...
					case 'single_select_page':
						...
						<tr class="single_select_page <?php echo esc_attr( $value['row_class'] ); ?>">
							...
							<td class="forminp">
								...
								echo str_replace( ' id=', " data-placeholder='" . esc_attr__( 'Select a page&hellip;', 'poocommerce' ) . "' style='" . esc_attr( $value['css'] ) . "' class='" . esc_attr( $value['class'] ) . "' id=", wp_dropdown_pages( $args ) ) . ' ' . $description;
								?>
							</td>
						</tr>
						<?php
					...
					case 'single_select_country':
						...
						<tr class="<?php echo esc_attr( $value['row_class'] ); ?>">
							...
							<td class="forminp"><select name="<?php echo esc_attr( $value['field_name'] ); ?>" id="<?php echo esc_attr( $value['id'] ); ?>" style="<?php echo esc_attr( $value['css'] ); ?>" data-placeholder="<?php esc_attr_e( 'Choose a country / region&hellip;', 'poocommerce' ); ?>" aria-label="<?php esc_attr_e( 'Country / Region', 'poocommerce' ); ?>" class="wc-enhanced-select">
						...
						<tr class="<?php echo esc_attr( $value['row_class'] ); ?>">
							...
							<td class="forminp">
								<select
									...
									id="<?php echo esc_attr( $value['id'] ); ?>"
									style="width:350px"
									data-placeholder="<?php esc_attr_e( 'Choose countries / regions&hellip;', 'poocommerce' ); ?>"
									aria-label="<?php esc_attr_e( 'Country / Region', 'poocommerce' ); ?>"
									...
									?>
								</select> <?php echo ( $description ) ? $description : ''; // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped -- $description is sanitized with wp_kses_post(). ?> <br /><a class="select_all button" href="#"><?php esc_html_e( 'Select all', 'poocommerce' ); ?></a> <a class="select_none button" href="#"><?php esc_html_e( 'Select none', 'poocommerce' ); ?></a>
							</td>
						</tr>
						<?php
						break;
