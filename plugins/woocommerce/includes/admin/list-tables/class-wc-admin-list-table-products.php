...
class WC_Admin_List_Table_Products extends WC_Admin_List_Table {
	...
	protected function render_blank_state() {
		echo '<div class="poocommerce-BlankState poocommerce-BlankState--products">';
		...
		echo '<a class="poocommerce-BlankState-cta button button-secondary" href="' . esc_url( admin_url( 'post-new.php?post_type=product&tutorial=true' ) ) . '">' . esc_html__( 'Create Product', 'poocommerce' ) . '</a>';
		echo '<a class="poocommerce-BlankState-cta button button-secondary" href="' . esc_url( admin_url( 'edit.php?post_type=product&page=product_importer' ) ) . '">' . esc_html__( 'Start Import', 'poocommerce' ) . '</a>';
	...
	public function define_columns( $columns ) {
		...
		$show_columns['global_unique_id'] = __( 'GTIN, UPC, EAN, or ISBN', 'poocommerce' );
		...
		if ( $this->cogs_is_enabled ) {
			$show_columns['cogs_value'] = __( 'Cost', 'poocommerce' );
		}
	...
	protected function render_product_cat_column() {
		...
		if ( ! $terms ) {
			 ...
			 */
			echo apply_filters( 'poocommerce_admin_product_term_list', implode( ', ', $termlist ), 'product_cat', $this->object->get_id(), $termlist, $terms ); // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped -- Term names and links are escaped above; the filter returns extension-controlled markup by design.
		}
	}
	...
	protected function render_product_tag_column() {
		...
		if ( ! $terms ) {
			 ...
			 * @param WP_Term[]|WP_Error $terms      Term objects assigned to the product, or the WP_Error returned by get_the_terms().
			 */
			echo apply_filters( 'poocommerce_admin_product_term_list', implode( ', ', $termlist ), 'product_tag', $this->object->get_id(), $termlist, $terms ); // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped -- Term names and links are escaped above; the filter returns extension-controlled markup by design.
		}
	}
	...
	protected function render_filters() {
		 ...
		 */
		echo apply_filters( 'poocommerce_product_filters', $output ); // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped -- $output is buffered admin markup and the filter returns extension-controlled markup by design.
	}
	...
	/**
	 * Parse a product search group using PooCommerce search-term rules.
	 *
	...
	private function get_search_stopwords() {
		...
		$filtered_stopwords = apply_filters( 'wp_search_stopwords', $stopwords ); // phpcs:ignore PooCommerce.Commenting.CommentHooks.MissingSinceComment -- The hook is documented by WordPress.
