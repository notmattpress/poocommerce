...
class WC_Product_CSV_Importer_Controller {
	...
	private static function cleanup_after_import( int $cursor = 0 ): ?int {
		...
		$time_limit = (int) apply_filters( 'poocommerce_product_importer_default_time_limit', 20 ); // phpcs:ignore PooCommerce.Commenting.CommentHooks.MissingSinceComment
		...
		while ( true ) {
			...
			// Ascending IDs delete a placeholder parent before its variations, so PooCommerce removes them through the normal lifecycle.
	...
	private static function dispatch_ajax_cleanup( int $cursor ): void {
		...
		if ( null === $next_cursor ) {
			$response['url'] = add_query_arg( array( '_wpnonce' => wp_create_nonce( 'poocommerce-csv-importer' ) ), admin_url( 'edit.php?post_type=product&page=product_importer&step=done' ) );
		}
