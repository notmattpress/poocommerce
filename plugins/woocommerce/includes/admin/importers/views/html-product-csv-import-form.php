...
<form class="wc-progress-form-content poocommerce-importer" enctype="multipart/form-data" method="post">
			...
			<tbody>
				...
				<tr>
					...
					<td>
						...
						<label for="poocommerce-importer-update-existing"><?php esc_html_e( 'Existing products that match by ID or SKU will be updated, and new variations of existing variable products will be created. Other products that do not exist will be skipped.', 'poocommerce' ); ?></label>
					</td>
				</tr>
				...
				<tr class="poocommerce-importer-advanced hidden">
					<th><label for="poocommerce-importer-character-encoding"><?php esc_html_e( 'Character encoding of the file', 'poocommerce' ); ?></label><br/></th>
					<td>
						<?php
						if ( function_exists( 'mb_list_encodings' ) ) {
							?>
							<select id="poocommerce-importer-character-encoding" name="character_encoding">
							...
							<div class="notice notice-warning inline">
								<p><?php esc_html_e( 'Your server does not support the mbstring PHP extension, so the file will be treated as UTF-8. Characters in other encodings may be removed.', 'poocommerce' ); ?></p>
							</div>
							<?php
						}
						?>
	...
	<script type="text/javascript">
		jQuery(function() {
			...
			const fileUrlInput = jQuery( '#poocommerce-importer-file-url' );
			const submitButton = jQuery( '.poocommerce-importer .button-next' );
	...
	<div class="wc-actions">
		...
		<button type="submit" class="button button-primary button-next" value="<?php esc_attr_e( 'Continue', 'poocommerce' ); ?>" name="save_step" disabled><?php esc_html_e( 'Continue', 'poocommerce' ); ?></button>
