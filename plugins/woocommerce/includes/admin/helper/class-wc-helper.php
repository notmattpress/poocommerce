...
class WC_Helper {
	...
	private const CACHE_KEY_CONNECTION_DATA = '_poocommerce_helper_connection_data';
	 ...
	 * subscription list.
	 */
	private const CACHE_KEY_API_ERROR = '_poocommerce_helper_subscriptions_api_error';
	...
	private static function record_api_error( int $code, string $message ): void {
		if ( $code < 100 ) {
			...
			self::log( 'Could not reach the PooCommerce.com API: ' . $message, 'error' );
		}
	...
	public static function get_api_error(): ?array {
		...
		if ( null !== $retry_after ) {
			...
			$message = sprintf(
				...
				__( 'You have exceeded the request limit. Please try again in %s.', 'poocommerce' ),
			...
			$message = __( 'Your store could not connect to PooCommerce.com. Please try again after a few minutes. If the issue persists, check whether your server can make outgoing requests.', 'poocommerce' );
	...
	protected static function add_api_error_notice() {
		...
		$note->set_props(
			array(
				'title'        => __( 'We’re having trouble connecting to PooCommerce.com', 'poocommerce' ),
				'content'      => __( 'Some subscription data may be temporarily unavailable. Please refresh the page in a few minutes to try again.', 'poocommerce' ),
				'type'         => Note::E_WC_ADMIN_NOTE_UPDATE,
				'name'         => self::NOTE_NAME,
				'content_data' => (object) array(),
				'source'       => 'poocommerce-admin',
	...
	private static function get_helper_redirect_url( $args = array() ) {
		...
		if (
			...
			'poocommerce_page_wc-admin' === $current_screen->id ) &&
			(
	...
	private static function _helper_auth_connect() {
		...
		delete_metadata( 'user', 0, \Automattic\PooCommerce\Admin\PluginsHelper::DISMISS_CONNECT_NOTICE, '', true );
		delete_metadata( 'user', 0, \Automattic\PooCommerce\Admin\PluginsHelper::DISMISS_CONNECTED_ACCOUNT_NOTICE, '', true );
	...
	public static function activate_helper_subscription( $product_key ) {
		...
		if ( $activated ) {
			...
			throw new WC_Data_Exception(
				...
				isset( $body['message'] ) ? esc_html( $body['message'] ) : esc_html__( 'Unknown error', 'poocommerce' ),
				(int) $status_code,
	...
	public static function activate_plugin( $product_key ) {
		$subscription = self::get_subscription( $product_key );
		if ( ! $subscription ) {
			throw new Exception( esc_html( __( 'Subscription not found', 'poocommerce' ) ) );
		}
		$product_id = $subscription['product_id'];
		$local      = self::_get_local_from_product_id( $product_id );
	...
	public static function get_product_usage_notice_rules() {
		...
		try {
			...
			if ( ! is_array( $data ) ) {
				...
				throw new Exception( __( 'PooCommerce.com API returned an invalid response.', 'poocommerce' ), 422 );
			}
	...
	/**
	 * Get details of the current PooCommerce.com connection.
	 *
	...
	public static function fetch_helper_connection_info() {
		...
		if ( 200 !== $status ) {
			...
			return new WP_Error(
				'invalid_response',
				'Invalid response from PooCommerce.com',
				array( 'status' => $status )
			);
		}
	...
	public static function get_subscriptions() {
		...
		try {
			...
			if ( 200 !== $code ) {
				...
				// PooCommerce.com already allows a retry. The backoff is the gate.
				if ( 429 === (int) $code ) {
	...
	public static function get_subscription_local_data( array $subscription ) {
		...
		$local_data = array(
			...
			// updates from WordPress.org, and nothing PooCommerce.com-related can hold it back.
	...
	public static function set_subscription_auto_update( string $product_key, bool $enabled ): void {
		$subscription = self::get_subscription( $product_key );
		if ( ! is_array( $subscription ) ) {
			throw new Exception( esc_html__( 'There is no subscription for this product.', 'poocommerce' ) );
		}
		...
		// PooCommerce.com products distributed through WordPress.org.
		$local = self::get_subscription_local_data( $subscription );
		if ( empty( $local['installed'] ) ) {
			throw new Exception( esc_html__( 'This subscription has no installed product to update.', 'poocommerce' ) );
		}
		...
		if ( ! $auto_update['auto_update_manageable'] ) {
			throw new Exception( esc_html__( "Auto-updates for this product can't be changed from here.", 'poocommerce' ) );
		}

		$all_items = $is_theme ? self::get_all_theme_keys() : self::get_all_plugin_keys();
		if ( ! in_array( $item_key, $all_items, true ) ) {
			throw new Exception( esc_html__( 'The product for this subscription is no longer installed.', 'poocommerce' ) );
		}
	...
	private static function get_all_plugin_keys(): array {
		...
		// phpcs:ignore PooCommerce.Commenting.CommentHooks.MissingHookComment, PooCommerce.Commenting.CommentHooks.MissingSinceComment -- Not a PooCommerce hook.
	...
	protected static function get_message_for_response_code( int $code ): string {
		if ( 429 === $code ) {
			return __( 'You have exceeded the request limit. Please try again after a few minutes.', 'poocommerce' );
		} elseif ( 403 === $code ) {
			return __( 'Authentication failed. Please try again after a few minutes. If the issue persists, disconnect your store from PooCommerce.com and reconnect.', 'poocommerce' );
		}

		// translators: %d: HTTP status code.
		return sprintf( __( 'PooCommerce.com API returned HTTP status code %d.', 'poocommerce' ), $code );
	}
}
