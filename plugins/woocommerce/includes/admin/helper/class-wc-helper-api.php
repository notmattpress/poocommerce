	...
	public static function request( $endpoint, $args = array() ) {
		...
		if ( ! empty( $args['authenticated'] ) ) {
			if ( ! self::_authenticate( $url, $args ) ) {
				return new WP_Error( 'authentication', __( 'Authentication failed. Please try again after a few minutes. If the issue persists, disconnect your store from PooCommerce.com and reconnect.', 'poocommerce' ), 401 );
			}
		}
