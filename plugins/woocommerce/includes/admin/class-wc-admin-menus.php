...
class WC_Admin_Menus {
	...
	public function admin_menu() {
		...
		if ( self::can_view_poocommerce_menu_item() ) {
			$menu[] = array( '', 'read', 'separator-poocommerce', '', 'wp-menu-separator poocommerce' ); // phpcs:ignore WordPress.WP.GlobalVariablesOverride.Prohibited -- WordPress uses the $menu global for admin menu registration.
		}
	...
	public function save_settings() {
		 ...
		 */
		if ( '' !== $current_section && apply_filters( "poocommerce_save_settings_{$current_tab}_{$current_section}", ! empty( $_POST['save'] ) ) ) { // phpcs:ignore WordPress.Security.NonceVerification.Missing -- Presence check only; WC_Admin_Settings::save() verifies the manage_poocommerce capability and the poocommerce-settings nonce before anything is written.
			 ...
			 * @param bool $save Whether to save. Defaults to true when the settings form was submitted.
			 */
			&& apply_filters( "poocommerce_save_settings_{$current_tab}", ! empty( $_POST['save'] ) ) // phpcs:ignore WordPress.Security.NonceVerification.Missing -- Presence check only; WC_Admin_Settings::save() verifies the manage_poocommerce capability and the poocommerce-settings nonce before anything is written.
		) {
			WC_Admin_Settings::save();
		}
	}
	...
	public function menu_highlight() {
		...
		switch ( $post_type ) {
			case 'shop_order':
			case 'shop_coupon':
				$parent_file = 'poocommerce'; // phpcs:ignore WordPress.WP.GlobalVariablesOverride.Prohibited -- WordPress uses $parent_file to control admin menu highlighting.
				break;
			case 'product':
				...
				if ( $order_count ) {
					foreach ( $submenu['poocommerce'] as $key => $menu_item ) {
						if ( 0 === strpos( $menu_item[0], _x( 'Orders', 'Admin menu name', 'poocommerce' ) ) ) {
							$submenu['poocommerce'][ $key ][0] .= ' <span class="menu-counter count-' . esc_attr( $order_count ) . '"><span class="processing-count">' . number_format_i18n( $order_count ) . '</span></span>'; // phpcs:ignore WordPress.WP.GlobalVariablesOverride.Prohibited
							break;
						}
					}
				}
	 ...
	 * four core visible boxes, the "PooCommerce endpoints" box, and the WC
	...
	public function filter_default_nav_menu_hidden_meta_boxes( $result, $option, $user ) {
		...
		$visible = array(
			...
			'add-product_cat',
			'add-product_tag',
			'poocommerce_endpoints_nav_link',
		);
	...
	public function nav_menu_links() {
		...
		// Include missing lost password endpoint, if set in PooCommerce > Settings > Advanced > Account endpoints.
		if ( ! empty( get_option( 'poocommerce_myaccount_lost_password_endpoint' ) ) ) {
