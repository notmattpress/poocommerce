...
class WC_Report_Downloads extends WP_List_Table {
	...
	public function get_items( $current_page, $per_page ) {
		...
		$this->items     = $wpdb->get_results( "SELECT * {$query_from} {$query_order}" ); // phpcs:ignore WordPress.DB.PreparedSQL.InterpolatedNotPrepared -- Core builds both fragments with $wpdb->prepare(); the poocommerce_report_downloads_query_from filter lets extensions supply their own SQL.
		$this->max_items = $wpdb->get_var( "SELECT COUNT( DISTINCT download_log_id ) {$query_from};" ); // phpcs:ignore WordPress.DB.PreparedSQL.InterpolatedNotPrepared -- Core builds the fragment with $wpdb->prepare(); the poocommerce_report_downloads_query_from filter lets extensions supply their own SQL.
	}
}
