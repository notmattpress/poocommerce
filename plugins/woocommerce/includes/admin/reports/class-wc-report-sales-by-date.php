	...
	public function get_chart_legend() {
		...
		$legend[] = array(
			'title'            => sprintf(
				...
				_n( '%1$s refunded %2$s order (%3$s item)', '%1$s refunded %2$s orders (%3$s items)', $this->report_data->total_refunded_orders, 'poocommerce' ),
				...
				var drawGraph = function( highlight ) {
					var series = [
						...
						},
						{
							...
							lines: { show: true, lineWidth: 2, fill: false },
							shadowSize: 0,
							prepend_tooltip: <?php echo wp_json_encode( get_poocommerce_currency_symbol(), JSON_UNESCAPED_UNICODE ); ?>
						},
						{
