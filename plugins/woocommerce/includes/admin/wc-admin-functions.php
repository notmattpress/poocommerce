...
use Automattic\PooCommerce\Internal\Utilities\OrderItemMetaUtil;
...
function wc_get_page_screen_id( $for ) {
	...
	if ( in_array( $for, wc_get_order_types( 'admin-menu' ), true ) ) {
		if ( OrderUtil::custom_orders_table_usage_is_enabled() ) {
			$screen_id = ( \WC_Admin_Menus::can_view_poocommerce_menu_item() ? 'poocommerce_page_wc-orders' : 'admin_page_wc-orders' ) . ( 'shop_order' === $for ? '' : '--' . $for );
		} else {
			$screen_id = $for;
		}
	}
...
function wc_save_order_items( $order_id, $items ) {
	...
	if ( ! empty( $qty_change_order_notes ) ) {
		...
		$order->add_order_note( sprintf( __( 'Adjusted stock: %s', 'poocommerce' ), implode( ', ', $qty_change_order_notes ) ), false, true, array( 'note_group' => OrderNoteGroup::PRODUCT_STOCK ) );
	}
