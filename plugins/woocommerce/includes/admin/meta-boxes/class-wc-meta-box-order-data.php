						...
						<p class="poocommerce-order-data__meta order_number">
							...
							if ( $order->get_date_paid() ) {
								$meta_list[] = sprintf(
									...
									__( 'Paid on %1$s at %2$s', 'poocommerce' ),
						...
						<p class="form-field form-field-wide wc-customer-user">
							...
							if ( $order->get_user_id() ) {
								...
								if ( ! is_wp_error( $user ) ) {
									...
									$found_users = array( $user_id => esc_html__( '(Not available)', 'poocommerce' ) );
								}
							}
							?>
							<select class="wc-customer-search" id="customer_user" name="customer_user" data-placeholder="<?php esc_attr_e( 'Guest', 'poocommerce' ); ?>" data-allow_clear="true">
								 ...
								 */
								?>
								<option value="<?php echo esc_attr( $user_id ); ?>" selected="selected"><?php echo esc_html( htmlspecialchars( wp_kses_post( current( apply_filters( 'poocommerce_json_search_found_customers', $found_users ) ) ), ENT_QUOTES | ENT_SUBSTITUTE | ENT_HTML401 ) ); ?></option>
						...
						<div class="address">
							...
							if ( $order->get_user_id() !== 0 && is_wp_error( $user ) ) {
								...
								if ( $order->get_formatted_billing_address() ) {
									...
									echo '<p class="none_set">' . esc_html__( 'No billing address set.', 'poocommerce' ) . '</p>';
								}
								...
								foreach ( $billing_fields as $key => $field ) {
									...
									if ( 'billing_phone' === $field_name ) {
										...
										$field_value      = '' === $normalized_email
											? '<span class="none_set">' . esc_html__( 'No email address set.', 'poocommerce' ) . '</span>'
						...
						<div class="edit_address">
							...
							<p class="form-field form-field-wide">
								...
								<select name="_payment_method" id="_payment_method" class="first">
									<option value=""><?php esc_html_e( 'All methods available', 'poocommerce' ); ?></option>
									<?php
									$found_method = false;
					...
					<div class="order_data_column order_data_column_shipping">
						...
						<div class="address">
							...
							if ( $order->get_user_id() !== 0 && is_wp_error( $user ) ) {
								 ...
								 * @since 11.1.0
								 */
								$hide_core_shipping_details = apply_filters( 'poocommerce_hide_order_admin_shipping_details', $hide_core_shipping_details, $order );
								...
								if ( $shipping_address ) {
									...
									echo '<p class="none_set">' . esc_html__( 'No shipping address set.', 'poocommerce' ) . '</p>';
								}
						...
						<div class="edit_address">
							...
							if ( apply_filters( 'poocommerce_enable_order_notes_field', 'yes' === get_option( 'poocommerce_enable_order_comments', 'yes' ) ) ) :
								?>
								<p class="form-field form-field-wide">
									<label for="excerpt"><?php esc_html_e( 'Customer provided note', 'poocommerce' ); ?>:</label>
