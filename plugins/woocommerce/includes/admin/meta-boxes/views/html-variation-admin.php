		...
		<div class="data">
			<div class="form-flex-box">
				<div class="form-row upload_image">
					 ...
					 * @param WP_Post $variation      Post data.
					 */
					do_action( 'poocommerce_variation_after_upload_image', $loop, $variation_data, $variation );
					?>
				</div>
			...
			<div class="variable_pricing">
				...
				if ( wc_tax_enabled() ) {
					$tax_text  = wc_prices_include_tax()
						? __( 'incl. tax', 'poocommerce' )
						: __( 'ex. tax', 'poocommerce' );
					$tax_label = ' (' . $tax_text . ')';
				}

				$label = sprintf(
					/* translators: 1: currency symbol, 2: tax label (prefixed with space if present) */
					__( 'Regular price (%1$s%2$s)', 'poocommerce' ),
				...
				$label = sprintf(
					/* translators: 1: currency symbol, 2: tax label (prefixed with space if present) */
					__( 'Sale price (%1$s%2$s)', 'poocommerce' ),
					get_poocommerce_currency_symbol(),
					$tax_label ? ' ' . $tax_label : ''
				);
				...
				<div class="variable_pricing">
					...
					poocommerce_wp_text_input(
						array(
							...
							'description'        => array(
								__( 'Add the amount it costs you to buy or make this product. Leave blank to use the default value from "General".', 'poocommerce' ),
								__( 'You can specify a <a href="#" class="switch-to-general-tab">default value</a> for all variations', 'poocommerce' ),
							),
							'placeholder'        =>
								/* Translators: %s = cost of the item (monetary value) */
								sprintf( __( '%s (default)', 'poocommerce' ), wc_format_localized_price( $base_cost ) ),
						)
					);
					?>
				</div>
