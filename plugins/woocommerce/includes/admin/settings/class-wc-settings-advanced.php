...
class WC_Settings_Advanced extends WC_Settings_Page {
	...
	protected function get_own_sections() {
		$sections = array(
			...
			'keys' => __( 'REST API keys', 'poocommerce' ),
		);

		$features_controller = wc_get_container()->get( FeaturesController::class );
		if ( $features_controller->feature_is_enabled( 'rest_api_caching' ) ) {
			$sections['rest_api_caching'] = __( 'REST API caching', 'poocommerce' );
		}

		$sections['webhooks'] = __( 'Webhooks', 'poocommerce' );

		if ( has_filter( 'poocommerce_settings_rest_api' ) ) {
			$sections['legacy_api'] = __( 'Legacy API', 'poocommerce' );
		}

		$sections['poocommerce_com'] = __( 'PooCommerce.com', 'poocommerce' );

		if ( FeaturesUtil::feature_is_enabled( 'blueprint' ) ) {
			$sections['blueprint'] = __( 'Blueprint (beta)', 'poocommerce' );
		}
		...
		$settings =
			array(
				...
				array(
					...
					'desc_tip'      => esc_html__( 'Leave this box unchecked if you do not want to pull suggested extensions from PooCommerce.com.', 'poocommerce' ),
	 ...
	 * `poocommerce_settings_rest_api` filter, so this acts purely as an
	...
	protected function get_settings_for_legacy_api_section() {
		 ...
		 */
		return apply_filters( 'poocommerce_settings_rest_api', array() );
	}
	...
	protected function get_settings_for_rest_api_caching_section() {
		...
		$settings = array(
			array(
				'title' => __( 'REST API response cache', 'poocommerce' ),
				'type'  => 'title',
				'desc'  => __( 'These settings control backend caching and cache control headers for REST API responses.', 'poocommerce' ),
		...
		if ( ! $has_object_cache ) {
			$settings[] = array(
				...
				'text'        => sprintf(
					...
					__( 'Backend caching requires a WordPress object cache plugin (Redis, Memcached, etc.) to be installed and active. %1$sLearn more about object caching%2$s.', 'poocommerce' ),
		...
		$backend_caching_setting = array(
			'title'       => __( 'Enable backend caching', 'poocommerce' ),
			'desc'        => __( 'Cache REST API responses on the server', 'poocommerce' ),
			'id'          => 'poocommerce_rest_api_enable_backend_caching',
			'type'        => 'checkbox',
			'default'     => 'no',
			'disabled'    => ! $has_object_cache,
			'fixed_value' => $has_object_cache ? null : 'no',
			'desc_tip'    => __( 'Enables responses for REST API endpoints configured as cacheable. Requires an external object cache.<br/>This setting should be enabled only if no other plugins that handle caching are active.', 'poocommerce' ),
		);

		$settings[] = $backend_caching_setting;

		$settings[] = array(
			'title'    => __( 'Enable cache control headers', 'poocommerce' ),
			'desc'     => __( 'Send cache control headers and support 304 Not Modified responses', 'poocommerce' ),
			'id'       => 'poocommerce_rest_api_enable_cache_headers',
			'type'     => 'checkbox',
			'default'  => 'yes',
			'desc_tip' => __( 'Enables including ETag and Cache-Control headers, and returning 304 Not Modified responses, for REST API endpoints configured as cacheable.', 'poocommerce' ),
		);
		 ...
		 */
		return apply_filters( 'poocommerce_rest_api_cache_settings', $settings );
	}
