...
<?php wc_back_header( $zone->get_zone_name() ? $zone->get_zone_name() : __( 'Add zone', 'poocommerce' ), __( 'Return to shipping', 'poocommerce' ), admin_url( 'admin.php?page=wc-settings&tab=shipping' ) ); ?>
					...
					<div class="inner">
						<div class="wc-backbone-modal-buttons">
							...
							<button id="btn-ok" data-status='{{ data.status }}' class="button button-primary button-large">
								<div class="wc-backbone-modal-action-{{ data.status === 'new' ? 'active' : 'inactive' }}"><?php esc_html_e( 'Save zone and method', 'poocommerce' ); ?></div>
