						...
						<?php
							...
							$topics = apply_filters(
								...
								array(
									...
									'product.published' => __( 'Product published', 'poocommerce' ),
					...
					<select name="webhook_api_version" id="webhook_api_version">
						...
						<?php if ( WC()->legacy_rest_api_is_available() ) : ?>
							<option value="legacy_v3" <?php selected( 'legacy_v3', $webhook->get_api_version(), true ); ?>>
								<?php esc_html_e( 'Legacy API v3 (deprecated)', 'poocommerce' ); ?>
							</option>
						<?php elseif ( ! in_array( $webhook->get_api_version(), wc_get_webhook_rest_api_versions(), true ) ) : ?>
							<option value="<?php echo esc_attr( $webhook->get_api_version() ); ?>" selected="selected">
								<?php
									...
									echo esc_html( sprintf( __( '%s (unsupported)', 'poocommerce' ), $webhook->get_api_version() ) );
								?>
							</option>
						<?php endif; ?>
					</select>
				...
				<td colspan="2" scope="row" style="padding-left: 0;">
					<p class="submit">
						...
						if ( $webhook->get_id() ) :
							...
							?>
							<a style="color: var(--wc-destructive, #cc1818); text-decoration: none; margin-left: 10px;" href="<?php echo esc_url( $delete_url ); ?>"><?php esc_html_e( 'Delete permanently', 'poocommerce' ); ?></a>
						<?php endif; ?>
					</p>
				</td>
			</tr>
