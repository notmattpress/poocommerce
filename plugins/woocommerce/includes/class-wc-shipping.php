...
class WC_Shipping {
	...
	public function get_shipping_method_class_names() {
		...
		wp_prime_option_caches( array_map( fn( string $method ) => sprintf( 'poocommerce_%s_settings', $method ), $maybe_load_legacy_methods ) );
	...
	public function calculate_shipping_for_package( $package = array(), $package_key = 0 ) {
		...
		if ( ! is_array( $stored_rates ) || $package_hash !== $stored_rates['package_hash'] || 'yes' === get_option( 'poocommerce_shipping_debug_mode', 'no' ) ) {
			...
			if ( 'yes' === get_option( 'poocommerce_shipping_hide_rates_when_free', 'no' ) ) {
				$free_shipping = array();
				$local_pickup  = array();
	...
	private function get_package_hash( array $package_to_hash ): string {
		...
		$ignored_fields = apply_filters(
			'poocommerce_shipping_package_hash_ignored_fields',
			array(
				'subtotal',
				'total',
				'package_id',
