...
if ( ! class_exists( 'WC_Email_Cancelled_Order', false ) ) :
	...
	class WC_Email_Cancelled_Order extends WC_Email {
		...
		public function __construct() {
			...
			add_action( 'poocommerce_order_status_pending_to_cancelled_notification', array( $this, 'trigger' ), 10, 2 );
			...
			$this->description = $this->email_improvements_enabled
				? __( 'Receive an email notification when an order that was pending, processing, or on hold gets cancelled', 'poocommerce' )
				: __( 'Cancelled order emails are sent to chosen recipient(s) when orders have been marked cancelled (if they were previously pending, processing, or on-hold).', 'poocommerce' );

			if ( $this->block_email_editor_enabled ) {
				$this->title       = __( 'Canceled order', 'poocommerce' );
				$this->description = __( 'Notifies admins when an order that was pending, processing, or on hold has been canceled.', 'poocommerce' );
			}
