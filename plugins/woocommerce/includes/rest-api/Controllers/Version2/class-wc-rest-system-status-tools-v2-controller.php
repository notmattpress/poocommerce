...
class WC_REST_System_Status_Tools_V2_Controller extends WC_REST_Controller {
	...
	public function get_tools() {
		...
		if ( wc_update_product_lookup_tables_is_running() ) {
			...
			$tools['regenerate_product_lookup_tables']['button']      = __( 'Regenerating in progress', 'poocommerce' );
		...
		if ( isset( $tools['regenerate_thumbnails'] ) && WC_Regenerate_Images::is_regeneration_in_progress() ) {
			...
			$tools['regenerate_thumbnails']['button']      = __( 'Regenerating in progress', 'poocommerce' );
	...
	private function get_regenerating_lookup_table_status_text() {
		...
		$progress_label      = $cron_disabled
			? __( 'View queued updates', 'poocommerce' )
			: __( 'View progress', 'poocommerce' );
	...
	private function get_regenerating_thumbnails_status_text() {
		...
		return sprintf(
			...
			esc_url( $cancel_url ),
			esc_attr__( 'Cancel thumbnail regeneration', 'poocommerce' ),
			esc_html__( 'Cancel', 'poocommerce' )
		);
	}
	...
	public function execute_tool( $tool ) {
		...
		switch ( $tool ) {
			...
			case 'clear_sessions':
				...
				$result = absint( $wpdb->query( "DELETE FROM {$wpdb->usermeta} WHERE meta_key='_poocommerce_persistent_cart_" . get_current_blog_id() . "';" ) );
				wp_cache_flush();
