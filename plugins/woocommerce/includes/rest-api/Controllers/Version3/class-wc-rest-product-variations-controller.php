...
class WC_REST_Product_Variations_Controller extends WC_REST_Product_Variations_V2_Controller {
		...
		$schema               = array(
			...
			'properties' => array(
				...
				'gallery_image_ids'     => array(
					'description' => __( 'Variation gallery image IDs, excluding the featured image (which is set via "image"). Mirrors how galleries work on parent products.', 'poocommerce' ),
					'type'        => 'array',
					'context'     => array( 'view', 'edit' ),
					'items'       => array(
						'type'    => 'integer',
	...
	public function get_collection_params() {
		...
		$params['pos_products_only'] = array(
			'description'       => __( 'Limit result set to variations visible in Point of Sale.', 'poocommerce' ),
			'type'              => 'boolean',
			'sanitize_callback' => 'wc_string_to_bool',
			'validate_callback' => 'rest_validate_request_arg',
		);
		...
		$params['image_size'] = array(
			'description'       => __( 'Use a specific registered image size for the returned variation image src. Falls back to the full size if the requested size is not registered.', 'poocommerce' ),
			'type'              => 'string',
			'default'           => 'full',
			'sanitize_callback' => 'sanitize_text_field',
			'validate_callback' => 'rest_validate_request_arg',
