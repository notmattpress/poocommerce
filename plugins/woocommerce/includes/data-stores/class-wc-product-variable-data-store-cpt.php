...
class WC_Product_Variable_Data_Store_CPT extends WC_Product_Data_Store_CPT implements WC_Object_Data_Store_Interface, WC_Product_Variable_Data_Store_Interface {
	...
	protected function read_attributes( &$product ) {
		...
		if ( ! empty( $meta_attributes ) && is_array( $meta_attributes ) ) {
			...
			foreach ( $meta_attributes as $meta_attribute_key => $meta_attribute_value ) {
				 ...
				 */
				$attributes[] = apply_filters( 'poocommerce_product_read_attribute', $attribute, $meta_value, $product );
			}
	...
	public function read_children( &$product, $force_read = false ) {
		...
		if ( ! isset( $children['all'] ) || ! isset( $children['visible'] ) || $force_read ) {
			...
			// phpcs:disable PooCommerce.Commenting.CommentHooks
	...
	public function read_price_data( &$product, $for_display = false ) {
		 ...
		 * - rewrite this method, if breaking the contracts is an option (it's not as per PooCommerce v11.1)
		...
		if ( empty( $this->prices_array[ $price_hash ] ) ) {
			...
			if ( empty( $transient_cached_prices_array[ $price_hash ] ) || ( null !== $opposite_price_hash && empty( $transient_cached_prices_array[ $opposite_price_hash ] ) ) ) {
				...
				$tax_display_mode = $for_display ? get_option( 'poocommerce_tax_display_shop' ) : null;
				...
				foreach ( $variation_ids as $variation_id ) {
					...
					if ( $variation ) {
						 ...
						 * 'poocommerce_get_variation_prices_hash' filter to include that factor in the cache key.
						 ...
						 *         add_filter( 'poocommerce_variation_prices_price', array( $this, 'apply_user_discount' ), 10, 3 );
						 *         add_filter( 'poocommerce_get_variation_prices_hash', array( $this, 'add_user_to_hash' ), 10, 3 );
						 *     }
						 *
						 ...
						 *
						 * IMPORTANT: see the documentation for the 'poocommerce_variation_prices_price' filter.
						 *
						 * @since 3.0.0
						 *
						...
						if ( has_filter( 'poocommerce_variation_prices_array' ) ) {
							...
							if ( null !== $opposite_price_hash ) {
								// $for_display doesn't affect raw prices here, but a poocommerce_variation_prices_array hook might.
								// phpcs:ignore PooCommerce.Commenting.CommentHooks
								$opposite_prices_array = apply_filters( 'poocommerce_variation_prices_array', $original_prices_array, $variation, ! $for_display );
			...
			if ( $this->prices_array[ $price_hash ] !== $transient_cached_prices_array[ $price_hash ] && ! $this->validate_prices_data( array( $price_hash => $this->prices_array[ $price_hash ] ), $transient_version ) ) {
				wc_doing_it_wrong(
					__METHOD__,
					__( '`poocommerce_variation_prices` returned an unsupported data format. The value is currently used as-is but will be ignored in a future release. Ensure your callback returns the expected array structure.', 'poocommerce' ),
					'11.1'
				);
			}

			if ( null !== $opposite_price_hash && $opposite_price_hash !== $price_hash ) {
				$this->prices_array[ $opposite_price_hash ] = apply_filters( 'poocommerce_variation_prices', $transient_cached_prices_array[ $opposite_price_hash ], $product, ! $for_display );
	...
	protected function taxes_influence_price( $product ): bool {
		 ...
		 * @since 10.9.0
		 */
		return (bool) apply_filters( 'poocommerce_variable_product_taxes_influence_price', $taxes_influence_price, $product );
	}
	...
	protected function get_price_hash( &$product, $for_display = false ) {
		...
		if ( $for_display && wc_tax_enabled() ) {
			$price_hash = array(
				get_option( 'poocommerce_tax_display_shop', TaxDisplayMode::EXCLUSIVE ),
				WC_Tax::get_rates(),
		 ...
		 *
		 * By default, PooCommerce will use the legacy algorithm to get the callback signatures
		 ...
		 *
		 * IMPORTANT: see also the documentation for the 'poocommerce_variation_prices_price' filter.
		 *
		 * @since 10.5.0
		 *
		 ...
		 * @param bool       $for_display If taxes should be calculated or not.
		 */
		$use_legacy_algorithm = apply_filters( 'poocommerce_use_legacy_get_variations_price_hash', true, $product, $for_display );
		 ...
		 */
		$price_hash = apply_filters( 'poocommerce_get_variation_prices_hash', $price_hash, $product, $for_display );
