...
abstract class Abstract_WC_Order_Data_Store_CPT extends WC_Data_Store_WP implements WC_Abstract_Order_Data_Store_Interface, WC_Object_Data_Store_Interface {
	...
	*/
	...
	protected function read_order_data( &$order, $post_object ) {
		...
		$this->set_order_props(
			...
			array(
				...
				'prices_include_tax' => metadata_exists( 'post', $id, '_prices_include_tax' ) ? 'yes' === $prices_include_tax : 'yes' === get_option( 'poocommerce_prices_include_tax' ),
			)
		);
	...
	protected function prime_order_item_caches_for_orders( $order_ids, $query_vars ) {
		...
		$raw_meta_data_array = $wpdb->get_results(
			$wpdb->prepare(
				...
				"SELECT order_item_id as object_id, meta_id, meta_key, meta_value FROM {$wpdb->prefix}poocommerce_order_itemmeta WHERE order_item_id IN ({$id_placeholders}) ORDER BY meta_id",
				...$order_item_ids
			)
		);
	...
	public function get_item_ids( $order, $type = null ) {
		...
		if ( null === $type ) {
			$item_ids = $wpdb->get_col(
				$wpdb->prepare(
					"SELECT order_item_id FROM {$wpdb->prefix}poocommerce_order_items WHERE order_id = %d",
					$order->get_id()
				)
			);
		} else {
			$item_ids = $wpdb->get_col(
				$wpdb->prepare(
					"SELECT order_item_id FROM {$wpdb->prefix}poocommerce_order_items WHERE order_id = %d AND order_item_type = %s",
					$order->get_id(),
					$type
				)
			);
		...
		if ( '' !== $wpdb->last_error ) {
			...
			throw new Exception( esc_html__( 'Unable to retrieve persisted order item IDs.', 'poocommerce' ) );
		}
	...
	public function delete_items_by_ids( $order, $ids ) {
		...
		$result = $wpdb->query(
			$wpdb->prepare(
				...
				"DELETE itemmeta FROM {$wpdb->prefix}poocommerce_order_itemmeta as itemmeta INNER JOIN {$wpdb->prefix}poocommerce_order_items as items WHERE itemmeta.order_item_id = items.order_item_id AND items.order_id = %d AND items.order_item_id IN ($ids_placeholders)",
		...
		if ( false === $result ) {
			...
			throw new Exception( esc_html__( 'Unable to delete order item metadata.', 'poocommerce' ) );
		}
		$result = $wpdb->query(
			$wpdb->prepare(
				// phpcs:ignore WordPress.DB.PreparedSQL.InterpolatedNotPrepared, WordPress.DB.PreparedSQLPlaceholders.UnfinishedPrepare -- $ids_placeholders is generated above.
				"DELETE FROM {$wpdb->prefix}poocommerce_order_items WHERE order_id = %d AND order_item_id IN ($ids_placeholders)",
				array_merge( array( $order->get_id() ), $sanitized_ids )
			)
		);
		if ( false === $result ) {
			...
			throw new Exception( esc_html__( 'Unable to delete order items.', 'poocommerce' ) );
		}
	...
	public function delete_items( $order, $type = null ) {
		...
		if ( ! empty( $type ) ) {
			$wpdb->query( $wpdb->prepare( "DELETE itemmeta FROM {$wpdb->prefix}poocommerce_order_itemmeta as itemmeta INNER JOIN {$wpdb->prefix}poocommerce_order_items as items WHERE itemmeta.order_item_id = items.order_item_id AND items.order_id = %d AND items.order_item_type = %s", $order_id, $type ) );
			$wpdb->query( $wpdb->prepare( "DELETE FROM {$wpdb->prefix}poocommerce_order_items WHERE order_id = %d AND order_item_type = %s", $order_id, $type ) );
		} else {
			$wpdb->query( $wpdb->prepare( "DELETE itemmeta FROM {$wpdb->prefix}poocommerce_order_itemmeta as itemmeta INNER JOIN {$wpdb->prefix}poocommerce_order_items as items WHERE itemmeta.order_item_id = items.order_item_id and items.order_id = %d", $order_id ) );
			$wpdb->query( $wpdb->prepare( "DELETE FROM {$wpdb->prefix}poocommerce_order_items WHERE order_id = %d", $order_id ) );
		}
	...
	protected function get_refunded_item_meta_total( $order, string $item_type, array $meta_keys ): float {
		...
		$order_itemmeta   = $wpdb->prefix . 'poocommerce_order_itemmeta';
		$order_items      = $wpdb->prefix . 'poocommerce_order_items';
	...
	private function should_suppress_transactional_email_dispatch( string $action, int $restored_order_id, array $args ): bool {
		if ( 0 !== strpos( $action, 'poocommerce_order_status_' ) ) {
			return false;
		}
