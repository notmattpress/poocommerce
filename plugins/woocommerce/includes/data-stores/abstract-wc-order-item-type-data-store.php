...
abstract class Abstract_WC_Order_Item_Type_Data_Store extends WC_Data_Store_WP implements WC_Object_Data_Store_Interface {
	...
	public function read( &$item ) {
		...
		if ( false === $data ) {
			$data = $wpdb->get_row( $wpdb->prepare( "SELECT order_item_type, order_item_id, order_id, order_item_name FROM {$wpdb->prefix}poocommerce_order_items WHERE order_item_id = %d LIMIT 1;", $item->get_id() ) );
