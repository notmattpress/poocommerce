...
class WC_Order_Item_Product extends WC_Order_Item {
	...
	public function set_variation_id( $value ) {
		if ( $value > 0 && 'product_variation' !== get_post_type( $value ) ) {
			$this->error(
				...
				__( 'Invalid variation ID', 'poocommerce' ),
				400,
		...
		if ( ! empty( $raw_tax_data['total'] ) && ! empty( $raw_tax_data['subtotal'] ) ) {
			...
			if ( $has_legacy_data ) {
				...
				wc_get_logger()->warning(
					...
					array(
						'source'        => 'poocommerce-order-item-product',
	...
	*/
	...
	public function get_product() {
		 ...
		 * @param WC_Order_Item_Product $line_item Line item instance.
		 */
		$product = apply_filters( 'poocommerce_order_item_product', $product, $this );
	...
	public function get_item_downloads() {
		 ...
		 */
		$files = apply_filters( 'poocommerce_get_item_downloads', $files, $this, $order );
	...
	public function get_formatted_meta_data( $hideprefix = '_', $include_all = false ) {
		...
		if ( $order && $order->has_status( OrderStatus::COMPLETED ) ) {
			 ...
			 * @return string
			 */
			$backorder_meta_key = apply_filters( 'poocommerce_backordered_item_meta_name', __( 'Backordered', 'poocommerce' ), $this );
