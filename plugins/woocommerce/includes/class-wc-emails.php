...
class WC_Emails {
	...
	public static function init_transactional_emails() {
		...
		$email_actions = apply_filters(
			...
			array(
				...
				'poocommerce_order_status_pending_to_cancelled',
				...
				'poocommerce_send_review_request',
		 ...
		 */
		if ( apply_filters( 'poocommerce_defer_transactional_emails', $defer_default ) ) {
	...
	public function init() {
		...
		wp_prime_option_caches(
			array_map(
				fn( string $class_name ) => sprintf( 'poocommerce_%s_settings', strtolower( str_replace( 'WC_Email_', '', $class_name ) ) ),
				array_keys( $emails )
			)
		);
	...
	public function get_from_name() {
		...
		return wp_specialchars_decode( get_option( 'poocommerce_email_from_name', $default ), ENT_QUOTES );
	}
	...
	public function backorder( $args ) {
		...
		if ( 'no' === get_option( 'poocommerce_notify_backorder', 'yes' ) ) {
			return;
		}
		 ...
		 * @since 11.0.0
		 */
		if ( false === apply_filters( 'poocommerce_should_send_backorder_notification', true, $args['product']->get_id() ) ) {
			return;
		}
