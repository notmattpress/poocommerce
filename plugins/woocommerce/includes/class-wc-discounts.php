...
class WC_Discounts {
	...
	protected function validate_coupon_product_ids( $coupon ) {
		if ( count( $coupon->get_product_ids() ) > 0 ) {
			 ...
			 * poocommerce_coupon_is_valid_for_* filters follow the poocommerce_coupon_is_valid convention where
			 * true means valid. This is the opposite of the poocommerce_coupon_validate_* filters
			 * (e.g. poocommerce_coupon_validate_expiry_date), where returning true rejects the coupon.
			 *
			 * @since 11.1.0
			 ...
			 */
			$valid = apply_filters( 'poocommerce_coupon_is_valid_for_product_ids', $valid, $coupon, $this );
	...
	protected function validate_coupon_product_categories( $coupon ) {
		if ( count( $coupon->get_product_categories() ) > 0 ) {
			 ...
			 * @param WC_Discounts $discounts The discounts instance.
			 */
			$valid = apply_filters( 'poocommerce_coupon_is_valid_for_product_categories', $valid, $coupon, $this );
	...
	protected function validate_coupon_sale_items( $coupon ) {
		if ( $coupon->get_exclude_sale_items() ) {
			 ...
			 * @param WC_Discounts $discounts The discounts instance.
			 */
			$valid = apply_filters( 'poocommerce_coupon_is_valid_for_sale_items', $valid, $coupon, $this );
	...
	protected function validate_coupon_excluded_items( $coupon ) {
		...
		if ( ! empty( $items ) && $coupon->is_type( wc_get_product_coupon_types() ) ) {
			 ...
			 * @param WC_Discounts $discounts The discounts instance.
			 */
			$valid = apply_filters( 'poocommerce_coupon_is_valid_for_excluded_items', $valid, $coupon, $this );
	...
	protected function validate_coupon_excluded_product_ids( $coupon ) {
		...
		if ( count( $coupon->get_excluded_product_ids() ) > 0 ) {
			 ...
			 * poocommerce_coupon_is_valid_for_* filters follow the poocommerce_coupon_is_valid convention where
			 * true means valid. This is the opposite of the poocommerce_coupon_validate_* filters
			 * (e.g. poocommerce_coupon_validate_expiry_date), where returning true rejects the coupon. When
			 * rejecting, use the poocommerce_coupon_error filter (error code
			 ...
			 * @param WC_Discounts $discounts The discounts instance.
			 */
			$valid = apply_filters( 'poocommerce_coupon_is_valid_for_excluded_product_ids', $valid, $coupon, $this );
	...
	protected function validate_coupon_excluded_product_categories( $coupon ) {
		if ( count( $coupon->get_excluded_product_categories() ) > 0 ) {
			 ...
			 * @param WC_Discounts $discounts The discounts instance.
			 */
			$valid = apply_filters( 'poocommerce_coupon_is_valid_for_excluded_product_categories', $valid, $coupon, $this );
