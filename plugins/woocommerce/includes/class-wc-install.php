...
use Automattic\PooCommerce\Admin\API\Reports\Orders\Stats\DataStore;
...
class WC_Install {
	...
	public static function init() {
		...
		add_action( 'poocommerce_newly_installed', array( __CLASS__, 'enable_email_improvements_for_newly_installed' ), 20 );
		add_action( 'poocommerce_newly_installed', array( __CLASS__, 'enable_customer_stock_notifications_signups' ), 20 );
		add_action( 'poocommerce_newly_installed', array( __CLASS__, 'enable_analytics_scheduled_import' ), 20 );
		add_action( 'poocommerce_newly_installed', array( __CLASS__, 'enable_product_instance_caching_for_newly_installed' ), 20 );
		add_action( 'poocommerce_updated', array( __CLASS__, 'enable_email_improvements_for_existing_merchants' ), 20 );
		...
		// DB update notice.
		add_filter( 'poocommerce_get_note_from_db', array( \WC_Notes_Run_Db_Update::class, 'maybe_update_notice' ), 10 );
		add_action( 'poocommerce_hide_update_notice', array( __CLASS__, 'remove_update_db_notice' ) );
	}
	...
	private static function add_update_db_notice() {
		...
		try {
			if ( WC()->is_wc_admin_active() && false !== get_option( 'poocommerce_admin_install_timestamp' ) ) {
	...
	public static function install_actions() {
		if ( ! empty( $_GET['do_update_poocommerce'] ) ) {
			...
			if ( ! empty( $return_url ) ) {
				...
				if ( 'wc-admin-referer' === $return_url ) {
					...
					if ( $return_url && false === strpos( $return_url, 'do_update_poocommerce' ) ) {
	...
	/**
	 * Get the names of any missing PooCommerce base tables.
	 *
	...
	public static function is_new_install() {
		// Performance note: poocommerce_version is absent before the very first install routine completes.
		if ( false === get_option( 'poocommerce_version' ) ) {
			return true;
		}

		// Performance note: verify if the store is live. This option is auto-loaded, and verification is essentially free.
		if ( 'no' === get_option( 'poocommerce_coming_soon', 'yes' ) ) {
			return false;
		}

		// Performance note: verify if onboarding is complete. This option is auto-loaded, and verification is essentially free.
		if ( in_array( 'setup', (array) get_option( 'poocommerce_task_list_completed_lists', array() ), true ) ) {
			return false;
		}
	 ...
	 * This is the default behavior since 9.9.0 and can be overridden via filter 'poocommerce_enable_auto_update_db'.
	 *
	 * @since 9.9.0
	 *
	...
	public static function is_db_auto_update_enabled(): bool {
		 ...
		 * @since 3.2.0
		 */
		return (bool) apply_filters( 'poocommerce_enable_auto_update_db', true );
	}
	...
	private static function update() {
		...
		if ( self::is_db_auto_update_enabled() ) {
			 ...
			 */
			$scheduled_time_delay = absint( apply_filters( 'poocommerce_db_update_schedule_delay', 0 ) );
	...
	public static function update_db_version( $version = null ) {
		...
		update_option( 'poocommerce_db_version', $version );
	}
	...
	public static function enable_email_improvements_for_newly_installed() {
		...
		update_option( 'poocommerce_email_improvements_default_enabled', 'yes' );
		update_option( 'poocommerce_email_auto_sync_with_theme', 'yes' );
		update_option( 'poocommerce_email_improvements_first_enabled_at', gmdate( 'Y-m-d H:i:s' ) );
		update_option( 'poocommerce_email_improvements_last_enabled_at', gmdate( 'Y-m-d H:i:s' ) );
		update_option( 'poocommerce_email_improvements_enabled_count', 1 );
	}
	...
	public static function enable_analytics_scheduled_import(): void {
		...
		add_option( 'poocommerce_analytics_scheduled_import', 'yes' );
	}
	...
	public static function enable_email_improvements_for_existing_merchants() {
		...
		$feature_controller = wc_get_container()->get( FeaturesController::class );
		$feature_controller->change_feature_enable( 'email_improvements', true );
		update_option( 'poocommerce_email_improvements_existing_store_enabled', 'yes' );
		$first_enabled_at = get_option( 'poocommerce_email_improvements_first_enabled_at' );
		if ( ! $first_enabled_at ) {
			update_option( 'poocommerce_email_improvements_first_enabled_at', gmdate( 'Y-m-d H:i:s' ) );
		}
		$last_enabled_at = get_option( 'poocommerce_email_improvements_last_enabled_at' );
		if ( ! $last_enabled_at ) {
			update_option( 'poocommerce_email_improvements_last_enabled_at', gmdate( 'Y-m-d H:i:s' ) );
		}
		$enabled_count = get_option( 'poocommerce_email_improvements_enabled_count' );
		if ( ! $enabled_count ) {
			update_option( 'poocommerce_email_improvements_enabled_count', 1 );
		} else {
			update_option( 'poocommerce_email_improvements_enabled_count', (int) $enabled_count + 1 );
		}
	}
		...
		if ( $wpdb->get_var( "SHOW TABLES LIKE '{$wpdb->prefix}wc_order_tax_lookup';" ) ) {
			...
			if ( $tax_lookup_alterations ) {
				...
				if ( false === $tax_lookup_altered ) {
					wc_get_logger()->error(
						sprintf(
							'Could not re-key %1$s by tax order item: %2$s. Analytics tax reports keep reading the way they did, and running "Verify base database tables" under PooCommerce > Status > Tools retries the change.',
...
$stock_notifications_table_schema;
	...
	private static function create_placeholder_image() {
		...
		$filename   = $upload_dir['basedir'] . '/poocommerce-placeholder.webp';
	 ...
	 * poocommerce_placeholder_image option still points at PooCommerce's own generated placeholder.
	 ...
	 * poocommerce_ options during uninstall.
	 *
	 * @since 11.0.0
	 *
	 * @return void
	...
	public static function delete_placeholder_image() {
		$placeholder_image = absint( get_option( 'poocommerce_placeholder_image', 0 ) );

		if ( ! $placeholder_image ) {
			return;
		}

		// Only delete PooCommerce's own generated placeholder, never a custom image the merchant may have set.
		$attached_file = (string) get_post_meta( $placeholder_image, '_wp_attached_file', true );
		if ( 'poocommerce-placeholder.webp' !== wp_basename( $attached_file ) ) {
			return;
		}
...
<!-- wp:poocommerce/product-collection {"queryId":0,"query":{"perPage":3,"pages":1,"offset":0,"postType":"product","order":"asc","orderBy":"title","search":"","exclude":[],"inherit":false,"taxQuery":{},"isProductCollectionBlock":true,"featured":false,"poocommerceOnSale":false,"poocommerceStockStatus":["instock","outofstock","onbackorder"],"poocommerceAttributes":[],"poocommerceHandPickedProducts":[],"filterable":false,"relatedBy":{"categories":true,"tags":true}},"tagName":"div","displayLayout":{"type":"flex","columns":3,"shrinkColumns":true},"dimensions":{"widthType":"fill"},"collection":"poocommerce/product-collection/cross-sells","hideControls":["filterable"],"queryContextIncludes":["collection"]} -->
<div class="wp-block-poocommerce-product-collection"><!-- wp:pattern {"slug":"poocommerce/cart-cross-sells-message"} /-->

<!-- wp:poocommerce/product-template -->
<!-- wp:poocommerce/product-image {"showSaleBadge":false,"imageSizing":"thumbnail"} -->
<!-- wp:poocommerce/product-sale-badge {"align":"right"} /-->
<!-- /wp:poocommerce/product-image -->

<!-- wp:post-title {"textAlign":"center","isLink":true,"style":{"spacing":{"margin":{"bottom":"0.75rem","top":"0"}},"typography":{"lineHeight":"1.4"}},"fontSize":"medium","__poocommerceNamespace":"poocommerce/product-collection/product-title"} /-->

<!-- wp:poocommerce/product-price {"textAlign":"center","fontSize":"small"} /-->

<!-- wp:poocommerce/product-button {"textAlign":"center","fontSize":"small"} /-->
...
<div class="wp-block-poocommerce-empty-cart-block"><!-- wp:pattern {"slug":"poocommerce/cart-empty-message"} /-->
