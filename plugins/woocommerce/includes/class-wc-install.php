...
class WC_Install {
	...
	public static function init() {
		...
		add_action( 'poocommerce_newly_installed', array( __CLASS__, 'enable_email_improvements' ), 20 );
		add_action( 'poocommerce_newly_installed', array( __CLASS__, 'enable_new_payments_settings_page' ), 20 );
		add_action( 'poocommerce_updated', array( __CLASS__, 'maybe_enable_new_payments_settings_page' ), 20 );
	...
	public static function maybe_enable_new_payments_settings_page() {
		$option_name = 'poocommerce_feature_reactify-classic-payments-settings_enabled';
		...
		// If the PooCommerce installed version is 9.7+, we enable it.
		if ( version_compare( $wc_initial_installed_version, '9.7.0', '>=' ) ) {
			update_option( $option_name, 'yes' );
		} else {
			// For stores created pre-9.7, we check 9.7 experiment group first.
			$experiment_transient = get_transient( 'abtest_variation_poocommerce_payment_settings_2025_v1' );
			if ( 'treatment' === $experiment_transient ) {
				...
				update_option( $option_name, 'yes' );
				delete_transient( 'abtest_variation_poocommerce_payment_settings_2025_v1' );
			}
		}
	}
