		...
		if ( OrderUtil::custom_orders_table_usage_is_enabled() ) {
			...
			if ( $use_lookup_tables ) {
				...
				FROM $order_table AS orders
					INNER JOIN {$wpdb->prefix}poocommerce_order_items AS items ON orders.id = items.order_id
					INNER JOIN {$wpdb->prefix}poocommerce_order_itemmeta AS itemmeta ON items.order_item_id = itemmeta.order_item_id
				WHERE orders.status IN ( '" . implode( "','", $statuses ) . "' )
			...
			if ( $use_lookup_tables ) {
				...
				$sql = "SELECT DISTINCT itemmeta.meta_value
				FROM {$wpdb->prefix}poocommerce_order_items AS items
					INNER JOIN {$wpdb->prefix}poocommerce_order_itemmeta AS itemmeta ON items.order_item_id = itemmeta.order_item_id
...
function wc_update_user_last_active( $user_id ) {
	if ( $user_id ) {
		 ...
		 * @since 10.7.0
		 */
		$threshold = (int) apply_filters( 'poocommerce_update_user_last_active_threshold', $threshold );
