...
class WC_Download_Handler {
	...
	public static function download( $file_path, $product_id ) {
		 ...
		 * @param integer $product_id Product ID of the product being downloaded.
		 */
		$filename = apply_filters( 'poocommerce_file_download_filename', self::derive_filename_from_file_path( $file_path ), $product_id );
	...
	public static function download_file_force( $file_path, $filename ) {
		...
		if ( $parsed_file_path['remote_file'] ) {
			...
			if ( false !== $handle ) {
				...
				$filename         = self::resolve_filename_from_response_headers(
					...
					$filename,
					// A filename customized via the `poocommerce_file_download_filename` filter (or by a
		...
		if ( self::READ_RESULT_SUCCESS !== $read_result ) {
			if ( self::READ_RESULT_FAILURE_AFTER_OUTPUT === $read_result ) {
				wc_get_logger()->warning(
					__( 'A file could not be completely served using the Force Download method because the response had already started.', 'poocommerce' )
				);
			} elseif ( $parsed_file_path['remote_file'] && 'yes' === get_option( 'poocommerce_downloads_redirect_fallback_allowed' ) ) {
	 ...
	 *                                 `poocommerce_file_download_filename` filter callback may produce any type.
	 ...
	 *                                 `poocommerce_file_download_filename` filter) and only completed with an
	...
	public static function resolve_filename_from_response_headers( array $response_headers, $filename, bool $preserve_filename = false ): string {
		if ( ! is_string( $filename ) ) {
			...
			// `instanceof Stringable` cannot be used here: it is PHP 8.0+, and PooCommerce supports PHP 7.4.
			...
			wc_doing_it_wrong(
				__METHOD__,
				sprintf(
					'The poocommerce_file_download_filename filter should return a string; %s returned.',
					$original_type
				),
				'11.1.0'
			);
