...
use Automattic\PooCommerce\Internal\VariationGallery\Telemetry as VariationGalleryTelemetry;
...
function wc_update_230_options() {
	...
	if ( DefaultCustomerAddress::NO_DEFAULT === get_option( 'poocommerce_default_customer_address', false ) && wc_prices_include_tax() ) {
...
function wc_update_450_sanitize_coupons_code() {
	...
	if ( $codes_changed ) {
		update_option( 'poocommerce_update_450_codes_changed', 'yes' );
	}
	 ...
	 */
	if ( 'yes' === get_option( 'poocommerce_update_450_codes_changed' ) ) {
		delete_option( 'poocommerce_update_450_codes_changed' );
...
function wc_update_870_prevent_listing_of_transient_files_directory() {
	...
	try {
		...
		error_log( 'PooCommerce: wc_update_870 could not write transient files directory protection files: ' . $exception->getMessage() ); // phpcs:ignore WordPress.PHP.DevelopmentFunctions.error_log_error_log
	}
}
...
/**
 * Update brand permalink setting to take into account obsolete 'poocommerce_prepend_shop_page_to_urls' option, removed in WC 2.0.3.
...
function wc_update_1050_migrate_brand_permalink_setting() {
	if ( 'yes' !== get_option( 'poocommerce_prepend_shop_page_to_urls' ) ) {
		return;
	}
	...
	$slug = trailingslashit( $shop_slug ) . __( 'brand', 'poocommerce' );
	update_option( 'poocommerce_brand_permalink', $slug );
}

/**
 * Autoload frequently used options for performance improvements (see https://github.com/poocommerce/poocommerce/issues/61855)
 *
...
function wc_update_1050_enable_autoload_options() {
	...
	$autoload_options = array(
		...
		'poocommerce_enable_delayed_account_creation',
		'wc_feature_poocommerce_brands_enabled',
		'wc_connect_taxes_enabled',
		'poocommerce_logs_logging_enabled',
		'poocommerce_email_improvements_existing_store_enabled',
		'poocommerce_custom_orders_table_data_sync_enabled',
	);

	$feature_options = array(
		'fulfillments'         => 'poocommerce_feature_fulfillments_enabled',
		'push_notifications'   => 'poocommerce_feature_push_notifications_enabled',
		'cart_checkout_blocks' => 'poocommerce_feature_cart_checkout_blocks_enabled',
	);
...
function wc_update_1050_remove_deprecated_marketplace_option(): void {
	delete_option( 'poocommerce_feature_marketplace_enabled' );
}
 ...
 *
 * In 10.5.0, `poocommerce_analytics_immediate_import` was renamed to
 * `poocommerce_analytics_scheduled_import` with inverted semantics, but no
...
function wc_update_1080_migrate_analytics_import_option(): void {
	$legacy_option = 'poocommerce_analytics_immediate_import';
	$new_option    = 'poocommerce_analytics_scheduled_import';
 ...
 * meta_key_value index to include only the meta_key column (see https://github.com/poocommerce/poocommerce/pull/63897),
 ...
 * future patch-release migrations. PooCommerce 10.8.0 beta 2 had a migration labeled as 10.8.0-1 already,
 ...
 *              `poocommerce_db_version` fence in `$db_updates`.
 */
...
/**
 * Remove the option poocommerce_task_list_reminder_bar_hidden.
...
function wc_update_1090_remove_task_list_reminder_bar_hidden_option() {
	delete_option( 'poocommerce_task_list_reminder_bar_hidden' );
}
...
function wc_update_10902_remove_deprecated_push_notifications_option(): void {
	delete_option( 'poocommerce_feature_push_notifications_enabled' );
}

/**
 * Set the stored value of the point_of_sale feature flag to enabled.
 *
 * The feature is deprecated as of 11.0.0 and always enabled in core, but the PooCommerce
...
function wc_update_1100_enable_point_of_sale_feature() {
	update_option( 'poocommerce_feature_point_of_sale_enabled', 'yes' );
}
...
function wc_update_1110_flush_product_count_cache() {
	if ( class_exists( \Automattic\PooCommerce\Caches\ProductCountCache::class ) ) {
		( new \Automattic\PooCommerce\Caches\ProductCountCache() )->flush( 'product' );
	}
}
...
function wc_update_1120_remove_abandoned_cart_recovery() {
	...
	if ( function_exists( 'as_unschedule_all_actions' ) ) {
		as_unschedule_all_actions( 'poocommerce_send_abandoned_cart_recovery_notification' );
	}
	...
	delete_option( 'poocommerce_abandoned_cart_recovery_recommendations_hidden' );
...
function wc_update_1120_cleanup_inherited_variation_images() {
	...
	$state_option     = 'poocommerce_update_1120_cleanup_state';
	$completed_option = 'poocommerce_update_1120_completed_at';
	$batch_size       = 250;
	...
	if ( version_compare( (string) get_option( 'poocommerce_db_version' ), '10.9.0', '<' ) ) {
		return false;
	}
...
function wc_update_11201_invalidate_analytics_reports_cache() {
	if ( class_exists( \Automattic\PooCommerce\Admin\API\Reports\Cache::class ) ) {
		\Automattic\PooCommerce\Admin\API\Reports\Cache::invalidate();
	}
}
...
function wc_update_11202_reset_refund_returning_customer_markers() {
	...
	$last_id_option    = 'poocommerce_update_11202_last_refund_order_id';
...
function wc_update_11203_normalize_stock_notification_emails() {
	...
	$last_id_option = 'poocommerce_update_11203_last_stock_notification_id';
...
function wc_update_1130_repair_hpos_order_dates_from_posts() {
	...
	$orders_table = \Automattic\PooCommerce\Internal\DataStores\Orders\OrdersTableDataStore::get_orders_table_name();
	if ( $orders_table !== $wpdb->get_var( $wpdb->prepare( 'SHOW TABLES LIKE %s', $orders_table ) ) ) {
		return false;
	}

	$last_id_option = 'poocommerce_update_1130_last_repaired_order_id';
	$batch_size     = 500;
	$zero           = '0000-00-00 00:00:00';
	$type_list      = array();
	$post_types     = array_merge( wc_get_order_types( 'cot-migration' ), array( \Automattic\PooCommerce\Internal\DataStores\Orders\DataSynchronizer::PLACEHOLDER_ORDER_POST_TYPE ) );
	...
	$forget_and_import = function ( array $order_ids ) {
		...
		wc_get_container()->get( \Automattic\PooCommerce\Internal\DataStores\Orders\OrdersTableDataStore::class )->clear_cached_data( $order_ids );
		$order_cache = wc_get_container()->get( \Automattic\PooCommerce\Caches\OrderCache::class );
		// With Analytics disabled nothing handles the import action, and queueing it would only leave failed actions behind.
		$import_hook    = \Automattic\PooCommerce\Internal\Admin\Schedulers\OrdersScheduler::get_action( 'import' );
		 ...
		 */
		$import_inline = ! get_option( 'schema-ActionScheduler_StoreSchema' ) || apply_filters( 'poocommerce_analytics_disable_action_scheduling', false );
		foreach ( $order_ids as $order_id ) {
			...
			if ( $import_inline ) {
				\Automattic\PooCommerce\Internal\Admin\Schedulers\OrdersScheduler::import( $order_id );
				continue;
			}
			...
			WC()->queue()->schedule_single( time() + 5, $import_hook, array( $order_id ), (string) \Automattic\PooCommerce\Internal\Admin\Schedulers\OrdersScheduler::$group );
		}
	};
