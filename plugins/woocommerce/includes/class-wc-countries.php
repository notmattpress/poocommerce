...
class WC_Countries {
	...
	public function get_countries() {
		...
		if ( empty( $this->geo_cache['countries'][ $locale ] ) ) {
			...
			try {
				 ...
				 */
				$this->geo_cache['countries'][ $locale ] = apply_filters( 'poocommerce_countries', include WC()->plugin_path() . '/i18n/countries.php' );
				/**
	...
	public function get_continents() {
		...
		if ( empty( $this->geo_cache['continents'][ $locale ] ) ) {
			...
			try {
				 ...
				 */
				$this->geo_cache['continents'][ $locale ] = apply_filters( 'poocommerce_continents', include WC()->plugin_path() . '/i18n/continents.php' );
			} finally {
	...
	public function load_country_states() {
		 ...
		 * @param array $states
		 */
		$this->geo_cache['states'][ $locale ] = apply_filters( 'poocommerce_states', $states );
	}
	...
	public function get_states( $cc = null ) {
		...
		if ( ! isset( $this->geo_cache['states'][ $locale ] ) ) {
			...
			try {
				 ...
				 * @param array $states
				 */
				$this->geo_cache['states'][ $locale ] = apply_filters( 'poocommerce_states', include WC()->plugin_path() . '/i18n/states.php' );
			} finally {
	...
	public function get_country_locale() {
		...
		if ( empty( $this->locale ) || ( null !== $this->country_locale_built_for && $cache_locale !== $this->country_locale_built_for ) ) {
			...
			try {
				 ...
				 */
				$locale['default'] = apply_filters( 'poocommerce_get_country_locale_default', $this->get_default_address_fields() );
				 ...
				 */
				$locale['default'] = apply_filters( 'poocommerce_get_country_locale_base', $locale['default'] );
				/**
				 * Filters the default and shop base country address field settings, which apply when the checkout country changes.
				 *
				 * @since 1.5.4
				 ...
				 * @param array $fields Address field settings.
				 */
				$locale[ $this->get_base_country() ] = apply_filters( 'poocommerce_get_country_locale_base', $locale[ $this->get_base_country() ] );
	...
	public function get_address_fields( $country = '', $type = 'billing_' ) {
		...
		if ( isset( $locale[ $country ] ) ) {
			// wc_array_overlay() only copies keys that exist in the base array, so seed 'hidden' where the locale defines it.
			foreach ( $locale[ $country ] as $field_key => $locale_field ) {
				if ( isset( $fields[ $field_key ], $locale_field['hidden'] ) && is_array( $fields[ $field_key ] ) ) {
