...
/**
 * Add an index to the poocommerce_downloadable_product_permissions table for (user_email) to improve anonymization/deletion performance.
 */
function wc_update_1050_add_idx_user_email() {
	global $wpdb;
	$index_exists = $wpdb->get_row( "SHOW INDEX FROM {$wpdb->prefix}poocommerce_downloadable_product_permissions WHERE key_name = 'idx_user_email'" );

	if ( is_null( $index_exists ) ) {
		$wpdb->query( "ALTER TABLE {$wpdb->prefix}poocommerce_downloadable_product_permissions ADD INDEX idx_user_email (user_email(100))" );
	}
}
 ...
 * "Rebuild analytics tax data" tool on PooCommerce > Status > Tools. Until it has been through an
