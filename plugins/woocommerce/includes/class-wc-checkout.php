...
class WC_Checkout {
		...
		try {
			...
			if ( WC()->cart->get_cart_contents_count() > 0 ) {
				...
				if ( ! $persisted_order || 0 === count( $persisted_order->get_items() ) ) {
					throw new Exception( __( 'Order items could not be saved. Please try again.', 'poocommerce' ) );
				}
			}
	 ...
	 * including those registered through the 'poocommerce_checkout_fields' filter, is validated without
				...
				if ( WC()->countries->country_exists( $shipping_country ) ) {
					...
					$errors->add(
						...
						sprintf(
							...
							__( 'Unfortunately, <strong>we do not ship %1$s %2$s</strong>. Please enter an alternative shipping address.', 'poocommerce' ),
	...
	public function process_checkout() {
		try {
			...
			if ( empty( $posted_data['poocommerce_checkout_update_totals'] ) && 0 === wc_notice_count( 'error' ) ) {
				 ...
				 * Use poocommerce_payment_complete or poocommerce_order_status_completed instead.
				 *
				 * @since 1.1.0
				 *
				 * @param int                      $order_id    Order ID.
				...
				wc_log_order_step(
					'[Shortcode #5] poocommerce_checkout_order_processed hook ran successfully',
					array( 'order_object' => $order )
				);
