...
class WC_Shortcode_Checkout {
	...
	private static function checkout() {
		...
		if ( empty( $_POST ) && wc_notice_count( 'error' ) > 0 ) { // phpcs:ignore WordPress.Security.NonceVerification.Missing -- Read-only flow selectors are cleaned; order access verifies key/ownership.
			...
			$non_js_checkout = ! empty( $_POST['poocommerce_checkout_update_totals'] ); // phpcs:ignore WordPress.Security.NonceVerification.Missing -- Read-only flow selectors are cleaned; order access verifies key/ownership.
