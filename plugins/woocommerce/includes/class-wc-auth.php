	...
	protected function make_validation() {
		...
		foreach ( $params as $param ) {
			...
			if ( ! is_string( $_REQUEST[ $param ] ) ) { // phpcs:ignore WordPress.Security.NonceVerification.Recommended -- Values are required for handshake validation; key creation verifies capability and nonce.
				/* translators: %s: parameter */
				throw new Exception( sprintf( __( 'Invalid parameter %s', 'poocommerce' ), $param ) ); // phpcs:ignore WordPress.Security.EscapeOutput.ExceptionNotEscaped -- The caught message is escaped by esc_html() where it is rendered; escaping here would double-encode translated text.
			}
