			...
			if ( $log ) {
				WP_CLI::log(
					sprintf(
						...
						'https://developer.poocommerce.com/docs/features/high-performance-order-storage/recipe-book/'
					)
				);
			}
		}
		...
		do {
			...
			foreach ( $order_ids as $order_id ) {
				try {
					...
					WP_CLI::debug( sprintf( __( 'Cleanup completed for order %d.', 'poocommerce' ), $order_id ), 'wc' );
				} catch ( \Exception $e ) {
