	...
	public static function menu_task_count() {
		...
		foreach ( $submenu['poocommerce'] as $key => $menu_item ) {
			if ( 0 === strpos( $menu_item[0], _x( 'Home', 'Admin menu name', 'poocommerce' ) ) ) {
				$submenu['poocommerce'][ $key ][0] .= ' <span class="menu-counter remaining-tasks-badge poocommerce-task-list-remaining-tasks-badge"><span class="count-' . esc_attr( $tasks_count ) . '">' . absint( $tasks_count ) . '</span></span>'; // phpcs:ignore WordPress.WP.GlobalVariablesOverride.Prohibited
				break;
			}
		}
	}
