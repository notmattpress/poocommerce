...
class ExportWCSettingsShipping extends ExportWCSettings {
	...
	private function get_steps_for_zones(): array {
		...
		return array_map(
			fn( $zone ) => RunSql::from_table_row( $zone, 'poocommerce_shipping_zones' ),
	...
	private function get_steps_for_locations(): array {
		...
		return array_map(
			fn( $location ) => RunSql::from_table_row( $location, 'poocommerce_shipping_zone_locations' ),
	...
	private function get_steps_for_methods_and_options(): array {
		...
		return array_merge(
			array_map(
				fn( $method ) => RunSql::from_table_row( $method, 'poocommerce_shipping_zone_methods' ),
				$methods
			),
			array_map(
