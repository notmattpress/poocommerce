...
class Controller extends GenericController implements ExportableInterface {
	...
	public function get_item_schema() {
		$schema = array(
			...
			'properties' => array(
				...
				'role'                 => array(
					'description' => __( 'Role(s) of the user, comma-separated. Empty for guest customers.', 'poocommerce' ),
					'type'        => 'string',
					'context'     => array( 'view', 'edit' ),
					'readonly'    => true,
				),
				...
				'billing_phone'        => array(
					'description' => __( 'Billing phone.', 'poocommerce' ),
					'type'        => 'string',
					'context'     => array( 'view', 'edit' ),
					'readonly'    => true,
				),
				'shipping_phone'       => array(
					'description' => __( 'Shipping phone.', 'poocommerce' ),
					'type'        => 'string',
					'context'     => array( 'view', 'edit' ),
					'readonly'    => true,
				),
	...
	public function get_collection_params() {
		...
		$params['customers_exclude']       = array(
			'description'       => __( 'Limit result to exclude items with specified customer ids.', 'poocommerce' ),
			'type'              => 'array',
			'sanitize_callback' => 'wp_parse_id_list',
			'validate_callback' => 'rest_validate_request_arg',
			'items'             => array(
		...
		$params['user_type']               = array(
			'description'       => __( 'Limit result to items with specified user type.', 'poocommerce' ),
			'type'              => 'string',
			'default'           => 'all',
			'validate_callback' => 'rest_validate_request_arg',
			'enum'              => array(
		...
		$params['location_includes']       = array(
			'description'       => __( 'Includes customers by location (state, country). Provide a comma-separated list of locations. Each location can be a country code (e.g. GB) or combination of country and state (e.g. US:CA).', 'poocommerce' ),
			'type'              => 'string',
			'validate_callback' => 'rest_validate_request_arg',
		);
		$params['location_excludes']       = array(
			'description'       => __( 'Excludes customers by location (state, country). Provide a comma-separated list of locations. Each location can be a country code (e.g. GB) or combination of country and state (e.g. US:CA).', 'poocommerce' ),
			'type'              => 'string',
			'validate_callback' => 'rest_validate_request_arg',
		);
		return $params;
	...
	public function get_export_columns() {
		...
		$export_columns = array(
			...
			'billing_phone'   => __( 'Billing Phone', 'poocommerce' ),
			'shipping_phone'  => __( 'Shipping Phone', 'poocommerce' ),
			'role'            => __( 'Role', 'poocommerce' ),
		);
