...
use Automattic\PooCommerce\Admin\Overrides\OrderRefund;
...
class DataStore extends ReportsDataStore implements DataStoreInterface {
	 ...
	 * @var string
	 */
	const OPTION_ORDER_STATS_TABLE_HAS_COLUMN_ORDER_FULFILLMENT_STATUS = 'poocommerce_order_stats_has_fulfillment_column';
	...
	public static function init() {
		...
		add_action( 'poocommerce_delete_order_refund', array( __CLASS__, 'delete_refund' ) );
	}
	...
	public static function update( $order ) {
		 ...
		 * @since 4.0.0
		 */
		$data = apply_filters( 'poocommerce_analytics_update_order_stats_data', $data, $order );
	...
	public static function delete_refund( $refund_id ): void {
		 ...
		 * @since 4.0.0
		 */
		do_action( 'poocommerce_analytics_delete_order_stats', $refund_id, absint( $customer_id ) );
	...
	public static function add_fulfillment_status_column() {
		...
		if ( false === $result ) {
			return $wpdb->last_error ? $wpdb->last_error : __( 'Unknown database error occurred while adding fulfillment_status column.', 'poocommerce' );
		}
