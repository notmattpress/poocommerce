...
class DataStore extends ReportsDataStore implements DataStoreInterface {
	 ...
	 * column that appears or disappears behind PooCommerce's back (a partial restore, a
	...
	protected function add_from_sql_params( $query_args, $order_status_filter ) {
		...
		$this->subquery->add_sql_clause( 'join', "JOIN {$wpdb->prefix}poocommerce_order_itemmeta itemmeta_rate_id ON itemmeta_rate_id.order_item_id = {$wpdb->prefix}poocommerce_order_items.order_item_id AND itemmeta_rate_id.meta_key = 'rate_id'" );
		$this->subquery->add_sql_clause( 'join', "JOIN {$wpdb->prefix}poocommerce_order_itemmeta itemmeta_rate_percent ON itemmeta_rate_percent.order_item_id = {$wpdb->prefix}poocommerce_order_items.order_item_id AND itemmeta_rate_percent.meta_key = 'rate_percent'" );
	}
	...
	protected function add_sql_query_params( $query_args ) {
		 ...
		 * background.
		 */
		$this->subquery->add_sql_clause( 'where', "AND ( {$order_tax_lookup_table}.order_item_id = {$wpdb->prefix}poocommerce_order_items.order_item_id OR " . self::get_legacy_row_condition() . ' )' );
	 ...
	 * left out.
	 *
	 * A store ends up holding both shapes when code outside PooCommerce writes the lookup on the
	 * (order_id, tax_rate_id) key the table was released with: such a write lands on the zero
	 * default instead of replacing the rows PooCommerce holds for the order. A prune that could
	...
	protected function initialize_queries() {
		...
		$this->subquery->add_sql_clause( 'group_by', ", {$wpdb->prefix}poocommerce_order_items.order_item_name, itemmeta_rate_percent.meta_value" );
	}
}
