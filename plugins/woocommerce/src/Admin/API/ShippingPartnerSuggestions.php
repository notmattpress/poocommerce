...
class ShippingPartnerSuggestions extends \WC_REST_Data_Controller {
	...
	public function get_permission_check( $request ) {
		if ( ! current_user_can( 'install_plugins' ) ) {
			return new \WP_Error( 'poocommerce_rest_cannot_update', __( 'You do not have permissions to manage plugins. Please contact your site administrator.', 'poocommerce' ), array( 'status' => rest_authorization_required_code() ) );
		}
		return true;
	}
		...
		$item_schema = array(
			...
			'properties' => array(
				...
				'countries_where_primary' => array(
					'description' => __( 'Countries where this partner should appear first.', 'poocommerce' ),
					'type'        => 'array',
					'items'       => array(
						'type' => 'string',
					),
