	...
	public static function install_plugins( $plugins, ?PluginsInstallLogger $logger = null, ?string $source = null ) {
		...
		foreach ( $plugins as $plugin ) {
			...
			if ( is_wp_error( $api ) ) {
				...
				$error_message = sprintf(
				/* translators: %s: plugin slug (example: poocommerce-services) */
					__( 'We couldn\'t install `%s`. Try again in a few minutes, or install it later from the Extensions page.', 'poocommerce' ),
					$slug
				);
			...
			if ( is_wp_error( $result ) || is_null( $result ) ) {
				...
				/* translators: %s: plugin slug (example: poocommerce-services) */
					__( 'We couldn\'t install `%s`. Try again, or install it manually. If it keeps failing, contact your host.', 'poocommerce' ),
					$slug
				);
				$errors->add(
					$plugin,
