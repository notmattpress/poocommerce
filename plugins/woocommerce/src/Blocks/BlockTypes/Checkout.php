...
class Checkout extends AbstractBlock {
	...
	protected function enqueue_data( array $attributes = [] ) {
		 ...
		 * Additional fields names can be added via the 'poocommerce_essential_shipping_data' filter.
		 ...
		 * @return array
		 */
		$filtered_fields = apply_filters( 'poocommerce_address_fields_for_shipping_rates', [] );
		...
		if ( is_array( $filtered_fields ) ) {
				...
				if ( ! is_string( $value ) ) {
					wc_get_logger()->warning(
						sprintf(
							...
							__( 'Address fields for shipping rates values must be strings. Non-string value removed: %1$s at index %2$s', 'poocommerce' ),
							gettype( $value ),
							$key
						),
						array( 'source' => 'poocommerce_address_fields_for_shipping_rates' )
					);
			...
			wc_get_logger()->warning(
				__( 'Address fields for shipping rates must be an array of strings.', 'poocommerce' ),
				array( 'source' => 'poocommerce_address_fields_for_shipping_rates' )
			);
		}
