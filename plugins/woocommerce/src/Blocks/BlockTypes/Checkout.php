...
class Checkout extends AbstractBlock {
	...
	protected function render( $attributes, $content, $block ) {
		...
		if ( $this->is_checkout_endpoint() ) {
			...
			return wp_is_block_theme() ? do_shortcode( '[poocommerce_checkout]' ) : '[poocommerce_checkout]';
		}
	...
	private function find_local_pickup_text_in_checkout_block( $post_content ) {
		...
		while ( $scanner->next_delimiter() ) {
			if ( $scanner->opens_block( 'poocommerce/checkout-shipping-method-block' ) ) {
	...
	protected function enqueue_data( array $attributes = [] ) {
		...
		$providers_payload = [];
		if ( class_exists( AddressProviderController::class ) && 'no' !== get_option( 'poocommerce_address_autocomplete_enabled', 'no' ) ) {
		...
		wp_prime_option_caches(
			array(
				...
				'poocommerce_tax_display_cart', // This one is autoloaded, but we add it here for clarity.
		...
		$this->asset_data_registry->add( 'displayCartPricesIncludingTax', TaxDisplayMode::INCLUSIVE === get_option( 'poocommerce_tax_display_cart' ) );
		...
		if ( ( $is_block_editor || current_user_can( 'manage_poocommerce' ) ) && ! $this->asset_data_registry->exists( 'incompatibleExtensions' ) ) {
