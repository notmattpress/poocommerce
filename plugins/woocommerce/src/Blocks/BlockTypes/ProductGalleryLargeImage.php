...
class ProductGalleryLargeImage extends AbstractBlock {
	...
	private function get_main_media_html( $context, $product, $inner_block ) {
		$media_data = ProductGalleryUtils::get_product_gallery_media_data( $product, 'poocommerce_single' );
	...
	private function replace_product_image_with_video( $image_html, $media, $context ) {
		...
		$video_classes = 'wc-block-poocommerce-product-gallery-large-image__image ' .
			'wc-block-poocommerce-product-gallery-large-image__video';

		if ( ! empty( $context['fullScreenOnClick'] ) ) {
			$video_classes             .= ' wc-block-poocommerce-product-gallery-large-image__image--full-screen-on-click';
