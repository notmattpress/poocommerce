...
final class ProductFilterCheckboxList extends AbstractBlock {
	...
	protected function render( $attributes, $content, $block ) {
		...
		$wrapper_attributes = array(
			'data-wc-interactive' => esc_attr( wp_json_encode( array( 'namespace' => 'poocommerce/product-filter-checkbox-list' ), JSON_HEX_TAG | JSON_HEX_APOS | JSON_HEX_QUOT | JSON_HEX_AMP ) ),
		...
		<div <?php echo get_block_wrapper_attributes( $wrapper_attributes ); // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped ?>>
			<ul class="wc-block-product-filter-checkbox-list__list" aria-label="<?php echo esc_attr__( 'Filter Options', 'poocommerce' ); ?>">
				<?php foreach ( $items as $item ) { ?>
					...
					$aria_label = sprintf( __( 'Filter item: %s', 'poocommerce' ), $item['ariaLabel'] ?? $item['label'] );
					?>
					<li
