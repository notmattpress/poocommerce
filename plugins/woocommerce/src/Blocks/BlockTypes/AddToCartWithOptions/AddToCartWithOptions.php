...
use Automattic\PooCommerce\Blocks\Utils\Utils as BlocksUtils;
...
class AddToCartWithOptions extends AbstractBlock {
	...
	protected function get_template_part_path( $product_type ) {
		 ...
		 * @param string $product_type The product type
		 */
		return apply_filters( '__experimental_poocommerce_' . $product_type . '_add_to_cart_with_options_block_template_part', false, $product_type );
	}
		...
		if ( isset( $product_id ) ) {
			...
			if ( $rendered_product instanceof \WC_Product ) {
				...
				if ( is_string( $template_part_path ) && '' !== $template_part_path && file_exists( $template_part_path ) ) {
					wp_enqueue_script_module( 'poocommerce/add-to-cart-with-options' );
				}
			}
		}
		...
		if ( is_string( $template_part_path ) && '' !== $template_part_path && file_exists( $template_part_path ) ) {
			...
			*/
			...
			if ( $this->is_wishlist_enabled() ) {
				$template_part_contents .= "\n<!-- wp:poocommerce/add-to-wishlist-button /-->";
			}
			...
			if ( $legacy_mode ) {
				...
				$form_attributes = array(
					'action'  => esc_url(
						 ...
						 */
						apply_filters( 'poocommerce_add_to_cart_form_action', $action_url )
					),
					'method'  => 'post',
			...
			if ( ProductType::SIMPLE === $product_type ) {
				...
				$hidden_input = '<div class="single_variation_wrap">
					...
					<input type="hidden"
						name="variation_id"
						data-wp-bind--value="poocommerce/products::state.productVariationInContext.id"
					/>
				</div>';
			}
			...
			if ( $has_visible_quantity_input ) {
				...
				$wrapper_attributes['data-wp-interactive'] = 'poocommerce/add-to-cart-form';
				wp_enqueue_script_module( 'poocommerce/add-to-cart-form' );
			}
	...
	protected function render_interactivity_notices_region( $form_html ) {
		...
		ob_start();
		?>
		<div data-wp-interactive="poocommerce/store-notices" class="wc-block-components-notices alignwide" <?php echo $context_directive; // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped ?>>
