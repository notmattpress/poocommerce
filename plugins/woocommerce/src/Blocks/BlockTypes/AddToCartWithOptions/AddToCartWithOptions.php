			...
			if ( $product->is_type( 'grouped' ) ) {
				...
				foreach ( $context['groupedProductIds'] as $child_product_id ) {
					...
					if ( $child_product ) {
						 ...
						 */
						$default_child_quantity                   = apply_filters( 'poocommerce_quantity_input_min', $child_product->get_min_purchase_quantity(), $child_product );
			...
			*/
			...
			if ( $legacy_mode ) {
				...
				if ( ProductType::SIMPLE === $product_type ) {
					...
					$hidden_input .= '<input type="hidden" name="variation_id" data-wp-interactive="poocommerce/add-to-cart-with-options" data-wp-bind--value="state.variationId" />';
				}
			} else {
