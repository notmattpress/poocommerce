...
class MiniCart extends AbstractBlock {
	...
	private function render_template_part_contents( $template_contents ) {
		$process_legacy_wrappers = function ( $parsed_block ) {
			if ( 'poocommerce/mini-cart-contents' !== ( $parsed_block['blockName'] ?? null ) ) {
				return $parsed_block;
			}
	...
	public function register_empty_cart_message_block_pattern() {
		register_block_pattern(
			...
			array(
				...
				'inserter' => false,
				'content'  => '<!-- wp:paragraph {"align":"center"} --><p class="has-text-align-center"><strong>' . __( 'Your shopping cart is empty', 'poocommerce' ) . '</strong></p><!-- /wp:paragraph -->',
			)
		);
	}
