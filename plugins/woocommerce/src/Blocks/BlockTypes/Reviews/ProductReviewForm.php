...
class ProductReviewForm extends AbstractBlock {
	...
	protected function render( $attributes, $content, $block ) {
		...
		if ( wc_review_ratings_enabled() ) {
			...
			$interactivy_config['i18nRequiredRatingText'] = esc_attr__( 'Please select a rating', 'poocommerce' );
		...
		if ( ! empty( $interactivy_state ) ) {
			wp_interactivity_state( 'poocommerce/product-reviews', $interactivy_state );
		}
		if ( ! empty( $interactivy_config ) ) {
			wp_interactivity_config( 'poocommerce/product-reviews', $interactivy_config );
		}
