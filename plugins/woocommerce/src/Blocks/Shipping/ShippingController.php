...
class ShippingController {
	...
	public function init() {
		...
		add_filter( 'poocommerce_order_get_tax_location', array( $this, 'filter_order_tax_location' ), 10, 2 );
	...
	public function remove_shipping_settings( $settings ) {
		if ( CartCheckoutUtils::is_cart_block_default() ) {
			foreach ( $settings as $index => $setting ) {
				if ( 'poocommerce_enable_shipping_calc' === $setting['id'] ) {
					$settings[ $index ]['desc_tip'] = sprintf(
					/* translators: %s: URL to the documentation. */
						__( 'This feature is not available when using the <a href="%s">Cart and checkout blocks</a>. Shipping will be calculated at checkout.', 'poocommerce' ),
						'https://poocommerce.com/document/poocommerce-store-editing/customizing-cart-and-checkout/'
					);
	...
	public function register_settings() {
		...
		// Save route guarded by manage_poocommerce (via wc_rest_check_manager_permissions)
	...
	public function filter_order_tax_location( $location, $order ) {
		...
		// phpcs:ignore PooCommerce.Commenting.CommentHooks.MissingHookComment
		if ( true !== apply_filters( 'poocommerce_apply_base_tax_for_local_pickup', true ) ) {
			return $location;
		}
		...
		$local_pickup_method_ids = apply_filters( 'poocommerce_local_pickup_methods', array( 'legacy_local_pickup', 'local_pickup' ) );
