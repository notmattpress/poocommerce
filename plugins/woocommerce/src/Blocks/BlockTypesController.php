...
final class BlockTypesController {
	...
	/**
	 * Whether PooCommerce block styles should be enqueued on demand for classic themes.
	 *
	 * @var bool|null
	 */
	...
	public function register_block_patterns() {
		...
		register_block_pattern(
			...
			array(
				'title'    => '',
				'inserter' => false,
				'content'  => '<!-- wp:heading {"textAlign":"center","className":"wc-block-cart__empty-cart__title"} --><h2 class="wp-block-heading has-text-align-center wc-block-cart__empty-cart__title">' . esc_html__( 'Your cart is empty', 'poocommerce' ) . '</h2><!-- /wp:heading --><!-- wp:buttons {"layout":{"type":"flex","justifyContent":"center"}} --><div class="wp-block-buttons"><!-- wp:button --><div class="wp-block-button"><a class="wp-block-button__link wp-element-button" href="' . esc_attr( esc_url( $shop_permalink ) ) . '">' . esc_html__( 'Return to shop', 'poocommerce' ) . '</a></div><!-- /wp:button --></div><!-- /wp:buttons -->',
			)
		);
		register_block_pattern(
		...
		register_block_pattern(
			...
			array(
				'title'    => '',
				'inserter' => false,
				'content'  => '<!-- wp:heading {"textAlign":"left","style":{"spacing":{"margin":{"bottom":"1rem"}}}} --><h2 class="wp-block-heading has-text-align-left" style="margin-bottom:1rem">' . esc_html__( 'You may be interested in…', 'poocommerce' ) . '</h2><!-- /wp:heading -->',
			)
		);
	}
