...
class Bootstrap {
	...
	protected function init() {
		...
		add_filter( 'poocommerce_short_description', array( $this, 'maybe_register_blocks_from_content' ), 8 );
	...
	/**
	 * Register PooCommerce block types on demand when a description containing one is rendered.
	 *
	 * Eager block registration is skipped on non-rendering requests (Store API, REST, AJAX, webhooks), but
	 * product and variation descriptions still run through do_blocks there, so a PooCommerce block in a
	 * description would render empty. Hooked to poocommerce_short_description just before do_blocks.
	 *
	 ...
	 * contains a PooCommerce block; registering covers nested blocks at any depth.
	 *
	 * @since 11.1.0
	 *
	...
	public function maybe_register_blocks_from_content( $content ) {
		if ( is_string( $content ) && preg_match( '/<!--\s+wp:(poocommerce\/|block\s)/', $content ) ) {
		...
		add_action(
			...
			function () {
				...
				printf(
					...
					esc_html__( 'PooCommerce Blocks development mode requires files to be built. From the root directory, run %1$s to install dependencies, %2$s to build the files or %3$s to build the files and watch for changes.', 'poocommerce' ),
