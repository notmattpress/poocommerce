...
class BlockTemplateUtils {
	...
	/**
	 * Removes templates from the theme or PooCommerce which have the same slug
	 * as template saved in the database with the `poocommerce/poocommerce` theme.
	 * Before WC migrated to the Template Registration API from WordPress, templates
	 * were saved in the database with the `poocommerce/poocommerce` theme instead
	 * of the theme's slug.
	 *
	...
	public static function remove_duplicate_customized_templates( $templates ) {
		...
		return array_filter(
			...
			function ( $template ) use ( $theme_slug, $customized_theme_template_slugs ) {
				...
				// Customized from the PooCommerce default template: keep only if there is no customized theme template with same slug.
	...
	public static function get_block_templates_from_db( $slugs = array(), $template_type = 'wp_template' ) {
		...
		$ids   = wp_cache_get( $template_type . '-ids', 'poocommerce_blocks' );
		if ( ! isset( $ids[ $theme ] ) ) {
			...
			wp_cache_set( $template_type . '-ids', $ids, 'poocommerce_blocks', 12 * HOUR_IN_SECONDS );
