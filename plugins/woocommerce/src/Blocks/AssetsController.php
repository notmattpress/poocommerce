...
final class AssetsController {
	...
	private function register_deprecated_script_handles(): void {
		...
		if ( is_admin() ) {
			...
			foreach ( false === $asset_files ? array() : $asset_files as $asset_file ) {
				...
				// The file comes from PooCommerce's generated assets directory, not user input.
	...
	private function add_deprecated_script_handle_warnings( array $handles ): void {
		foreach ( $handles as $handle ) {
			$message = wp_json_encode(
				sprintf(
					'[PooCommerce] The "%s" script handle is a deprecated compatibility placeholder and does not load a script. See the enqueueable packages documentation: https://github.com/poocommerce/poocommerce/tree/trunk/plugins/poocommerce/client/blocks/docs/internal-developers/enqueueable-packages',
					$handle
				)
			);
	...
	private function get_block_asset_resource_hints_cache() {
		...
		$cache = get_transient( 'poocommerce_block_asset_resource_hints' );

		$current_version = array(
			'poocommerce' => Constants::get_constant( 'WC_VERSION' ),
	...
	private function set_block_asset_resource_hints_cache( $filename, $data ) {
		...
		set_transient( 'poocommerce_block_asset_resource_hints', $updated, WEEK_IN_SECONDS );
	}
