...
trait CheckoutTrait {
	...
	private function order_moved_past_payment( \WC_Order $order ): bool {
		 ...
		 * @param \WC_Order $order                Order object.
		 */
		$payable_statuses = apply_filters( 'poocommerce_valid_order_statuses_for_payment', array( OrderStatus::PENDING, OrderStatus::FAILED ), $order );
	...
	private function recover_order_that_took_payment( \WC_Order $order, \Throwable $error, PaymentResult $payment_result ): void {
		...
		$order->add_order_note(
			sprintf(
				...
				__( 'Checkout could not be completed after payment was taken: %s', 'poocommerce' ),
		 ...
		 * redirect_url: WooPayments and PooCommerce Stripe both call String.match() on it
	...
	private function update_order_from_request( \WP_REST_Request $request, bool $persist = true ) {
		...
		if ( null !== $payment_method ) {
			...
			if ( $order_needs_payment && 'POST' === $request->get_method() ) {
				throw new RouteException(
					...
					esc_html__( 'No payment method provided.', 'poocommerce' ),
					400
				);
			}
		 ...
		 * @param \WP_REST_Request $request Full details about the request.
		 */
		do_action( 'poocommerce_store_api_checkout_update_order_from_request', $order, $request );
