...
class CartController {
	...
	public function validate_add_to_cart( \WC_Product $product, $request ) {
		...
		if ( floatval( $request['quantity'] ) <= 0 ) {
			throw new RouteException(
				'poocommerce_rest_product_invalid_quantity',
				sprintf(
					/* translators: %s: product name */
					esc_html__( 'You cannot add &quot;%s&quot; with a quantity less than or equal to 0 to the cart.', 'poocommerce' ),
		...
		if ( ! $product->is_in_stock() ) {
			throw new RouteException(
				...
				sprintf(
					/* translators: %s: product name */
					esc_html__( 'You cannot add &quot;%s&quot; to the cart because the product is out of stock.', 'poocommerce' ),
					$product->get_name()
				),
				400
			);
			...
			if ( $qty_remaining < $qty_in_cart + $request_quantity ) {
				throw new RouteException(
					...
					sprintf(
						...
						esc_html__( 'You cannot add that amount of &quot;%1$s&quot; to the cart because there is not enough stock (%2$s remaining).', 'poocommerce' ),
						$product->get_name(),
	...
	private function get_error_message_for_stock_exception_type( $exception_type, $singular_or_plural ) {
		...
		return esc_html__( 'There was an error with an item in your cart.', 'poocommerce' );
	}
	...
	public function validate_cart() {
		 ...
		 * `poocommerce_store_api_cart_errors`, which passes a WP_Error to callbacks directly.
	...
	public function validate_cart_not_empty() {
		...
		if ( empty( $cart_items ) ) {
			throw new InvalidCartException(
				...
				new WP_Error( 'poocommerce_rest_cart_empty', esc_html__( 'Cannot place an order, your cart is empty.', 'poocommerce' ), 400 ),
				400
			);
		}
	}
		...
		if ( ! $coupon->is_valid() ) {
			...
			throw new RouteException(
				...
				sprintf(
					...
					esc_html__( 'The "%1$s" coupon has been removed from your cart: %2$s', 'poocommerce' ),
					$coupon->get_code(),
		...
		if ( ! $product || ProductStatus::TRASH === $product->get_status() ) {
			throw new RouteException(
				...
				sprintf(
					...
					esc_html__( 'Product with ID "%s" was not found and cannot be added to the cart.', 'poocommerce' ),
					esc_html( $request['id'] )
				),
				400
			);
	...
	protected function throw_default_product_exception( \WC_Product $product ) {
		...
		if ( empty( $product_name ) ) {
			$message = __( 'This item is not available for purchase.', 'poocommerce' );
		} else {
			$message = sprintf(
				/* translators: %s: product name */
	...
	protected function parse_variation_data( $request ) {
		...
		if ( ! empty( $missing_attributes ) ) {
			throw new RouteException(
				...
				esc_html__( 'Missing variation data for variable product.', 'poocommerce' ) . ' ' .
				esc_html(
					sprintf(
	...
	protected function get_variation_id_from_variation_data( $request, $product ) {
		...
		if ( empty( $variation_id ) ) {
			...
			if ( count( $selected_attributes ) < count( $required_attributes ) ) {
				throw new RouteException(
					'poocommerce_rest_missing_attributes',
					esc_html__( 'Missing attributes for variable product.', 'poocommerce' ),
					400
				);
			}
			throw new RouteException(
				'poocommerce_rest_variation_id_from_variation_data',
				esc_html__( 'No matching variation found.', 'poocommerce' ),
				400
			);
		}
	...
	protected function get_variable_product_attributes( $product ) {
		...
		if ( ! $product || ProductStatus::TRASH === $product->get_status() ) {
			throw new RouteException(
				...
				esc_html__( 'This product cannot be added to the cart.', 'poocommerce' ),
				400
			);
		}
