...
class Checkout extends AbstractCartRoute {
	...
	private function process_order( \WP_REST_Request $request ) { // phpcs:ignore Squiz.Commenting.FunctionComment.IncorrectTypeHint
		 ...
		 * call WC_Order::payment_complete(). Use poocommerce_payment_complete or
		 * poocommerce_order_status_completed instead.
		 *
	...
	private function validate_order_totals( \WP_REST_Request $request ): void {
		...
		$actual_total = poocommerce_store_api_get_formatter( 'money' )->format(
		...
		throw new RouteException(
			'poocommerce_rest_checkout_total_mismatch',
			sprintf(
				/* translators: %1$s: total the shopper confirmed, %2$s: updated, higher total */
				esc_html__( 'The order total changed from %1$s to %2$s while you were checking out. Please review the updated total and place your order again.', 'poocommerce' ),
