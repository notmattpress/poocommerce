	...
	public function validate_required_additional_fields( \WP_REST_Request $request ) {
		...
		if ( WC()->cart->needs_shipping() ) {
			foreach ( $address_fields as $field_key => $field ) {
				if ( ! isset( $request['shipping_address'][ $field_key ] ) && $this->additional_fields_controller->is_field_required( $field, $document_object, 'shipping_address' ) ) {
					...
					throw new RouteException( 'poocommerce_rest_checkout_missing_required_field', esc_html( sprintf( __( 'There was a problem with the provided shipping address: %s is required', 'poocommerce' ), $field['label'] ) ), 400 );
				}
			}
		}
		...
		foreach ( $address_fields as $field_key => $field ) {
			if ( ! isset( $request['billing_address'][ $field_key ] ) && $this->additional_fields_controller->is_field_required( $field, $document_object, 'billing_address' ) ) {
				/* translators: %s: is the field label */
				throw new RouteException( 'poocommerce_rest_checkout_missing_required_field', esc_html( sprintf( __( 'There was a problem with the provided billing address: %s is required', 'poocommerce' ), $field['label'] ) ), 400 );
			}
		}
		...
		foreach ( $other_fields as $field_key => $field ) {
			if ( ! isset( $request['additional_fields'][ $field_key ] ) && $this->additional_fields_controller->is_field_required( $field, $document_object ) ) {
				/* translators: %s: is the field label */
				throw new RouteException( 'poocommerce_rest_checkout_missing_required_field', esc_html( sprintf( __( 'There was a problem with the provided additional fields: %s is required', 'poocommerce' ), $field['label'] ) ), 400 );
			}
		}
	}
