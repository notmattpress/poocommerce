...
class CheckoutSessionsUpdate extends AbstractCartRoute {
	...
	protected function has_cart_token( \WP_REST_Request $request ) {
		...
		if ( true === $this->has_cart_token ) {
			...
			// phpcs:ignore PooCommerceStoreApi.StoreApi.CartTokenSource.ServerSuperglobalWrite -- $session_id was validated above; this keeps the consumed token in sync with the validated one.
