...
All the code in this directory (`Automattic\PooCommerce\Api` namespace and nested namespaces) is part of [an experimental feature](https://github.com/poocommerce/poocommerce/pull/63772). The code could (and probably will) get backwards-incompatible changes, or even be completely removed, in future releases of PooCommerce.
...
Also as a reminder, **ALL** the code that's inside the `Automattic\PooCommerce\Internal` namespace and nested namespaces, or that's annotated with `@internal`, is for exclusive usage of PooCommerce core and must **NEVER** be used in extensions or otherwise in production environments.
...
The public signatures on these classes use only `Api\*` types (and `Schema\*` for any engine-shaped objects), generated trees committed to plugin repos will stay compilable in the event of PooCommerce switching to a different underlying GraphQL engine in the future.
