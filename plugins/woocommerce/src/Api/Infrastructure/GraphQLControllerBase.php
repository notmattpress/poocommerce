...
use Automattic\PooCommerce\Internal\Api\StatusResolverFailedException;
...
/**
 * Handles incoming GraphQL requests over the PooCommerce REST API.
 *
 * Abstract: the autogenerated `GraphQLController` subclass emitted by
 * ApiBuilder (both for PooCommerce core and for sibling plugins reusing this
...
abstract class GraphQLControllerBase {
	 ...
	 * (alphanumerics, underscores, hyphens). Shared with {@see \Automattic\PooCommerce\Internal\Api\Settings::sanitize_endpoint_url()}
	 ...
	 *
	 * Typed as `?object` rather than a PooCommerce-defined interface so that
	 * sibling plugins do not have to import a PooCommerce type for what is
	...
	/**
	 * DI: injected by PooCommerce container.
	 *
	 * @internal
	 ...
	 * save by {@see \Automattic\PooCommerce\Internal\Api\Settings::sanitize_endpoint_url()}, so values that bypass
	...
	private function process_request( \WP_REST_Request $request, object $principal ): \WP_REST_Response {
		...
		$result->setErrorFormatter(
			function ( \Throwable $error ) use ( $debug_mode ): array {
				$formatted = \Automattic\PooCommerce\Vendor\GraphQL\Error\FormattedError::createFromException( $error );

				if ( ! isset( $formatted['extensions']['code'] ) ) {
					$client_safe                     = $error instanceof \Automattic\PooCommerce\Vendor\GraphQL\Error\ClientAware && $error->isClientSafe();
				...
				if ( 'BAD_USER_INPUT' !== ( $formatted['extensions']['code'] ?? null ) ) {
					...
					while ( $cursor instanceof \Throwable ) {
						if ( $cursor instanceof \Automattic\PooCommerce\Vendor\GraphQL\Error\SerializationError ) {
	 ...
	 * (both for PooCommerce core and for sibling plugins that reuse this
	 ...
	 * {@see \Automattic\PooCommerce\Api\Infrastructure\Principal}, that's
	...
	private function resolve_request_principal( \WP_REST_Request $request ): object {
		...
		if ( null === $fqcn ) {
			return new \Automattic\PooCommerce\Api\Infrastructure\Principal( wp_get_current_user() );
		}
	 ...
	 * through the {@see 'poocommerce_graphql_can_introspect'} filter so sites
	...
	private function is_introspection_allowed( ?object $principal, \WP_REST_Request $request ): bool {
		...
		try {
			 ...
			 */
			$can_introspect = apply_filters( 'poocommerce_graphql_can_introspect', $can_introspect, $principal, $request );
		} catch ( \Throwable $e ) {
			return false;
		}
	 ...
	 * passed through the {@see 'poocommerce_graphql_can_use_debug_mode'} filter.
	 *
	 * Fail-closed contract: the principal must be non-null (principal-resolution
	 * failures deny outright, before the filter is consulted), the principal
	 * method's return value is treated with `=== true`, and any throw from
	...
	private function is_debug_mode( ?object $principal, \WP_REST_Request $request ): bool {
		...
		try {
			 ...
			 * @param \WP_REST_Request $request   The REST request being processed.
			 */
			$can_debug = apply_filters( 'poocommerce_graphql_can_use_debug_mode', $can_debug, $principal, $request );
		} catch ( \Throwable $e ) {
			return false;
		}
