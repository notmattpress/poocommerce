...
This directory is the sole point of contact between autogenerated GraphQL code and the underlying GraphQL engine (currently webonyx/graphql-php, renamespaced under `Automattic\PooCommerce\Vendor\GraphQL\*`).
...
PooCommerce plugins that reuse this infrastructure commit their autogenerated trees to their own repos. If those trees imported from `Automattic\PooCommerce\Vendor\GraphQL\*`, an engine switch in PooCommerce would break every already-committed plugin. Routing every engine reference through this namespace prevents that: the generator emits imports from `Automattic\PooCommerce\Api\Infrastructure\Schema\*` only, and the classes here translate to whichever engine is current.
...
If PooCommerce switches off webonyx, the changes localized to this directory are:
