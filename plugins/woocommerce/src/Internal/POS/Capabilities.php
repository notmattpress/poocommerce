 ...
 *
 * POS access is defined entirely by `poocommerce_pos_*` capabilities granted per-user — the
 * same primitive WordPress uses for every other authorization decision. A user
 * has POS access if and only if they hold at least one of the known `poocommerce_pos_*`
 ...
 * `poocommerce_pos_*` caps and assigns or clears them per user via
 * set_pos_preset(). The assigned preset is recorded in `poocommerce_pos_preset`
...
class Capabilities {
	 ...
	 *
	 * POS access is keyed on `poocommerce_pos_*` capabilities, not this role (see
	 ...
	 * response — no shadow permission store. All share the `poocommerce_pos_`
	...
	public const CAP_PROCESS_SALES = 'poocommerce_pos_process_sales';
	// Look up and view existing orders.
	public const CAP_VIEW_ORDERS = 'poocommerce_pos_view_orders';
	// Apply an existing coupon to a cart.
	public const CAP_APPLY_COUPONS = 'poocommerce_pos_apply_coupons';
	// Create a new coupon during a sale.
	public const CAP_CREATE_COUPONS = 'poocommerce_pos_create_coupons';
	// Refund a paid order.
	public const CAP_ISSUE_REFUNDS = 'poocommerce_pos_issue_refunds';
	// View POS settings (read-only).
	public const CAP_VIEW_SETTINGS = 'poocommerce_pos_view_settings';
	// Change POS settings.
	public const CAP_EDIT_SETTINGS = 'poocommerce_pos_edit_settings';
	// Manage POS staff and their access.
	public const CAP_MANAGE_STAFF = 'poocommerce_pos_manage_staff';
	// Leave POS mode for the full admin.
	public const CAP_EXIT_POS = 'poocommerce_pos_exit';
	 ...
	 * is not the authorization signal: has_pos_access() reads the `poocommerce_pos_*` caps, not
	 ...
	 */
	public const POS_PRESET_META_KEY = 'poocommerce_pos_preset';

	/**
	 * All known POS capability identifiers.
	 *
	 * The canonical list of `poocommerce_pos_*` caps — used to test for POS access and, by the
	 ...
	 *
	 * True if the user holds at least one of the known `poocommerce_pos_*` capabilities (those
	 ...
	 * (individual `poocommerce_pos_*` caps assigned without a baseline cap).
	 *
	 ...
	 * explicit `poocommerce_pos_*` cap like anyone else.
	 *
	 ...
	 *
	 * Returns the `poocommerce_pos_preset` meta value only if it matches an
	 ...
	 * `poocommerce_pos_*` capability, via WP_User_Query's capability__in.
	 *
	...
	/**
	 * Preset metadata: the `poocommerce_pos_*` cap bundle and display label for each preset.
	 *
	 ...
	 *     poocommerce_pos_process_sales    yes      yes      yes
	 *     poocommerce_pos_view_orders      yes      yes      yes
	 *     poocommerce_pos_apply_coupons    yes      yes      yes
	 *     poocommerce_pos_create_coupons   no       yes      yes
	 *     poocommerce_pos_issue_refunds    no       yes      yes
	 *     poocommerce_pos_view_settings    no       yes      yes
	 *     poocommerce_pos_edit_settings    no       no       yes
	 *     poocommerce_pos_manage_staff     no       no       yes
	 *     poocommerce_pos_exit             no       no       yes
	 *
	...
	private static function preset_definitions(): array {
		...
		return array(
			POSPreset::CASHIER => array(
				'caps'  => $cashier_caps,
				'label' => __( 'POS cashier', 'poocommerce' ),
			),
			POSPreset::MANAGER => array(
				'caps'  => $manager_caps,
				'label' => __( 'POS manager', 'poocommerce' ),
			),
			POSPreset::ADMIN   => array(
				'caps'  => $admin_caps,
				'label' => __( 'POS admin', 'poocommerce' ),
			),
		);
	}

	/**
	 * The `poocommerce_pos_*` capability bundle for a given preset (see preset_definitions()).
	 *
	 ...
	 * `poocommerce_pos_*` cap the user holds directly (via add_cap — the only way this class grants
	 ...
	 * Role-granted `poocommerce_pos_*` caps, if any, are out of scope: this class never adds caps
	 * to roles.
	 *
	 * @param int         $user_id Target user.
	...
	public static function set_pos_preset( int $user_id, ?string $preset ): bool {
		...
		// Strip the user's directly-held poocommerce_pos_* caps so a preset change (or clear)
		...
		// so the key still matches the poocommerce_% uninstall sweep).
