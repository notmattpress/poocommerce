...
namespace Automattic\PooCommerce\Internal\RestApi\Routes\V4\Products;

use Automattic\PooCommerce\Enums\CatalogVisibility;
use Automattic\PooCommerce\Enums\DimensionUnit;
...
use Automattic\PooCommerce\Internal\RestApi\ProductRequestPreparationTrait;
...
class Controller extends WC_REST_Products_V2_Controller {
	 ...
	 * https://github.com/poocommerce/poocommerce/pull/61470
	 *
	...
	public function get_item_permissions_check( $request ) {
		...
		if ( $object && 0 !== $object->get_id() ) {
			...
			*/
			$permission = apply_filters( 'poocommerce_rest_check_permissions', $permission, 'read', $object_id, $object->post_type );
			...
			foreach ( $images as $index => $image ) {
				...
				if ( 0 === $attachment_id && isset( $image['src'] ) ) {
					...
					if ( is_wp_error( $upload ) ) {
						 ...
						 */
						if ( ! apply_filters( 'poocommerce_rest_suppress_image_upload_error', false, $upload, $product->get_id(), $images ) ) { // phpcs:ignore PooCommerce.Commenting.CommentHooks.MissingSinceComment
							throw new WC_REST_Exception( 'poocommerce_product_image_upload_error', $upload->get_error_message(), 400 ); // phpcs:ignore WordPress.Security.EscapeOutput.ExceptionNotEscaped
						} else {
							continue;
						}
					}
				...
				if ( ! wp_attachment_is_image( $attachment_id ) ) {
					...
					throw new WC_REST_Exception( 'poocommerce_product_invalid_image_id', sprintf( __( '#%s is an invalid image ID.', 'poocommerce' ), $attachment_id ), 400 ); // phpcs:ignore WordPress.Security.EscapeOutput.ExceptionNotEscaped
				}
	...
	protected function prepare_object_for_database( $request, $creating = false ) {
		 ...
		 * @param bool            $creating If is creating a new object.
		 */
		return apply_filters( "poocommerce_rest_pre_insert_{$this->post_type}_object", $product, $request, $creating ); // phpcs:ignore PooCommerce.Commenting.CommentHooks.MissingSinceComment
	}
	...
	public function get_item_schema() {
		$weight_unit_label    = I18nUtil::get_weight_unit_label( get_option( 'poocommerce_weight_unit', WeightUnit::KILOGRAM ) );
		$dimension_unit_label = I18nUtil::get_dimensions_unit_label( get_option( 'poocommerce_dimension_unit', DimensionUnit::CENTIMETER ) );
		$schema               = array(
		...
		$schema['properties']['add_to_cart'] = array(
			'description' => __( 'Add to cart details.', 'poocommerce' ),
			'type'        => 'object',
			'context'     => array( 'view', 'edit' ),
			'properties'  => array(
				'url'         => array(
				...
				'description' => array(
					'description' => __( 'Add to cart description.', 'poocommerce' ),
					'type'        => 'string',
					'context'     => array( 'view', 'edit' ),
				),
				'text'        => array(
					'description' => __( 'Add to cart text.', 'poocommerce' ),
					'type'        => 'string',
					'context'     => array( 'view', 'edit' ),
				),
				'single_text' => array(
					'description' => __( 'Add to cart single text.', 'poocommerce' ),
					'type'        => 'string',
					'context'     => array( 'view', 'edit' ),
				),
			),
	...
	private function add_embed_context_to_schema( array $schema ): array {
		...
		// view fields embeddable for now; scalability is tracked in https://github.com/poocommerce/poocommerce/issues/64652.
	...
	public function get_collection_params() {
		...
		$params['stock_status'] = array(
			'description'       => __( 'Limit result set to products with any of the specified stock statuses.', 'poocommerce' ),
		...
		$params['exclude_category'] = array(
			'description'       => __( 'Exclude products that belong to specific product category IDs.', 'poocommerce' ),
			'type'              => 'array',
			'items'             => array(
				'type' => 'integer',
			),
		...
		$params['min_stock_quantity'] = array(
			'description'       => __( 'Limit result set to products with stock quantity greater than or equal to the specified amount.', 'poocommerce' ),
			'type'              => wc_is_stock_amount_integer() ? 'integer' : 'number',
		...
		$params['max_stock_quantity'] = array(
			'description'       => __( 'Limit result set to products with stock quantity less than or equal to the specified amount.', 'poocommerce' ),
			'type'              => wc_is_stock_amount_integer() ? 'integer' : 'number',
			'sanitize_callback' => 'wc_stock_amount',
			'validate_callback' => 'rest_validate_request_arg',
		);
	...
	public function get_suggested_products_collection_params() {
		...
		$params['categories'] = array(
			'description'       => __( 'Limit result set to specific product category ids.', 'poocommerce' ),
			'type'              => 'array',
			'items'             => array(
				'type' => 'integer',
			),
