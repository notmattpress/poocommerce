...
class OrderItemSchema extends AbstractLineItemSchema {
	...
	public function get_item_schema_properties(): array {
		$schema = array(
			...
			'can_be_refunded' => array(
				'description' => __( 'Whether the line item can be refunded. True when the item has a product and its ordered quantity has not been fully refunded.', 'poocommerce' ),
				'type'        => 'boolean',
				'context'     => self::VIEW_EDIT_EMBED_CONTEXT,
				'readonly'    => true,
			),
	...
	public function get_item_response( $order_item, WP_REST_Request $request, array $include_fields = array() ): array {
		...
		$data            = array(
			...
			'currency'        => $currency,
			'currency_symbol' => html_entity_decode( get_poocommerce_currency_symbol( $currency ), ENT_QUOTES ),
		);
