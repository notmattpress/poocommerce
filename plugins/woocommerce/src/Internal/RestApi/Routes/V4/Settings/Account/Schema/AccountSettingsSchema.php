...
namespace Automattic\PooCommerce\Internal\RestApi\Routes\V4\Settings\Account\Schema;
...
class AccountSettingsSchema extends AbstractSchema {
	...
	public function get_item_response( $item, WP_REST_Request $request, array $include_fields = array() ): array {
		...
		$response = array(
			'id'          => 'account',
			'title'       => __( 'Accounts & Privacy', 'poocommerce' ),
			'description' => __( 'Set options relating to customer accounts and data privacy.', 'poocommerce' ),
			'values'      => $values,
			'groups'      => $groups,
		);
