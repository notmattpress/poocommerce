...
class Settings {
	...
	public static function get_default_excluded_order_statuses(): array {
		 ...
		 */
		$filtered = apply_filters( 'poocommerce_analytics_settings_default_excluded_order_statuses', $builtin_default );
	...
	public static function get_default_actionable_order_statuses(): array {
		 ...
		 */
		$filtered = apply_filters( 'poocommerce_analytics_settings_default_actionable_order_statuses', $builtin_default );
	...
	public function add_component_settings( $settings ) {
		...
		// Performance note: refer back to https://github.com/poocommerce/poocommerce/pull/41092: unconditionally loading /jetpack/v4/connection.
	...
	public function add_settings( $settings ) {
		...
		$settings[] = array(
			'id'          => 'poocommerce_analytics_scheduled_import',
			'option_key'  => 'poocommerce_analytics_scheduled_import',
			'label'       => __( 'Updates', 'poocommerce' ),
			'description' => __( 'Controls how analytics data is imported from orders.', 'poocommerce' ),
			'type'        => 'radio',
			// Default to null so we can know if it's a new site or an existing site. New sites will have the option set.
			'default'     => null,
			'options'     => array(
				'yes' => __( 'Scheduled (recommended)', 'poocommerce' ),
				'no'  => __( 'Immediately', 'poocommerce' ),
			),
		);

		// Add hidden setting for the import interval to display in the client side.
		$import_interval = \Automattic\PooCommerce\Internal\Admin\Schedulers\OrdersScheduler::get_import_interval();
		...
		$settings[]             = array(
			'id'         => 'poocommerce_analytics_import_interval',
			'option_key' => 'poocommerce_analytics_import_interval',
			'type'       => 'hidden',
