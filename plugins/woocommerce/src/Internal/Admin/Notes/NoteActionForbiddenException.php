...
/**
 * Thrown by internal PooCommerce handlers on the `poocommerce_note_action[_*]`
 * hooks to signal that the current user lacks the per-action capability the
 * handler enforces.
 *
 * `Automattic\PooCommerce\Admin\API\NoteActions::trigger_note_action()` catches
 ...
 * `poocommerce_note_action_<name>` hook are silently skipped, and
 ...
 * `poocommerce_note_action[_*]` should not rely on this class — its API and
