...
class Analytics {
	 ...
	 * @since 11.2.0
	 */
	const REFUND_DOUBLE_COUNT_TOOL_ID = 'fix_poocommerce_analytics_refund_double_count';
	 ...
	 * @since 11.2.0
	 */
	const REFUND_DOUBLE_COUNT_OPTION = 'poocommerce_analytics_refund_double_count';
	 ...
	 * @since 11.2.0
	 */
	const REFUND_DOUBLE_COUNT_FIX_HOOK = 'poocommerce_analytics_refund_double_count_fix_batch';
	...
	public function __construct() {
		...
		add_filter( 'poocommerce_debug_tools', array( $this, 'register_refund_double_count_tool' ) );
		add_action( self::REFUND_DOUBLE_COUNT_FIX_HOOK, array( $this, 'process_refund_double_count_fix_batch' ), 10, 2 );
		add_action( 'poocommerce_analytics_regenerate_init', array( $this, 'maybe_cancel_refund_double_count_fix_on_regenerate' ), 10, 2 );
	...
	public function run_full_refund_fix_data_tool() {
		...
		if ( self::is_batch_pending_or_running( 'poocommerce_analytics_refund_fix_batch' ) ) {
		...
		self::schedule_batch( 'poocommerce_analytics_refund_fix_batch', array( 0 ) );
	...
	public function process_refund_fix_batch( $min_order_id = 0 ): void {
		...
		if ( count( $refunded_orders ) >= 100 ) {
			...
			self::schedule_batch( 'poocommerce_analytics_refund_fix_batch', array( $last_order_id ), 5 );
		}
	}
	 ...
	 *
	 * Only stores that use the new full refund data and ran PooCommerce before 11.1.0,
	...
	/**
	 * Register the double-counted refunds fix tool on the PooCommerce > Status > Tools page.
	 *
	...
	public function register_refund_double_count_tool( $debug_tools ) {
		...
		$button      = __( 'Check and fix', 'poocommerce' );
		$disabled    = false;
		$status_text = '';

		if ( self::is_refund_double_count_fix_running( $state ) ) {
			$button      = __( 'Checking and fixing…', 'poocommerce' );
			$disabled    = true;
			$status_text = sprintf(
				/* translators: %d: number of orders fixed so far. */
				_n( '%d order fixed so far.', '%d orders fixed so far.', $state['fixed'], 'poocommerce' ),
				$state['fixed']
			);
			...
			$status_text = sprintf(
				...
				_n( '%d order could not be fixed. See the wc-analytics-order-import log for details.', '%d orders could not be fixed. See the wc-analytics-order-import log for details.', $state['unresolved'], 'poocommerce' ),
				$state['unresolved']
			);
		} elseif ( self::REFUND_DOUBLE_COUNT_STATUS_COMPLETE === $state['status'] ) {
			$button      = __( 'Dismiss', 'poocommerce' );
			$status_text = 0 === $state['fixed']
				? __( 'No affected orders were found.', 'poocommerce' )
				: sprintf(
					/* translators: 1: number of orders fixed, 2: date the fix finished. */
					_n( 'Fixed %1$d order on %2$s.', 'Fixed %1$d orders on %2$s.', $state['fixed'], 'poocommerce' ),
					$state['fixed'],
					wp_date( wc_date_format(), $state['completed_at'] )
				);
		} elseif ( '' !== $state['status'] ) {
			$status_text = __( 'The previous run did not finish.', 'poocommerce' );
		}

		$debug_tools[ self::REFUND_DOUBLE_COUNT_TOOL_ID ] = array(
			'name'             => __( 'Fix double-counted refunds in Analytics', 'poocommerce' ),
			'button'           => $button,
			'desc'             => __( 'This tool finds orders where a partial refund followed by a full refund was counted twice in the Analytics returns, and re-imports them. Only orders refunded before PooCommerce 11.1 can be affected.', 'poocommerce' ),
	...
	public function run_refund_double_count_tool() {
		...
		if ( self::is_batch_pending_or_running( 'poocommerce_analytics_refund_fix_batch' ) ) {
			...
			return __( 'The full refund data fix is still running. Please try again once it has finished.', 'poocommerce' );
		}
		...
		return __( 'Checking for affected orders and fixing them in the background. Reload this page to see the progress.', 'poocommerce' );
	}
	...
	private static function get_refund_double_count_batch_size(): int {
		 ...
		 */
		return max( 1, (int) apply_filters( 'poocommerce_analytics_refund_double_count_batch_size', self::REFUND_DOUBLE_COUNT_BATCH_SIZE ) );
	}
	...
	private static function get_refund_double_count_range_size(): int {
		 ...
		 */
		return max( 1, (int) apply_filters( 'poocommerce_analytics_refund_double_count_range_size', self::REFUND_DOUBLE_COUNT_RANGE_SIZE ) );
	}
	...
	public function ajax_check_refund_fix_needed(): void {
		...
		$fix_in_progress = self::is_batch_pending_or_running( 'poocommerce_analytics_refund_fix_batch' );
	...
	public static function get_report_pages() {
		$overview_page = array(
			...
			'position' => 57,
		// After PooCommerce & Product menu items.
		);
