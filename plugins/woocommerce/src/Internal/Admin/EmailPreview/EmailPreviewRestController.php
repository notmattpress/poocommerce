...
class EmailPreviewRestController extends RestApiControllerBase {
	...
	public function send_email_preview( WP_REST_Request $request ) {
		...
		try {
			...
			ob_end_clean();
			return new WP_Error(
				'poocommerce_rest_email_preview_not_rendered',
				__( 'There was an error rendering an email preview.', 'poocommerce' ),
				array( 'status' => 500 )
			);
		}
		ob_end_clean();
		...
		// (e.g. poocommerce_email_sent) and log entries when the test email is sent.
		$email->recipient = $email_address;
		$mark_as_test     = function ( array $context ): array {
			$context['is_test'] = true;
			return $context;
		};
		add_filter( 'poocommerce_email_log_context', $mark_as_test );
		try {
			$sent = $email->send( $email_address, $email_subject, $email_content, $email->get_headers(), $email->get_attachments() );
		} finally {
			remove_filter( 'poocommerce_email_log_context', $mark_as_test );
		}
