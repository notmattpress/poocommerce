...
class EmailPreview {
	...
	public function set_up_filters() {
		...
		add_filter( 'poocommerce_order_needs_shipping_address', array( $this, 'enable_shipping_address' ), 10, 3 );
		...
		add_filter( 'poocommerce_order_get_downloadable_items', array( $this, 'get_dummy_downloadable_items' ) );
	}
	...
	private function should_show_shipping_details( $order = null ) {
		 ...
		 */
		return (bool) apply_filters( 'poocommerce_email_preview_show_shipping_details', true, $order, $this->email_type );
	}
	...
	public function provide_dummy_product_file( $file ) {
		...
		if ( apply_filters( 'poocommerce_is_email_preview', false ) ) {
			...
			$dummy_file->set_name( __( 'Sample Download File.pdf', 'poocommerce' ) );
