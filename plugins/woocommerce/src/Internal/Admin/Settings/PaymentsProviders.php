...
use Automattic\PooCommerce\Internal\Admin\Settings\PaymentsProviders\Eway;
use Automattic\PooCommerce\Internal\Admin\Settings\PaymentsProviders\GoCardless;
use Automattic\PooCommerce\Internal\Admin\Settings\PaymentsProviders\Helcim;
...
use Automattic\PooCommerce\Internal\Admin\Settings\PaymentsProviders\KustomCheckout;
use Automattic\PooCommerce\Internal\Admin\Settings\PaymentsProviders\Mastercard;
...
use Automattic\PooCommerce\Internal\Admin\Settings\PaymentsProviders\NexiCheckout;
...
use Automattic\PooCommerce\Internal\Admin\Settings\PaymentsProviders\Visa;
...
class PaymentsProviders {
	...
	private const GATEWAY_DETAILS_REQUEST_CACHE_GROUP = 'poocommerce_payment_gateway_details';
	private const GATEWAY_DETAILS_REQUEST_CACHE_KEY   = 'gateway_details';

	public const PROVIDER_LISTS_REQUEST_CACHE_GROUP = 'poocommerce_payments_providers';
		...
		if ( ! is_null( $provider_class ) && ! is_subclass_of( $provider_class, PaymentGateway::class ) ) {
			wc_doing_it_wrong(
				...
				sprintf(
					...
					esc_html__( 'The provider class for gateway ID "%s" must extend the PaymentGateway class.', 'poocommerce' ),
					$gateway_id
				),
				'10.4.0'
			);
			...
			if ( ! is_subclass_of( $this->payment_extension_suggestions_providers_class_map[ $pes_id ], PaymentGateway::class ) ) {
				wc_doing_it_wrong(
					...
					sprintf(
						...
						esc_html__( 'The provider class for payment extension suggestion ID "%s" must extend the PaymentGateway class.', 'poocommerce' ),
						$pes_id
					),
					'10.4.0'
				);
