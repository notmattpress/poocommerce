...
class Settings {
			...
			if ( false === $realpath ) {
				...
				if ( true === $result ) {
					...
					try {
						...
						error_log( 'PooCommerce: could not write log directory protection files: ' . $exception->getMessage() ); // phpcs:ignore WordPress.PHP.DevelopmentFunctions.error_log_error_log
					}
				}
			}
		}
	...
	private function get_filesystem_settings_definitions(): array {
		...
		if ( wp_is_writable( $directory ) ) {
			...
			$status_info[] = __( '⚠️ The log directory is not writable.', 'poocommerce' );
