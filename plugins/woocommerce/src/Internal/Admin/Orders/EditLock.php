...
class EditLock {
	...
	public function refresh_lock_ajax( $response, $data ) {
		...
		if ( ! $order || ! is_a( $order, \WC_Order::class ) || ( ! current_user_can( get_post_type_object( $order->get_type() )->cap->edit_post, $order->get_id() ) && ! current_user_can( 'manage_poocommerce' ) ) ) {
			return $response;
		}
