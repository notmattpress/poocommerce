	...
	public function sync_now() {
		...
		if ( 'sync-now' === $action ) {
			if ( ! $this->data_synchronizer->check_orders_table_exists() && ! $this->data_synchronizer->create_database_tables() ) {
				WC_Admin_Settings::add_error(
					__( 'Unable to create HPOS tables for synchronization.', 'poocommerce' )
				);
				return;
			}
		...
		$get_sync_message = function () {
			...
			if ( $is_dangerous ) {
				$sync_message[] = wp_kses_data(
					__( "There are orders pending sync.", 'poocommerce' )
					. '<strong>'
			...
			if ( $sync_in_progress && $sync_is_pending ) {
				...
				if ( ! $is_dangerous ) {
					$sync_message[] = wp_kses_data(
						__( "You can switch order data storage <strong>only when the posts and orders tables are in sync</strong>. There are currently orders out of sync.", 'poocommerce' ),
					);
				}
