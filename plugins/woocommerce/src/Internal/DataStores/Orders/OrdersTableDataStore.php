...
",
	...
	public function read_multiple( &$orders ) {
		...
		$data_sync_enabled = $data_synchronizer->data_sync_is_enabled()
			&& ! doing_action( 'poocommerce_deliver_webhook_async' )
		...
		if ( $data_sync_enabled ) {
			 ...
			 * @since 8.1.0
			 */
			$data_sync_enabled = apply_filters( 'poocommerce_hpos_enable_sync_on_read', false );
		}
	...
	public function filter_raw_meta_data( &$object, $raw_meta_data ) { // phpcs:ignore Universal.NamingConventions.NoReservedKeywordParameterNames.objectFound
		 ...
		 * poocommerce_data_store_wp_post_read_meta filter (orders use the 'post' meta type), which
...
FROM $order_meta_table
	...
	private function save_cogs_data( WC_Abstract_Order $order, bool $cogs_value_changed ): void {
		 ...
		 */
		$cogs_value = apply_filters( 'poocommerce_save_order_cogs_value', $cogs_value_original, $order );
		if ( null === $cogs_value ) {
			return;
		}
	...
	public function untrash_order( WC_Order $order ): bool {
		...
		// listeners on the poocommerce_order_status_* actions. Suppress dispatch for this order
	...
	public function query( $query_vars ) {
		 ...
		 * @since 10.4.0
		 */
		$query_vars = apply_filters( 'poocommerce_orders_table_datastore_get_orders_query', $query_vars, $this );
