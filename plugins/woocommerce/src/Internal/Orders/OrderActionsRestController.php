...
class OrderActionsRestController extends RestApiControllerBase {
	...
	private function get_args_for_order_actions( string $action_slug ): array {
		...
		if ( 'send_email' === $action_slug ) {
			$args['template_id'] = array(
				'description'       => __( 'The email template to use. If omitted, the best template is auto-selected based on order status.', 'poocommerce' ),
				'type'              => 'string',
	...
	private function select_default_template( WC_Order $order, array $available_templates ): ?WC_Email {
		...
		$preferred_template_ids = apply_filters(
			'poocommerce_rest_order_actions_email_preferred_template_ids',
	...
	protected function send_email( WP_REST_Request $request ) {
		...
		if ( empty( $template_id ) ) {
			...
			if ( is_null( $template ) ) {
				return new WP_Error(
					'poocommerce_rest_no_email_template',
					__( 'No email template is available for this order.', 'poocommerce' ),
					array( 'status' => 400 )
				);
			}
