...
?>
<div class="wrap poocommerce poocommerce-customer-stock-notifications">

	<h1 class="wp-heading-inline"><?php esc_html_e( 'Edit Notification', 'poocommerce' ); ?></h1>
	<a href="<?php echo esc_url( NotificationsPage::PAGE_URL ); ?>" class="page-title-action"><?php esc_html_e( 'View All', 'poocommerce' ); ?></a>
	...
	<?php wp_nonce_field( 'poocommerce-customer-stock-notification-edit', 'customer_stock_notification_edit_security' ); ?>
	...
	<div id="poststuff">
		<div id="post-body" class="columns-2">
			...
			<div id="postbox-container-1" class="postbox-container">
				...
				<div id="poocommerce-order-actions" class="postbox">

					<h2 class="hndle ui-sortable-handle"><span><?php esc_html_e( 'Notification actions', 'poocommerce' ); ?></span></h2>
						...
						<ul class="order_actions submitbox">
							...
							<li class="wide" id="actions">
								<select name="wc_customer_stock_notification_action">
									...
									<?php if ( $notification->get_status() === NotificationStatus::ACTIVE ) : ?>
										<option value="send_notification"><?php esc_html_e( 'Send', 'poocommerce' ); ?></option>
										<option value="cancel_notification"><?php esc_html_e( 'Cancel', 'poocommerce' ); ?></option>
									<?php elseif ( $notification->get_status() === NotificationStatus::PENDING ) : ?>
										<option value="send_verification_email"><?php esc_html_e( 'Resend verification email', 'poocommerce' ); ?></option>
										<option value="activate_notification"><?php esc_html_e( 'Activate', 'poocommerce' ); ?></option>
							...
							<li class="wide">
								<div id="delete-action">
									<a class="submitdelete deletion" href="<?php echo esc_url( wp_nonce_url( admin_url( sprintf( NotificationsPage::PAGE_URL . '&notification_action=delete&notification_id=%d', $notification->get_id() ) ), 'delete_customer_stock_notification' ) ); ?>"><?php esc_html_e( 'Delete permanently', 'poocommerce' ); ?></a>
								</div>

								<button type="submit" class="button save_order button-primary" name="save" value="<?php esc_attr_e( 'Update', 'poocommerce' ); ?>"><?php esc_html_e( 'Update', 'poocommerce' ); ?></button>
							</li>
				...
				<div id="notification-data" class="postbox notification-data">
					...
					<div class="notification-data__row notification-data__row--columns">
						...
						<div class="notification-data__header-column">
							...
							<h2 class="notification-data__header">
								...
								echo esc_html( sprintf( __( 'Notification #%d details', 'poocommerce' ), $notification->get_id() ) );
								?>
							</h2>
					...
					<div class="notification-data__row notification-data__row--columns">
						...
						<div class="notification-data__form-field">
							<label><?php esc_html_e( 'Customer', 'poocommerce' ); ?></label>
							<?php
							$user_string = '&mdash;';
							...
							<div class="form-field__actions">
								<?php if ( isset( $user ) && is_a( $user, 'WP_User' ) ) { ?>
									<a href="<?php echo esc_url( get_edit_user_link( $user->ID ) ); ?>"><?php esc_html_e( 'View profile &rarr;', 'poocommerce' ); ?></a>
								<?php } ?>
								<a href="<?php echo esc_url( admin_url( NotificationsPage::PAGE_URL . '&s=' . rawurlencode( $notification->get_user_email() ) ) ); ?>"><?php esc_html_e( 'View notifications &rarr;', 'poocommerce' ); ?></a>
							</div>
						</div>

						<div class="notification-data__form-field">

							<label><?php esc_html_e( 'Product', 'poocommerce' ); ?></label>
							...
							<div class="notification-data__product-data">
								...
								if ( is_a( $product, 'WC_Product' ) ) {
									...
									?>
									<small><?php esc_html_e( 'Product not found.', 'poocommerce' ); ?></small>
									<?php
								}
								?>
							</div>
						...
						<div class="notification-data__row notification-data__row--columns">
							...
							<div class="notification-data__meta-column">
								<div class="notification-data__meta-data">
									<label><?php esc_html_e( 'Waiting', 'poocommerce' ); ?></label>
									<span>
										<?php
										if ( ! $notification->get_date_created() || $notification->get_status() !== 'active' ) {
											...
											$t_time                 = date_i18n( _x( 'Y/m/d g:i:s a', 'list table date hover format', 'poocommerce' ), $date_created_timestamp );
								...
								<div class="notification-data__meta-data">
									<label><?php esc_html_e( 'Signed up', 'poocommerce' ); ?></label>
									<?php
							...
							<div class="notification-data__meta-column">
								<div class="notification-data__meta-data">
									<label><?php esc_html_e( 'Signed-up customers', 'poocommerce' ); ?></label>
									<span>
										<?php
										...
										if ( $signed_up_customers > 0 ) {
											?>
											<a href="<?php echo esc_attr( add_query_arg( array( 'customer_stock_notifications_product_filter' => $notification->get_product_id() ), NotificationsPage::PAGE_URL ) ); ?>"><?php esc_html_e( 'View notifications &rarr;', 'poocommerce' ); ?></a>
										<?php } ?>
									</span>
								</div>
								<?php
								...
								if ( ! empty( $attributes ) ) {
									?>
									<div class="notification-data__meta-data">
										<label><?php esc_html_e( 'Attributes', 'poocommerce' ); ?></label>
										<span>
