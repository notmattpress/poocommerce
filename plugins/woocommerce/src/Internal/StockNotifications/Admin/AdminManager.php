...
class AdminManager {
	...
	public static function admin_resources() {
		...
		$params = array(
			'i18n_wc_delete_notification_warning'       => __( 'Delete this notification permanently?', 'poocommerce' ),
			'i18n_wc_bulk_delete_notifications_warning' => __( 'Delete the selected notifications permanently?', 'poocommerce' ),
		);
		...
		if (
			! in_array(
				$screen_id,
				array( 'poocommerce_page_wc-customer-stock-notifications', 'poocommerce_page_wc-settings' ),
				true
			)
		) {
			return;
		...
		if ( 'poocommerce_page_wc-settings' === $screen_id && isset( $_GET['section'] ) && 'customer_stock_notifications' !== $_GET['section'] ) {
			return;
		}
