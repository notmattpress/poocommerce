...
class Integration {
	...
	public function register_hooks() {
		...
		add_filter( 'poocommerce_email_editor_send_preview_email', array( $this, 'send_preview_email_for_email_type' ), 10, 1 );
		add_filter( 'poocommerce_email_editor_send_preview_email_without_post_permission', array( $this, 'authorize_postless_send_preview' ), 10, 2 );
	...
	public function render_preview_html_for_email_type( string $email_type ): array {
		...
		add_filter( 'poocommerce_email_editor_rendering_email_context', array( $send_preview, 'add_preview_context' ) );
		try {
			...
			remove_filter( 'poocommerce_email_editor_rendering_email_context', array( $send_preview, 'add_preview_context' ) );
		}
	 ...
	 *
	 * Mirrors the `preview_poocommerce_mail` admin page pattern: a nonce-gated
	...
	public function render_block_email_preview_page(): void {
		...
		if ( ! current_user_can( 'manage_poocommerce' ) ) {
			wp_die( esc_html__( 'You do not have permission to preview emails.', 'poocommerce' ) );
		}
		...
		try {
			...
			wp_die( esc_html__( 'This email cannot be previewed.', 'poocommerce' ) );
		}
