...
- New hooks: use the version from `includes/class-poocommerce.php` on trunk, removing the `-dev` suffix
...
Hooks in `plugins/poocommerce/src/Blocks` and `plugins/poocommerce/src/StoreApi` are published as a developer reference generated from their docblocks:

- `plugins/poocommerce/client/blocks/docs/third-party-developers/extensibility/hooks/actions.md`
- `plugins/poocommerce/client/blocks/docs/third-party-developers/extensibility/hooks/filters.md`
...
```bash
pnpm --filter=@poocommerce/block-library build:docs
```
...
- Keeping it that way takes maintenance. The generator scans all of `plugins/poocommerce/src` except the directories listed under `extra.wp-hooks.ignore-files` in `plugins/poocommerce/client/blocks/composer.json`. When you add a new top-level directory to `src/`, add it to that list as well — otherwise the next `build:docs` run stops with an error the first time it finds a documented hook there.
...
See `plugins/poocommerce/client/blocks/bin/hook-docs/README.md` for how the pipeline works and how to change its scope.
